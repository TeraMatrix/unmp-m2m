...
def start_page(h):
    ...
    #"css/demo_table_jui.css
        ...
        end_time = datetime.time(datetime.now()).strftime("%H:%M")
        start_date = datetime.date(
            datetime.now() + timedelta(days=-30)).strftime("%d/%m/%Y")
        html.write("<div id=\"top_image_div\">")
        html.write(
            "<table class=\"addform\" style=\"border:0px None;height:43px;border-bottom:1px solid #AAA;\"><tr class=\"odd\">")

        html.write("<td style=\"font-weight: bold;width:70px;\">Alarm Type")
        html.write("</td>")

        html.write("<td style=\"width:20px;\">")
        html.write("<input type=\"radio\" value=\"3\" name=\"option\" id=\"option3\" class=\"table_option\" checked=\"checked\"/>")
        html.write("</td>")

        html.write("<td style=\"width:55px;\">")
        html.write("<label for=\"option3\">Notifications</label>")
        html.write("</td>")

        html.write("<td style=\"width:20px;display:none;\">")
        html.write(
            "<input type=\"radio\" value=\"1\" name=\"option\" id=\"option1\" class=\"table_option\"/>")
        html.write("</td>")

        html.write("<td style=\"width:60px;display:none;\">")
        html.write("<label for=\"option1\">Current Alarms</label>")
        html.write("</td>")
        ...
        html.write("<td style=\"width:70px;display:none;\">")
        html.write("<label for=\"option2\">Clear Alarms</label>")
        html.write("</td>")

        html.write("<td style=\"width:20px;display:none;\">")
        html.write("<input type=\"radio\" value=\"4\" name=\"option\" id=\"option4\" class=\"table_option\"/>")
        html.write("</td>")

        html.write("<td style=\"width:90px;display:none;\">")
        html.write("<label for=\"option4\">Reachability Alarms</label>")
        html.write("</td>")

        html.write(
            "<td style=\"width:auto;text-align:right;margin-right:10px;\">")
        html.write(
            "<input class=\"yo-button yo-small\" type=\"button\" value=\"Advanced Search\" id=\"btn_filter\" name=\"btn_filter\"/>")
        html.write("</td>")
        html.write("</tr></table>")
        html.write("</div>")

        html.write(
            "<div id=\"main_div\" class='form-div' style='margin-top:44px;margin-bottom:27px;'>")
        ...
        html_form = '<form id=\"alarm_info_form\" action=\"alarm_current_detail.py\" method=\"get\" style="padding:0;" >\
				...
				<div class=\"row-elem\">\
				    ...
				    <input type=\"text\" name=\"event_start_time\" value=\"%s\" id=\"event_start_time\" style=\"width:70px;\"/>\
				    <lable>--</lable>\
				    <input type=\"text\" name=\"event_end_date\" value=\"%s\" id=\"event_end_date\" style=\"width:70px;\"/>\
				    <input type=\"text\" name=\"event_end_time\" value=\"%s\" id=\"event_end_time\" style=\"width:70px;\"/>\
				</div>\
				<div class=\"row-elem\">\
        ...
        html.write(
            "<img src=\"images/lb.png\"  name=\"select_option_div\" alt=\"Informational\" title=\"Informational\" style=\"width:12px\" class=\"imgbutton\" /><span style=\"line-height:20px; padding:0px 12px;cursor:pointer;cursor:hand;\">Informational</span>")
        html.write("</div>")
        html.write("</td>")
...
def trap_filter_function(h):
    ...
    # html object(Trap_information page).
    event_type = html.var(
    ...
    st_time = html.var("event_start_time")
    st_date = st_date + " " + st_time.strip()
    start_date = datetime.strptime(st_date, "%d/%m/%Y %H:%M")
    # start_time = datetime.strptime(st_time,"%H:%M")
    end_date = html.var("event_end_date")
    end_time = html.var("event_end_time")
    end_date = end_date + " " + end_time.strip()
    ...
    if flag_value == 0:
        ...
        if (serevity2) != None and (serevity2) != "None":
            ...
            sql_1 += " or"
            sql_1 += " serevity like '%" + (
                'None' if serevity2 == None or serevity2.strip() == '' else '0') + "%'"
            sql_j += 1
        ...
        try:
            ...
            a_columns = ["ta.serevity", "STR_TO_DATE(ta.trap_receive_date,'%a %b %e %H:%i:%s %Y')", 
                         "ta.description","hosts.host_alias", "hosts.device_type_id",
                         "ta.trap_event_type", "ta.event_id",
                         "ta.agent_id", "ta.manage_obj_id", "ta.manage_obj_name", "ta.component_id",
            ...
            s_where = "WHERE  hostgroups.hostgroup_id IN (%s) and %s AND ta.timestamp >= '%s' and  ta.timestamp <='%s'" % (
            ...
            if agent_id == "" or agent_id == None:
                ...
                if "-" in agent_id:
                    s_where = "WHERE  hostgroups.hostgroup_id IN (%s) AND agent_id='%s' AND %s AND ta.timestamp >= '%s' and  ta.timestamp <='%s'" % (
            ...
            query = "SELECT SQL_CALC_FOUND_ROWS %s FROM %s %s %s %s %s" % (
        ...
        output_dict = {
            'success': 4, 'output': 'Access Restricted. Please request access from UNMP admin', "sEcho": 1,
            "iTotalRecords": 0,
...
def trap_detail_information(h):
    ...
    try:
        ...
        if len(result) > 0:
            ...
            html.write("</td>")
            html.write("<td>%s</td>" % result[0][0])
            html.write("</tr>")
            html.write("<tr>")
            html.write("<td>")
            html.write("<b>IP Address</b>")
            html.write("</td>")
            html.write("<td>%s</td>" % result[0][1])
            html.write("</tr>")
            ...
            html.write("</td>")
            html.write("<td>%s</td>" % str(result[0][2]))
            # html.write("<td>%s</td>"%(datetime.strptime(str(result[0][4]),'%Y-%m-%d
            # %H:%M:%S.%f').strftime('%d %b %y %I:%M:%S %p')))
            html.write("</tr>")
            ...
            html.write("</td>")
            html.write("<td>%s</td>" % image_title_name[result[0][4]])
            html.write("</tr>")
            ...
            html.write("</td>")
            # html.write("<td>%s</td>"%" ".join([ele.capitalize() for ele in
            # result[13].split(" ")]))
...
def trap_search_elements(h):
    ...
    try:
        ...
        if search_element != None or option != None:
            ...
            for row in result:
                trap_list.append(
                    {'Hidden': "", 'Name': '' if row[0] == None else row[0]})
            ...
            for option in range(1, 5):
                ...
                if report_type == 'excelReport':
                    xls_sheet = xls_book.add_sheet('%s %s' % (
                        report_option_dict[int(option)], sheet_count), cell_overwrite_ok=True)
                    ...
                    for k in range(len(merge_result)):
                        for j in range(len(merge_result[k])):
                            width = 5000
                            xls_sheet.write(
                                i, j, str(merge_result[k][j]), style1)
                            xls_sheet.col(j).width = width
                        if i == 60000:
                            sheet_count += 1
                            xls_sheet = xls_book.add_sheet('%s %s' % (report_option_dict[int(
                                option)], sheet_count), cell_overwrite_ok=True)
                            xls_sheet.row(0).height = 521
                            xls_sheet.row(1).height = 421
                            xls_sheet.write_merge(0, 0, 0, len(
                                headings) - 1, "%s" % report_option_dict[int(option)], style)
                            xls_sheet.write_merge(1, 1, 0, len(headings) - 1, " %s -- %s " % (
                                str(start_date)[:11], str(end_date)[:11]), style)
                            xls_sheet.write_merge(
                                2, 2, 0, len(headings) - 1, "", style)
                            i = 4
                            heading_xf = xlwt.easyxf(
                    ...
                    blank_row = ["", "", ""]
                    main_row = ["%s    %s -- %s " % (report_option_dict[int(
                        option)], str(start_date)[:11], str(end_date)[:11])]
                    second_row = ["", "", ""]
