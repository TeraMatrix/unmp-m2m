...
// Inventory messages
var messages = {
    "add": "Device added successfully.",
    "edit": "Device details updated successfully.",
    "del": "Selected host(s) deleted successfully",
    "delConfirm": "Are you sure want to delete the selected host(s)?",
    "duplicateError": "Please enter a different Device Name, Device Alias, IP  and MAC",
    ...
    "validationError": "Invalid Device details are entered, please recheck",
    ...
    "licenseDeviceError": "Maximum limit of allowed Device for this device type is reached",
...
function deviceTypeChange(deviceTypeSelectList) {
    deviceTypeSelectList.change(function () {
        var selectedDeviceType = $(this).val(),
            ipAddress = $formInput.eq($formInputIndex["ip_address"]).val();
            $monitoring = $("#monitoringing_parameter"),
            $monitoringDiv = $("#monitoringing_parameter_div"),
            $monitoringDetails = $("#monitoringing_parameter_details"),
            $monitoringDetailsDiv = $("#monitoringing_parameter_details_div");

        $("#device_details_div").hide();
        if ("ivis" == selectedDeviceType) {
            $("#device_details_div").show();
        }
        /*
        ...
        $monitoringDetailsDiv.hide();
        if ("" != selectedDeviceType) {
            $.ajax({
                type: "get",
                url: "get_service_parameters.py",
                data: {"device_type_id": selectedDeviceType},
                cache: false,
                success: function (result) {
                    $monitoringDiv.html(result.html);
                    $monitoringDetailsDiv.html(result.param_html);
                    if (void 0 != $.hostData) {
                        fillServiceParams($.hostData);
                    }
                    if (result.status) {
                        $monitoring.show();
                        $monitoringDiv.show();
                        $monitoringDetails.show();
                        $monitoringDetailsDiv.show();
                    }
                }
            });
        }
...
//            },
        ...
        messages: {
            host_name: {
                required: "Device name is a required field",
                alphaNumeric: "Device name should be alpha numeric",
                noSpace: "In Device name space are not allowed"
            },
            host_alias: {
                required: "Device alias is a required field",
                alphaNumeric: "Device alias should be alpha numeric"
            },
            ip_address: {
            ...
            host_state: {
                required: "Device state is a required field"
            },
            host_priority: {
                required: "Device priority is a required field"
            },
            host_parent: {
                required: "Device parent is a required field"
            },
            host_comment: {
            ...
            host_vendor: {
                required: "Device Vendor is a required field"
            },
//            longitude: {
//                number: "it should be a number",
...
//            },
            host_os: {
                required: "Device OS name is a required field"
            }
        }
    });
}
...
function setValues(details) {
    ...
    $formInput.eq($formInputIndex["phone_no"]).val(details["phone_no"]);
    ...
    $formInput.eq($formInputIndex["device_path"]).val(details["device_path"]);
    $formInput.eq($formInputIndex["number_of_ports"]).val(details["number_of_ports"]);
...
function fillServiceParams(details) {
    // manage service parameters
    var $serviceParameters = $("input[name*=service_parameter_]"), i, iln = $serviceParameters.length, $input,
    $seviceDetails = $("input[name*=service_]"), j, jln = $seviceDetails.length, data;
    for (i = 0; i < iln; i++) {
        $input = $serviceParameters.eq(i);
        if (1 == details[$input.attr("name")]) {
    ...
    for (j = 0; j < jln; j++) {
        $input = $seviceDetails.eq(j);
        data = details[$input.attr("name")];
        if (void 0 != data) {
            $input.val(data);
        }
        else {
