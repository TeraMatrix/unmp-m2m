...
function multiselect_hostgroup_func() {
    $("#multiselect_hostgroup").multiselect({
        selectedList: 1,
        multiple: false,
        noneSelectedText: 'Select Device group',
        header: "Available Device groups",
        minWidth: 290
    });
            ...
            if (device_type_user_selected_id == "None") {
                for (i = 0; i < all_array.length; i++) {
                    ...
                    if (parseInt(temp_arr[0]) == sel) {
                        //alert(String(temp_arr[2]).toLowerCase().indexOf('ccu')>=0);
...
function multiselect_device_func() {
    ...
    $("#multiselect_device").multiselect({
        ...
        header: "Available Device Type",
        minWidth: 290
    });
//	console.log(" multiselect_device_func() ");
...
function multiselect_report_func() {
    $("#multiselect_report").multiselect({
        selectedList: 1,
        multiple: false,
        noneSelectedText: 'Select Report',
    ...
    $("#multiselect_report").bind("multiselectclose", function (event, ui) {
        ...
        if (sel != null) {
            ...
            $.ajax({
                type: "get",
                url: "main_reporting_get_host_data.py" + '?hostgroup_id=' + hostgroup_id + '&device_type_id=' + device_type_id + '&report_type=' + sel,
                data: "",
                cache: false,
                success: function (result) {
                    try {
                        ...
                        if (sel == "Clients") {
                            ...
                            $().toastmessage('showNoticeToast', "Click 'View' to generate the reports of clients.");
                        } else if (sel == 'CONFIGURATION' || sel == 'Configuration') {
                            $("#multiselect_dates").multiselect("disable");
                            $("#more_options").attr('disabled', 'disabled');
...
function multiselect_host_func() {
    ...
    $("#multiselect_hosts").bind("multiselectclose", function (event, ui) {
        ...
        if (sel != null) {
            ...
            flag_more_options = true;
            html_string = "";
            if ($("#multiselect_report").val() == "CONFIGURATION" || $("#multiselect_report").val() == "Configuration") {
                $("#multiselect_dates").multiselect("disable");
                $("#more_options").attr('disabled', 'disabled');
                $("#submit").attr('disabled', 'disabled');
                $().toastmessage('showNoticeToast', "Only 'Excel file' can be generated of configuration.");
            }
            //html_string+="</ul";
...
function multiselect_dates_func() {
    ...
    $("#multiselect_dates").multiselect({
        selectedList: 1,
        multiple: false,
        noneSelectedText: 'Select Time Range',
        header: "Available Time Range",
        minWidth: 290
    });
    ...
    $("#multiselect_dates").bind("multiselectclose", function (event, ui) {
        ...
        if (sel != null) {
            ...
            var s_date = start_date.getDate() + "/" + parseInt(parseInt(start_date.getMonth(), 10) + 1, 10) + "/" + start_date.getFullYear();
            var s_time = start_date.getHours() + ":" + start_date.getMinutes();
            var e_date = end_date.getDate() + "/" + parseInt(parseInt(end_date.getMonth(), 10) + 1, 10) + "/" + end_date.getFullYear();
...
$(document).ready(function () {
    ...
    $("#get_main_reporting_data").submit(function () {
        ...
        ///////// new time modification end
        var start_date = $("#start_date").val();
        var end_date = $("#end_date").val();
        var start_time = $("#start_time").val();
        var end_time = $("#end_time").val();

        if ($("#multiselect_hosts").val() == null && $("#multiselect_report").val() != "Clients") {
            return false;
        }
        ...
        if (date2 < date1) {
            ...
            return false;
        } else if ($("#hd").val() == "" && ($("#multiselect_report").val() != "CONFIGURATION" || $("#multiselect_report").val() != "Configuration")) {
        ...
        var url = "main_reporting_get_excel.py?start_date=" + start_date + "&end_date=" + end_date + "&start_time=" + start_time + "&end_time=" + end_time + "&all_host=" + $("#multiselect_hosts").val() + "&columns=" + $("#hd").val();
        url += "&report_type=" + $("#multiselect_report").val() + "&device_type=" + $("#multiselect_device").val() + "&hostgroup=" + $("#multiselect_hostgroup").val() + "&view_type=data_table";
...
//	{
//		href: "view_page_tip_main_reporting.py",
//		title: "Page Tip",
//		opacity: 0.4,
//		maxWidth: "80%",
//		width: "600px",
...
function excel_report(report_type) {
    ...
    var url = "main_reporting_get_excel.py?start_date=" + start_date + "&end_date=" + end_date + "&start_time=" + start_time + "&end_time=" + end_time + "&all_host=" + $("#multiselect_hosts").val() + "&columns=" + $("#hd").val();
    url += "&report_type=" + $("#multiselect_report").val() + "&device_type=" + $("#multiselect_device").val() + "&hostgroup=" + $("#multiselect_hostgroup").val() + "&view_type=" + report_type;
    $.ajax({
        type: method,
        url: url,
        cache: false,
