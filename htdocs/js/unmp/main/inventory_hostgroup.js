...
var messages = {
    "add": "Device group is added successfully.",
    "edit": "Device group details Updated successfully.",
    "del": "Selected Device group(s) deleted successfully",
    "delConfirm": "Are You Sure want to delete Selected Device group(s)?",
    "duplicateNameError": "Device group Name already exists, Please Enter different Device group Name.",
    "duplicateAliasError": "Device group Alias already exists, Please Enter different Device group Alias.",
    "noneSelectedError": "Please select atleast one Device group.",
    "multiSelectedError": "Please select single Device group.",
    "defaultDelError": "Deletion of Default Device group is restricted.",
    "defaultEditError": "Updation of Default Device group is restricted.",
        ...
        success: function (result) {
            ...
            oTable = $('#grid_view').dataTable({
                ...
                "aoColumns": [
                    ...
                    { "sTitle": "Device group Name", "sClass": "center", "sWidth": "150px"},
                    { "sTitle": "Device group Alias", "sClass": "center", "sWidth": "150px"},
    ...
    $img.colorbox(
        {
            ...
            title: "Assign/Add Groups To Device group",
            opacity: 0.4,
            maxWidth: "80%",
            width: "550px",
            height: "450px"
...
function grpSearch(searchGrp) {
    ...
    if (grpData.length < 1) {
        var $grpTr = $("<tr/>");
        var $grpTd = $("<td class=\"cell-info1\" > All UserGroups Assigned successfully to Device Group </td>");
...
function boxAddGrp() {
    ...
    if (boxGrpArray.length == 0) {
        ...
        $.prompt('Are you sure, you want to Add group(s) to this Device group?', { buttons: {Yes: true, No: false}, prefix: 'jqismooth', callback: boxAddGrpCallback });
    }
...
function delGrpFrmHg() {
    ...
    if (grpArray.length == 0) {
        ...
        $.prompt('Are you sure, you want to delete Group(s) from this Device group?', { buttons: {Yes: true, No: false}, prefix: 'jqismooth', callback: delGrpCallback });
    }
}
    ...
    if (v != undefined && v == true) {
        ...
        $.ajax({
            ...
            success: function (result) {
                result = eval("(" + result + ")");
                if (result.success == 0) {
                    $().toastmessage('showSuccessToast', "Device group(s) Deleted Successfully.");
                    user_get_log($("input#hostgroup_id2").val());
                }
                else {
                    $().toastmessage('showWarningToast', String(result.result));
        ...
        var move_box = $.colorbox(
            {
                ...
                title: "Move User Group to Another Device group",
                opacity: 0.4,
                maxWidth: "80%",
                width: "400px",
                height: "150px"
...
function boxMoveGrp() {
    if (boxHGroupId == "" || boxHGroupId == null) {
        $.prompt("Select at least one Device Group", {prefix: 'jqismooth'});
    }
        ...
        $.prompt('Are you sure, you want to Move Group(s) from this Device group?', { buttons: {Yes: true, No: false}, prefix: 'jqismooth', callback: boxMoveGrpCallback });
    }
...
function valiateForm($formObj) {
    $formObj.validate({
        ...
        messages: {
            hostgroup_name: {
                required: "Device group Name is Required Field",
                alphaNumeric: "Name should be alpha numeric",
                noSpace: "Space Not Allowed"
            },
            hostgroup_alias: {
                required: "Device group Alias is Required Field",
...
function addForm() {
    $("#advanced_settings").hide();
    $formTitle.html("Add Device group");
...
function editForm(id) {
    $formTitle.html("Edit Device group");
