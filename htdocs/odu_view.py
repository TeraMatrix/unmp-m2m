...
def odu_listing(h):
    ...
    @var css_list : this is used to store the names of all the
                    css files which is used in this function
    @var javascript_list : this is used to store the names of all the javascript
                            files which is used in this function
    ...
    @var device_list_state : this is used to store the select list state
                            i.e. select list is enable or disable.
                            if it's Value is None then it is enable by default
    @var selected_device_type : this is used to store the device type
                                which selected form the device_type list
    @since : 20 August 2011
    @version :0.0
    @date : 20 Augugst 2011
    @note : This function is used for displaying the list of all host in the site
            and also search the host accrding to IpAddress,MAC Address,and Device Type
    ...
    # or not
    if html.var("device_type"):  # we get the variable of page through html.var
        device_type = html.var("device_type")
    if html.var("device_list_state"):
        device_list_state = html.var("device_list_state")
    if html.var("selected_device_type"):
        selected_device_type = html.var("selected_device_type")
    if html.var("ip_address"):
        ip_address = html.var("ip_address")
    if html.var("mac_address"):
    ...
    html.write(str(page_header_search(ip_address,
                                      mac_address,
                                      "RM18,RM,IDU,Access Point,CCU",
                                      selected_device_type,
                                      "enabled",
                                      "device_type")
    ))
...
def odu_profiling(h):
    ...
    @var css_list : this is used to store the names of all the
                    css files which is used in this function
    @var javascript_list : this is used to store the names of all the
                            javascript files which is used in this function
    @var host_id : this is used to store the Host Id which is come from the page
    @var device_type : this is used to store the device types i.e UBR,Switch,AP25 etc
    @var device_list_state : this is used to store the select list state
                            i.e. select list is enable or disable.
                            if it's Value is None then it is enable by default
    @var device_list_param : this is used to store all the host details
                    accoding to host-id like host ip address,
                    host mac address,host device type,host configuration id
    @since : 20 August 2011
    @version :0.0
    @date : 20 Augugst 2011
    @note : this function is used to write the search bar in which user search
            the device according to ipaddress, macaddress, device type and
            show the configuration forms of device according to search
    @organisation : Codescape Consultants Pvt. Ltd.
    @copyright : 2011 Anuj Samariya from Codescape Consultants Pvt. Ltd.
    """
    ...
    # call the odu_controller function get_device_param returns the tuple.
    #   tuple consist ipaddress, macaddress, devicetypeid, config_profile_id
    ...
    if device_list_param == [] or device_list_param is None:
        ...
        # used for listing the Devices based on IPaddress, Macaddress,
        # DeviceType
    ...
    if host_id == "" or host_id == "None":
        html.write("<div id=\"odu16_form_div\" class=\"form-div\">\
                        There is no profile selected\
                    </div>")
    else:
        obj_get_data = IduGetData()
        if device_type == "odu16":
            ru_data = obj_get_data.common_get_data(
                'SetOdu16RUConfTable', host_id)
            ra_data = obj_get_data.common_get_data(
                'SetOdu16RAConfTable', host_id)
            sync_data = obj_get_data.common_get_data(
                'SetOdu16SyncConfigTable', host_id)

        else:
            ru_data = obj_get_data.common_get_data(
                'Odu100RuConfTable', host_id)
            ra_data = obj_get_data.common_get_data(
                'Odu100RaConfTable', host_id)
            sync_data = obj_get_data.common_get_data(
                'Odu100SyncConfigTable', host_id)
            ru_status = odu100_get_status(host_id, "Odu100RuStatusTable")
            ra_status = odu100_get_status(host_id, "Odu100RaStatusTable", 1)
            sync_status = odu100_get_status(
                host_id, "Odu100SynchStatusTable", 1)
            if len(ru_status) > 0:
                if ru_status[0].ruoperationalState is None:
                    ru_op_state = 1
                else:
            ...
            if len(ra_status) > 0:
                if ra_status[0].raoperationalState is None:
                    ra_op_state = 1
                else:
            ...
            if len(sync_status) > 0:
                if sync_status[0].syncoperationalState is None:
                    sync_op_state = 1
                else:
        ...
        if device_type == "odu16":
            ...
            html.write("<div id=\"odu16_form_div\" \
                             class=\"form-div\" \
                             style=\"margin-top: 56px;\">")

            # function call , it is used to make a form of selected profiling
            odu_profiling_form(h, host_id)

            html.write("</div>")

            # i know its still hard to understand %s
            ...
            <div class=\"form-div-footer\">\
                <div id=\"adminDiv\" style=\"float: left;margin-left:15px\">\
                    <ul class=\"button_group\" style=\" margin:10px 0 0 10px !important;\">\
                        <li>\
                            <a class=\"%s n-reconcile\" \
                                id=\"ru.ruConfTable.adminstate\" \
                                name=\"ru.ruConfTable.adminstate\" \
                                state=\"%s\" \
                                onclick=\"adminStateCheck(event, this, '%s', 'ru.ruConfTable.adminstate')\" \
                                >\
                                Radio Unit %s\
                            </a>\
                        </li>\
                        <li>\
                            <a class=\"%s n-reconcile\" \
                                id=\"ru.syncClock.syncConfigTable.adminStatus\" \
                                name=\"ru.syncClock.syncConfigTable.adminStatus\" \
                                state=\"%s\" \
                                onclick=\"adminStateCheck(event, this, '%s', 'ru.syncClock.syncConfigTable.adminStatus')\"\
                                >\
                                SYN %s\
                            </a>\
                        </li>\
                        <li>\
                            <a class=\"%s n-reconcile\" \
                                id=\"ru.ra.raConfTable.raAdminState\" \
                                name=\"ru.ra.raConfTable.raAdminState\" \
                                state=\"%s\" \
                                onclick=\"adminStateCheck(event, this, '%s', 'ru.ra.raConfTable.raAdminState')\"\
                                >\
                                Radio Access %s\
                            </a>\
                        </li>\
                ...
                <div id=\"operationDiv\" style=\"float:right;margin-right:100px;\">\
                    <input type=\"button\" \
                            id=\"RU.RUOMOperationsTable.omOperationsReq\" \
                            name=\"odu16_commit_to_flash\" \
                            value=\"Commit To Flash\" \
                            class=\"yo-small yo-button\"\
                    />\
                    <input type=\"button\" \
                            id=\"odu16_reconcile\" \
                            name=\"odu16_reconcile\" \
                            value=\"Reconciliation\" \
                            class=\"yo-small yo-button\"\
                    />\
                    <!-- <input type=\"button\" \
                                id=\"site_survey_btn\" \
                                name=\"site_survey_btn\" \
                                value=\"Site Survey Results\" \
                                class=\"yo-small yo-button\" \
                                onClick=\"siteSurvey();\"/>\
                    -->\
                </div>\
            </div>" % ("red" if ru_state == 0 else "green",
                       ru_state,
                       device_list_param[0][2],
                       "Locked" if ru_state == 0 else "Unlocked",
                       "red" if sync_state == 0 else "green",
                       sync_state,
                       device_list_param[0][2],
                       "Locked" if sync_state == 0 else "Unlocked",
                       "red" if ra_state == 0 else "green",
                       ra_state,
                       device_list_param[0][2],
                       "Locked" if ra_state == 0 else "Unlocked"))

        else:
            if len(ru_data) > 0:
                if ru_data[0].adminstate is None:
                    ru_state = 1
                    image_ru_title = "RU State UnLocked"
                else:
                    if int(ru_data[0].adminstate) == 0:
            ...
            if len(ra_data) > 0:
                if ra_data[0].raAdminState is None:
                    ra_state = 1
                    image_ra_title = "RA State Unlocked"
            ...
            if len(sync_data) > 0:
                if sync_data[0].adminStatus is None:
                    sync_state = 1
                    image_sync_title = " SYNC State Unlocked"
            ...
            html.write("\
            <div id=\"odu16_form_div\" class=\"form-div\" style=\"margin-top: 56px;\"></div>\
            <div class=\"form-div-footer\">\
                <div id=\"adminDiv\" style=\"float: left;margin-left:15px\">\
                    <ul class=\"button_group\" style=\"margin:10px 0 0 10px !important;\">\
                        <li>\
                ...
                <div id=\"operation_status\" style=\"float: left; margin-top: 12px; margin-left: 15px;vertical-align: middle;\">\
                    ...
                    &nbsp;&nbsp;&nbsp;\
                    <img class=\"imgbutton n-reconcile\" \
                            id=\"operation_status\" \
                ...
                <div id=\"operationDiv\" style=\"float:right;margin-right:10px;\">\
                    ...
                    <input type=\"button\" \
                        ...
                        />\
                    <input type=\"button\" \
                            id=\"bw_calculator_form\" \
                            name=\"bw_calculator_form\" \
                            value=\"BW Calculator\" \
                            class=\"yo-small yo-button\" \
                        />\
                </div>\
        ...
        # different indentation its outside else
        html.write("<input type=\"hidden\" \
                    name=\"device_type\" \
                    id=\"device_type\" \
                    value=%s>" % (device_type)
        )
...
def odu_profiling_form(h, host_id):
    ...
    if host_id is None:
        host_id = html.var("host_id")
    device_list_param = get_device_param(host_id)
    if device_list_param[0][3] is None or device_list_param[0][3] == "":
        html.write("No Configuration Profile Exist")

    else:
        # This makes the filter buttons and a commit to flash button and writes
        # on page
        tab_str = "\
        ...
        </div>" % (RU_Configuration(h, host_id),
                   Llc_Configuration(h, host_id),
                   Omc_Config(h, host_id),
                   SysOmc_Registration_Configuration(h, host_id),
                   Syn_Omc_Registration_Configuration(h, host_id),
                   Acl_Configuration(h, host_id),
                   Tdd_Mac_Configuration(h, host_id),
                   Peer_mac(h, host_id),
                   device_list_param[0][0],
                   device_list_param[0][1])
...
def RU_Configuration(h, host_id):
    ...
    <form action=\"ru_configuration.py\" method=\"get\" id=\"ru_configuration_form\">\
        <div class=\"row-elem\">\
            ...
            <Select id=\"RU.RUConfTable.channelBandwidth\" \
                    name=\"RU.RUConfTable.channelBandwidth\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.1.1.7.1\" \
                    val=\"Integer\" \
                    value=\"%s\" \
                    tablename=\"SetOdu16RUConfTable\" \
                    field=\"channel_bandwidth\">Mhz\
        ...
        <div class=\"row-elem\">\
            ...
            <Select id=\"RU.RUConfTable.synchSource\" \
                    disabled=\"disabled\" \
                    name=\"RU.RUConfTable.synchSource\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.1.1.8.1\" \
                    val=\"Integer\" \
                    tablename=\"SetOdu16RUConfTable\" \
                    field=\"sysnch_source\">\
        ...
        <div class=\"row-elem\">\
            ...
            <Select id=\"RU.RUConfTable.countryCode\" \
                    name=\"RU.RUConfTable.countryCode\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.1.1.9.1\" \
                    val=\"Unsigned\" \
                    value=\"%s\" \
                    tablename=\"SetOdu16RUConfTable\" \
                    field=\"country_code\">\
        ...
        <div class=\"row-elem\">\
                ...
                <br/><br/>\
                <span  style=\"font-size: 11px;\" class=\"note\">\
                    ** Change for all these values will cause reboot\
                </span>\
        </div>\
    </form>\
    " % ("" if ru_config_get[0][0] is None else ru_config_get[0][0],
         "" if ru_config_get[0][0] is None else ru_config_get[0][0],
         "" if host_id is None else host_id,
         "" if ru_config_get[0][2] is None else ru_config_get[0][2],
         "" if ru_config_get[0][2] is None else ru_config_get[0][2])
    return str_form
...
def RU_Cancel_Configuration(h, host_id):
    ...
    </form>" % ("" if ru_config_get[0][0] is None else ru_config_get[0][0],
                "" if ru_config_get[0][0] is None else ru_config_get[0][0],
                "" if host_id is None else host_id,
                "" if ru_config_get[0][2] is None else ru_config_get[0][2],
                "" if ru_config_get[0][2] is None else ru_config_get[0][2])
...
def Omc_Config(h, host_id):
    ...
    <form action=\"omc_config_detail.py\" method=\"get\" id=\"omc_configuration_form\">\
        <div class=\"row-elem\">\
            ...
            <input type=\"text\" \
                    name=\"RU.OMCConfTable.omcIPAddress\" \
                    id=\"RU.OMCConfTable.omcIPAddress\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.7.1.2.1\" \
                    val=\"ip\" \
                    field=\"omc_ip_address\" \
                    value=\"%s\" \
                    tablename=\"SetOdu16OmcConfTable\"/> \
            <input type=\"hidden\" id=\"host_id\" name=\"host_id\" value=\"%s\"/>\
        </div>\
        <div class=\"row-elem\" style=\"display:none\">\
            <label class=\"lbl lbl-big\">Periodic Statistics Timer</label>\
            <input type=\"text\" \
                    name=\"RU.OMCConfTable.periodicStatisticsTimer\" \
                    id=\"RU.OMCConfTable.periodicStatisticsTimer\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.7.1.3.1\" \
                    val=\"Integer\" \
                    field=\"periodic_stats_timer\" \
                    value=\"%s\" \
                    tablename=\"SetOdu16OmcConfTable\"/>\
        </div>\
        <div class=\"row-elem\">\
            ...
            <br/><br/>\
            <span  style=\"font-size: 11px;\" class=\"note\">\
                **UNMP IP - Configuring UNMP IP is important for capturing and monitoring the device alarms\
            </span>\
        </div>\
    </form>\
    " % ("" if omc_config[0].omc_ip_address is None else omc_config[0].omc_ip_address,
         "" if host_id is None else host_id,
...
## ---------------- configuration report download ----------------------
# import the config report download function
...
def configuration_report_download(h):
    ...
    host_id = html.var("host_id")
    if host_id:
        html.write(str(get_configuration_details((host_id,))))
    else:
        html.write("Host information not found, please refresh your page")
...
def SysOmc_Registration_Configuration(h, host_id):
    ...
    # This is the odu_controller function.
    # It returns the sys registration form parameters and config profile id
    ...
    <form action=\"omc_registration_configuration.py\" method=\"get\" id=\"omc_registration_form\">\
        <div class=\"row-elem\">\
            ...
            <input type=\"text\" \
                    name=\"RU.SysOmcRegistrationTable.sysOmcRegistercontactAddr\" \
                    id=\"RU.SysOmcRegistrationTable.sysOmcRegistercontactAddr\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.8.1.2.1\" \
                    maxlength=\"20\" \
                    val=\"string\" \
                    field=\"sys_omc_register_contact_addr\" \
                    value=\"%s\" \
                    tablename=\"SetOdu16SysOmcRegistrationTable\"/>\
            <input type=\"hidden\" id=\"host_id\" name=\"host_id\" value=\"%s\"/>\
        </div>\
        <div class=\"row-elem\">\
            <label class=\"lbl lbl-big\">Contact Person</label>\
            <input type=\"text\" \
                    name=\"RU.SysOmcRegistrationTable.sysOmcRegistercontactPerson\" \
                    id=\"RU.SysOmcRegistrationTable.sysOmcRegistercontactPerson\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.8.1.3.1\" \
                    val=\"string\" \
                    value=\"%s\" \
                    field=\"sys_omc_register_contact_person\" \
                    maxlength=\"20\" \
                    tablename=\"SetOdu16SysOmcRegistrationTable\" />\
        </div>\
        <div class=\"row-elem\">\
            ...
            <input type=\"text\" \
                    name=\"RU.SysOmcRegistrationTable.sysOmcRegistercontactMobile\" \
                    id=\"RU.SysOmcRegistrationTable.sysOmcRegistercontactMobile\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.8.1.4.1\" \
                    field=\"sys_omc_register_contact_mobile\" \
                     val=\"string\" \
                    value=\"%s\" \
                    maxlength=\"10\" \
                    tablename=\"SetOdu16SysOmcRegistrationTable\" />\
        ...
        <div class=\"row-elem\">\
            ...
            <input type=\"text\" \
                    name=\"RU.SysOmcRegistrationTable.sysOmcRegisteralternateCont\" \
                    id=\"RU.SysOmcRegistrationTable.sysOmcRegisteralternateCont\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.8.1.5.1\" \
                    val=\"string\" \
                    value=\"%s\" \
                    maxlength=\"20\" \
                    field=\"sys_omc_register_alternate_contact\" \
                    tablename=\"SetOdu16SysOmcRegistrationTable\" />\
        </div>\
        <div class=\"row-elem\">\
            <label class=\"lbl lbl-big\">Email</label>\
            <input type=\"text\" \
                    name=\"RU.SysOmcRegistrationTable.sysOmcRegistercontactEmail\" \
                    id=\"RU.SysOmcRegistrationTable.sysOmcRegistercontactEmail\" \
                    field=\"sys_omc_register_contact_email\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.8.1.6.1\" \
                    val=\"string\" \
                    value=\"%s\" \
                    maxlength=\"20\" \
                    tablename=\"SetOdu16SysOmcRegistrationTable\" />\
...
def Syn_Omc_Registration_Configuration(h, host_id):
    ...
    #--------------------------------------------------------------------------
    str_form = "\
    <form action=\"syn_configuration.py\" method=\"get\" id=\"syn_configuration_form\">\
        <div class=\"row-elem\">\
    ...
    " % ("" if sync_config_table[0][0] is None else sync_config_table[0][0],
         "" if host_id is None else host_id,
         "" if sync_config_table[0][1] is None else sync_config_table[0][1],
         "" if sync_config_table[0][1] is None else sync_config_table[0][1],
         "" if sync_config_table[0][2] is None else sync_config_table[0][2],
         "" if sync_config_table[0][3] is None else sync_config_table[0][3],
         "" if sync_config_table[0][4] is None else sync_config_table[0][4],
         "" if sync_config_table[0][5] is None else sync_config_table[0][5])
...
def Syn_Cancel_Omc_Registration_Configuration(h, host_id):
    ...
    # syn_configuration get Method-------------------------------------------------
    # This is the odu_controller function.
    # It returns the synomcregistration form parameters and  onfig profile id
    sync_config_table, sync_config_profile_id = sync_config_table_get(host_id)
    #--------------------------------------------------------------------------
    str_form = "\
    <form action=\"syn_configuration.py\" method=\"get\" id=\"syn_configuration_form\">\
        <div class=\"row-elem\">\
            <label class=\"lbl lbl-big\">Raster Time</label>\
            <Select id=\"RU.SyncClock.SyncConfigTable.rasterTime\" \
                    name=\"RU.SyncClock.SyncConfigTable.rasterTime\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.13.4.1.5.1\" \
                    val=\"Integer\" \
                    tablename=\"SetOdu16SyncConfigTable\" \
                    field=\"raster_time\">\
                <option value=\"0\">0</option>\
                <option value=\"2\">2</option>\
                <option value=\"4\">4</option>\
        ...
        <div class=\"row-elem\">\
            <label class=\"lbl lbl-big\">Number of Slaves</label>\
            <Select id=\"RU.SyncClock.SyncConfigTable.numSlaves\" \
                    name=\"RU.SyncClock.SyncConfigTable.numSlaves\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.11.1.1.5.1\" \
                    val=\"Integer\" \
                    tablename=\"SetOdu16SyncConfigTable\" \
                    field=\"num_slaves\">\
                <option value=\"1\">1 Timeslot(T1/T2)</option>\
        ...
        <div class=\"row-elem\">\
            <label class=\"lbl lbl-big\">SyncLossThreshold</label>\
            <input type=\"text\" \
                    name=\"RU.SyncClock.SyncConfigTable.syncLossThreshold\" \
                    id=\"RU.SyncClock.SyncConfigTable.syncLossThreshold\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.11.1.1.6.1\" \
                    val=\"Integer\" \
                    value=\"%s\" \
                    field=\"sync_loss_threshold\" \
                    tablename=\"SetOdu16SyncConfigTable\"/>\
        </div>\
        <div class=\"row-elem\">\
            <label class=\"lbl lbl-big\">Leaky Bucket Timer</label>\
            <input type=\"text\" \
                    name=\"RU.SyncClock.SyncConfigTable.leakyBucketTimer\" \
                    id=\"RU.SyncClock.SyncConfigTable.leakyBucketTimer\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.11.1.1.7.1\" \
                    val=\"Integer\" \
                    value=\"%s\" \
                    field=\"leaky_bucket_timer\" \
                    tablename=\"SetOdu16SyncConfigTable\"/>\
        </div>\
        <div class=\"row-elem\">\
            <label class=\"lbl lbl-big\">Sync Loss Time Out</label>\
            <input type=\"text\" \
                    name=\"RU.SyncClock.SyncConfigTable.syncLostTimeout\" \
                    id=\"RU.SyncClock.SyncConfigTable.syncLostTimeout\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.11.1.1.8.1\" \
                    val=\"Integer\" \
                    value=\"%s\" \
                    field=\"sync_lost_timeout\" \
                    tablename=\"SetOdu16SyncConfigTable\"/>\
        </div>\
        <div class=\"row-elem\">\
            <label class=\"lbl lbl-big\">Sync Timer Adjust</label>\
            <input type=\"text\" \
                    name=\"RU.SyncClock.SyncConfigTable.timerAdjust\" \
                    id=\"RU.SyncClock.SyncConfigTable.timerAdjust\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.11.1.1.9.1\" \
                    val=\"Integer\" \
                    value=\"%s\" \
                    field=\"sync_config_time_adjust\" \
                    tablename=\"SetOdu16SyncConfigTable\"/>\
        </div>\
        <div class=\"row-elem\">\
            <input type=\"submit\" value=\"Save\" class=\"yo-small yo-button\"/>\
...
def Llc_Configuration(h, host_id):
    ...
    <form action=\"ra_llc_configuration.py\" method=\"get\" id=\"ra_llc_config_form\">\
        <div class=\"row-elem\">\
            ...
            <Select id=\"RU.RA.1.LLC.RALLCConfTable.llcArqEnable\" \
                    name=\"RU.RA.1.LLC.RALLCConfTable.llcArqEnable\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.13.6.1.1.1.1\" \
                    val=\"Integer\" \
                    value=\"%s\" \
                    field=\"llc_arq_enable\" \
                    tablename=\"SetOdu16RALlcConfTable\">\
        ...
        <div class=\"row-elem\">\
            ...
            <input type=\"text\" \
                    name=\"RU.RA.1.LLC.RALLCConfTable.arqWin\" \
                    id=\"RU.RA.1.LLC.RALLCConfTable.arqWin\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.13.6.1.1.2.1\" \
                    val=\"Integer\" \
                    value=\"%s\" \
                    field=\"arq_win\" \
                    tablename=\"SetOdu16RALlcConfTable\"/>\
        ...
        <div class=\"row-elem\">\
            ...
            <input type=\"text\" \
                    name=\"RU.RA.1.LLC.RALLCConfTable.frameLossThreshold\" \
                    id=\"RU.RA.1.LLC.RALLCConfTable.frameLossThreshold\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.13.6.1.1.3.1\" \
                    val=\"Integer\" \
                    value=\"%s\" \
                    field=\"frame_loss_threshold \" \
                    tablename=\"SetOdu16RALlcConfTable\"/>\
        </div>\
        <div class=\"row-elem\">\
            <label class=\"lbl\">Leaky Bucket Timer</label>\
            <input type=\"text\" \
                    name=\"RU.RA.1.LLC.RALLCConfTable.leakyBucketTimer\" \
                    id=\"RU.RA.1.LLC.RALLCConfTable.leakyBucketTimer\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.13.6.1.1.4.1\" \
                    val=\"Integer\" \
                    value=\"%s\" \
                    field=\"leaky_bucket_timer_val\" \
                    tablename=\"SetOdu16RALlcConfTable\"/>\
        </div>\
        <div class=\"row-elem\">\
            <label class=\"lbl\">Frame Loss Time Out</label>\
            <input type=\"text\" \
                    name=\"RU.RA.1.LLC.RALLCConfTable.frameLossTimeout\" \
                    id=\"RU.RA.1.LLC.RALLCConfTable.frameLossTimeout\" \
                    fact=\".1.3.6.1.4.1.26149.2.2.13.6.1.1.5.1\" \
                    val=\"Integer\" \
                    value=\"%s\" \
                    field=\"frame_loss_timeout\" \
                    tablename=\"SetOdu16RALlcConfTable\"/>\
            <input type=\"hidden\" id=\"host_id\" name=\"host_id\" value=\"%s\"/>\
        </div>\
        <div class=\"row-elem\">\
    ...
    " % ("" if ra_llc_config[0][0] is None else ra_llc_config[0][0],
         "" if ra_llc_config[0][0] is None else ra_llc_config[0][0],
         "" if ra_llc_config[0][1] is None else ra_llc_config[0][1],
         "" if ra_llc_config[0][2] is None else ra_llc_config[0][2],
         "" if ra_llc_config[0][3] is None else ra_llc_config[0][3],
         "" if ra_llc_config[0][4] is None else ra_llc_config[0][4],
         "" if host_id is None else host_id)
...
def Llc_Cancel_Configuration(h, host_id):
    ...
    ra_llc_config = []
    # This is the odu_controller function.
    # It returns the llcconfiguration form parameters and  onfig profile id
    ra_llc_config, ra_llc_config_id = ra_llc_conf_table_get(host_id)
    #--------------------------------------------------------------------------
    str_form = "\
    <form action=\"ra_llc_configuration.py\" method=\"get\" id=\"ra_llc_config_form\">\
        <div class=\"row-elem\">\
    ...
    " % ("" if ra_llc_config[0][0] is None else ra_llc_config[0][0],
         ...
         "" if ra_llc_config[0][2] is None else ra_llc_config[0][2],
         "" if ra_llc_config[0][3] is None else ra_llc_config[0][3],
         "" if ra_llc_config[0][4] is None else ra_llc_config[0][3],
         "" if host_id is None else host_id)
    html.write(str(str_form))
...
def Acl_Configuration(h, host_id):
        ...
        if i < 10:
            ...
            strmac_li.append("\
                <div class=\"row-elem\" id=\"acl_row_element_%s\">\
                    ...
                    <input type=\"text\" name=\"RU.RA.1.RAACLConfig.%s.macAddress\" \
                        id=\"RU.RA.1.RAACLConfig.%s.macAddress\" \
                        fact=\".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s\" \
                        val=\"string\" \
                        value=\'%s\' field=\"mac_address\" \
                        tablename=\"SetOdu16RAAclConfigTable\" \
                        index=\"%s\"/>\
                </div>\
                " % (temp_var,
                     temp_var,
            ...
            strmac_li.append("\
                <div class=\"row-elem\" id=\"acl_row_element_%s\">\
                    <label class=\"lbl lbl-big\">MAC Address %s</label>\
                    <input type=\"text\" \
                        name=\"RU.RA.1.RAACLConfig.%s.macAddress\" \
                        id=\"RU.RA.1.RAACLConfig.%s.macAddress\" \
                        fact=\".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s\" \
                        val=\"string\" \
                        value=\"%s\" \
    ...
    </form>" % ("" if ra_conf[0][0] is None else ra_conf[0][0],
                "" if ra_conf[0][0] is None else ra_conf[0][0],
...
def tdd_mac_config(h):
    ...
    tdd_mac_config_fields.append(
        'RU.RA.1.TddMac.RATDDMACConfigTable.passPhrase')
    ...
    tdd_mac_config_param.append(
        html.var('RU.RA.1.TddMac.RATDDMACConfigTable.passPhrase'))
    ...
    tdd_mac_config_param.append(
        html.var('RU.RA.1.TddMac.RATDDMACConfigTable.maxCrcErrors'))
    tdd_mac_config_param.append(
...
def ra_llc_configuration(h):
    ...
    llc_configuration_fields.append(
    ...
    llc_configuration_param.append(
        html.var('RU.RA.1.LLC.RALLCConfTable.llcArqEnable'))
    llc_configuration_param.append(
        html.var('RU.RA.1.LLC.RALLCConfTable.arqWin'))
    llc_configuration_param.append(
        html.var('RU.RA.1.LLC.RALLCConfTable.frameLossThreshold'))
    llc_configuration_param.append(
        html.var('RU.RA.1.LLC.RALLCConfTable.leakyBucketTimer'))
    llc_configuration_param.append(
...
def omc_registration_configuration(h):
    ...
    omc_registration_fields.append(
    ...
    omc_registration_param.append(
        html.var('RU.SysOmcRegistrationTable.sysOmcRegistercontactAddr'))
    omc_registration_param.append(
        html.var('RU.SysOmcRegistrationTable.sysOmcRegistercontactPerson'))
    omc_registration_param.append(
        html.var('RU.SysOmcRegistrationTable.sysOmcRegistercontactMobile'))
    omc_registration_param.append(
        html.var('RU.SysOmcRegistrationTable.sysOmcRegisteralternateCont'))
    omc_registration_param.append(
...
def syn_configuration(h):
    ...
    syn_configuration_fields.append(
    ...
    syn_configuration_param.append(
        html.var('RU.SyncClock.SyncConfigTable.rasterTime'))
    syn_configuration_param.append(
        html.var('RU.SyncClock.SyncConfigTable.numSlaves'))
    syn_configuration_param.append(
        html.var('RU.SyncClock.SyncConfigTable.syncLossThreshold'))
    syn_configuration_param.append(
        html.var('RU.SyncClock.SyncConfigTable.leakyBucketTimer'))
    syn_configuration_param.append(
        html.var('RU.SyncClock.SyncConfigTable.syncLostTimeout'))
    syn_configuration_param.append(
...
def peer_config(h):
    ...
    for i in range(0, int(time_slot_param)):
        peer_config_fields.append(
            'ru.np.ra.1.peer.%s.config.macAddress' % (i + 1))
        peer_config_param.append(
            html.var('ru.np.ra.1.peer.%s.config.macAddress' % (i + 1)))
...
def acl_reconciliation(h):
    ...
    table_prefix = "odu16_"
    insert_update = True
    result = acl_reconcile(
        host_id, device_type_id, table_prefix, insert_update)
    html.write(str(result))
...
def retry_set_all_for_odu16(h):
    ...
    textbox_field_list = []
    index_value_list = []
    if table_name and textbox and textbox_value and textbox_field:
        ...
        if device_type_id == "odu100":
            ...
            if host_op_status is None or host_op_status == 0:
                ...
                result = "Device is busy, Device " + str(
                    host_status_dic[host_op_status]) + " is in progress. please wait ..."
                html.write(str(result))
...
def get_device_data_table(h):
    ...
    # check that value is None Then It takes the empty string
    if html.var("device_type") is None:
        selected_device = "odu"
    else:
...
class OduConfiguration():
    ...
    def odu100_profiling(self, host_id, selected_device):
        ...
        html.write(
            "<div id = \"odu100_main_form_container\" name=\"odu100_main_form_container\">")
    ...
    def odu100_ru_configuration(self, host_id, selected_device):
        ...
        ru_status_data = ru_status_obj.common_get_data_by_host(
            'Odu100RuStatusTable', host_id)
        firmware_result = get_firmware_version(host_id)
        if firmware_result['success'] == 0:
        ...
        if len(ru_status_data) > 0:
            if ru_status_data[0].cpuId is None:
                cpuid = 6
            else:
    ...
    def odu100_ra_configuration(self, host_id, selected_device):
        ...
        if firmware_version == '7.2.29':
            forcemimo_div = "\
            <div class=\"row-elem\" style=\"\">\
                <label class=\"lbl\">forceMimo</label>\
                %s\
            </div>\
            ...
            <form id=\"odu100_ra_configuration_form\" name=\"odu100_ra_configuration_form\" action=\"odu100_ra_configuration.py\" method=\"get\">\
                <div id=\"odu100_ra_configuration_form_parmeters_container\" name=\"odu100_ra_configuration_form_parmeters_container\">\
                    ...
                    <div class=\"row-elem\" style=\"%s\">\
                        ...
                        <input type = \"text\" id =\"ru.ra.raConfTable.ssID\" name = \"ru.ra.raConfTable.ssID\" value = \"%s\" maxlength = \"32\" %s/>\
                    ...
                    <div class=\"row-elem\" style=\"%s\">\
                        ...
                        <input type = \"text\" id=\"ru.ra.raConfTable.guaranteedBroadcastBW\" name=\"ru.ra.raConfTable.guaranteedBroadcastBW\" value=\"%s\" maxlength=\"15\" %s \>\
        ...
        if len(peer_configuration) > 0:
            ...
            for i in range(0, 16):
                if len(peer_configuration) > i:
                    ...
                    odu100_peer_config_form += "\
                    <div class=\"row-elem\" style=\"width:1230px;display:none\">\
    ...
    def odu100_omc_configuration(self, host_id, selected_device):
        ...
        @copyright : 2011 Anuj Samariya from Codescape Consultants Pvt. Ltd.
        """
        # This variable is used for storing html form which is in string
        # form[type - string]
        try:
            global html
    ...
    def odu100_acl_configuration(self, host_id, selected_device):
        ...
        if len(acl_configuration) > 0:
            ...
            # This variable is used for storing total number of records of
            # database table [type - int]
    ...
    def odu100_channel_configuration(self, host_id, selected_device, i=0):
        ...
        ra_channel_list = []
        ra_channel_list = obj_get_data.common_get_data_by_host(
            'Odu100RaChannelListTable', host_id)
        str_form = ""
        if len(ra_channel_list) == 0 and i == 0 or i == 2:
            ...
            str_form += "\
                        <form action=\"channel_configuration.py\" method=\"get\" id=\"channel_configuration_form\">"
            if int(i) == 1:
                result = refresh_channel_freq_list_table(
                    host_id, selected_device)
                # html.write(str(result))
                # return str(result)
                if result['success'] == 1:
...
def bw_action(h):
    ...
    try:
        ...
        #
        if host_op_status is None or host_op_status == 0:
            if host_id is None or device_type is None:
                dic_result['success'] = 1
                    ...
                    if Validation.is_required(html.var("tx_rate")):
                        if Validation.is_number(html.var("tx_rate")):
                            if int(html.var("tx_rate")) >= 0 and int(html.var("tx_rate")) <= 300000:
                                ...
                                flag = 1
                                dic_result[
                                    "result"] = "TX Rate must be in between 0 and 300000"
                        else:
                            flag = 1
                    ...
                    if Validation.is_required(html.var("tx_time")):
                        if Validation.is_number(html.var("tx_time")):
                            if int(html.var("tx_time")) >= 0 and int(html.var("tx_time")) <= 4000:
                                ...
                                flag = 1
                                dic_result[
                                    "result"] = "TX Time must be in between 0 and 4000"
                        else:
                            flag = 1
            ...
            dic_result['result'] = "Device is busy, Device " + str(
                host_status_dic[host_op_status]) + " is in progress. please wait ..."
            dic_result['success'] = 1
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(dic_result)))
        ...
        if host_op_status is None or host_op_status == 0:
            ...
            if html.var("odu100_submit") == "Save" or html.var("odu100_submit") == "Retry" or html.var(
                ...
                if html.var("host_id") == "" or html.var("host_id") is None:
                    ...
                    if html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactAddr"):
                        if Validation.is_required(html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactAddr")):
                            dic_result["ru.sysOmcRegistrationTable.sysOmcRegisterContactAddr"] = html.var(
                                "ru.sysOmcRegistrationTable.sysOmcRegisterContactAddr")
                        else:
                            flag = 1
                            dic_result["result"] = "Address is required field"
                    if html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactPerson"):
                        if Validation.is_required(html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactPerson")):
                            dic_result["ru.sysOmcRegistrationTable.sysOmcRegisterContactPerson"] = html.var(
                                "ru.sysOmcRegistrationTable.sysOmcRegisterContactPerson")
                        else:
                            flag = 1
                            dic_result[
                                "result"] = "Contact Person is required field"
                    if html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactMobile"):
                        if Validation.is_required(html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactMobile")):
                            dic_result["ru.sysOmcRegistrationTable.sysOmcRegisterContactMobile"] = html.var(
                                "ru.sysOmcRegistrationTable.sysOmcRegisterContactMobile")
                        else:
                            flag = 1
                    ...
                    if html.var("ru.sysOmcRegistrationTable.sysOmcRegisterAlternateContact"):
                        if Validation.is_required(
                            ...
                            flag = 1
                            dic_result[
                                "result"] = "Alternate Contact is required field"
                    if html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactEmail"):
                        if Validation.is_required(html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactEmail")):
                            dic_result["ru.sysOmcRegistrationTable.sysOmcRegisterContactEmail"] = html.var(
                                "ru.sysOmcRegistrationTable.sysOmcRegisterContactEmail")
                        else:
                            flag = 1
                ...
                dic_result["result"] = {}
                if html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactAddr"):
                    dic_result["result"]["ru.sysOmcRegistrationTable.sysOmcRegisterContactAddr"] = html.var(
                        "ru.sysOmcRegistrationTable.sysOmcRegisterContactAddr")
                if html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactPerson"):
                    dic_result["result"]["ru.sysOmcRegistrationTable.sysOmcRegisterContactPerson"] = html.var(
                        "ru.sysOmcRegistrationTable.sysOmcRegisterContactPerson")
                if html.var("RU.SysOmcRegistrationTable.sysOmcRegistercontactMobile"):
                    dic_result["result"]["ru.sysOmcRegistrationTable.sysOmcRegisterContactMobile"] = html.var(
                        "ru.sysOmcRegistrationTable.sysOmcRegisterContactMobile")
                if html.var("ru.sysOmcRegistrationTable.sysOmcRegisterContactMobile"):
                    dic_result["result"]["ru.sysOmcRegistrationTable.sysOmcRegisterAlternateContact"] = html.var(
                        "ru.sysOmcRegistrationTable.sysOmcRegisterAlternateContact")
                if html.var("ru.sysOmcRegistrationTable.sysOmcRegisterAlternateContact"):
                    dic_result["result"]["ru.sysOmcRegistrationTable.sysOmcRegisterContactEmail"] = html.var(
                        "ru.sysOmcRegistrationTable.sysOmcRegisterContactEmail")
    ...
    try:
        ...
        if host_op_status is None or host_op_status == 0:
            if html.var("odu100_submit") == "Save":
                ...
                if html.var("host_id") == "" or html.var("host_id") is None:
                    ...
                    if Validation.is_required(html.var("ru.omcConfTable.periodicStatsTimer")):
                        if Validation.is_number(html.var("ru.omcConfTable.periodicStatsTimer")):
                            dic_result["ru.omcConfTable.periodicStatsTimer"] = html.var(
                                "ru.omcConfTable.periodicStatsTimer")
                        else:
                            flag = 1
                            dic_result[
                                "Periodic Stats Timer"] = "It Must be Integer"
                    else:
                        dic_result[
                            "Periodic Stats Timer"] = "Periodic stats timer is required"
                        flag = 1
                    if Validation.is_required(html.var("ru.omcConfTable.omcIpAddress")):
                        if Validation.is_valid_ip(html.var("ru.omcConfTable.omcIpAddress")):
                            dic_result["ru.omcConfTable.omcIpAddress"] = html.var(
                                "ru.omcConfTable.omcIpAddress")
                        else:
                            flag = 1
                            dic_result[
                                "Unmp Ip Address"] = "Ip is not valid IP"
                    else:
                        flag = 1
                ...
                obj_essential.host_status(host_id, 12)
                if html.var("ru.omcConfTable.periodicStatsTimer"):
                    dic_result["ru.omcConfTable.periodicStatsTimer"] = html.var(
                        "ru.omcConfTable.periodicStatsTimer")
                if html.var("ru.omcConfTable.omcIpAddress"):
                    dic_result["ru.omcConfTable.omcIpAddress"] = html.var(
                        "ru.omcConfTable.omcIpAddress")
                dic_result["success"] = 0
                result = controller_validation(
                ...
                dic_result["result"] = {}
                if html.var("ru.omcConfTable.periodicStatsTimer"):
                    dic_result["result"]["ru.omcConfTable.periodicStatsTimer"] = html.var(
                        "ru.omcConfTable.periodicStatsTimer")
                if html.var("ru.omcConfTable.omcIpAddress"):
                    dic_result["result"]["ru.omcConfTable.omcIpAddress"] = html.var(
                        "ru.omcConfTable.omcIpAddress")
        ...
        if host_op_status is None or host_op_status == 0:
            if html.var("odu100_submit") == "Save":
                ...
                if html.var("host_id") == "" or html.var("host_id") is None:
                    ...
                    if html.var("ru.syncClock.syncConfigTable.rasterTime"):
                        if Validation.is_required(html.var("ru.syncClock.syncConfigTable.rasterTime")):
                            dic_result["ru.syncClock.syncConfigTable.rasterTime"] = html.var(
                                "ru.syncClock.syncConfigTable.rasterTime")
                        else:
                            dic_result[
                                "Raster Time"] = "Please Select a value in Raster time"
                            flag = 1
                    if Validation.is_required(html.var("ru.syncClock.syncConfigTable.syncLossThreshold")):
                        if Validation.is_number(html.var("ru.syncClock.syncConfigTable.syncLossThreshold")):
                            dic_result["ru.syncClock.syncConfigTable.syncLossThreshold"] = html.var(
                                "ru.syncClock.syncConfigTable.syncLossThreshold")
                        else:
                            flag = 1
                            dic_result[
                                "Sync Loss Threshold"] = "It Must be Integer"
                    else:
                        dic_result[
                            "Sync Loss Threshold"] = "Sync Loss Threshold is required"
                        flag = 1
                    if Validation.is_required(html.var("ru.syncClock.syncConfigTable.leakyBucketTimer")):
                        if Validation.is_number(html.var("ru.syncClock.syncConfigTable.leakyBucketTimer")):
                            dic_result["ru.syncClock.syncConfigTable.leakyBucketTimer"] = html.var(
                                "ru.syncClock.syncConfigTable.leakyBucketTimer")
                        else:
                            flag = 1
                            dic_result[
                                "Leaky bucket Timer"] = "It Must be Integer"
                    else:
                        dic_result[
                            "Leaky bucket Timer"] = "Sync Loss Threshold is required"
                        flag = 1
                    ...
                    if Validation.is_required(html.var("ru.syncClock.syncConfigTable.syncLostTimeout")):
                        if Validation.is_number(html.var("ru.syncClock.syncConfigTable.syncLostTimeout")):
                            dic_result["ru.syncClock.syncConfigTable.syncLostTimeout"] = html.var(
                                "ru.syncClock.syncConfigTable.syncLostTimeout")
                        else:
                            flag = 1
                            dic_result[
                                "Sync Loss Timeout"] = "Sync Lost Timeout Must be Integer"
                    else:
                        dic_result[
                            "Sync Loss Timeout"] = "Sync Loss Timeout is required"
                        flag = 1
                    if html.var("ru.syncClock.syncConfigTable.syncConfigTimerAdjust"):
                        if Validation.is_required(html.var("ru.syncClock.syncConfigTable.syncConfigTimerAdjust")):
                            if Validation.is_number(html.var("ru.syncClock.syncConfigTable.syncConfigTimerAdjust")):
                                dic_result["ru.syncClock.syncConfigTable.syncConfigTimerAdjust"] = html.var(
                                    "ru.syncClock.syncConfigTable.syncConfigTimerAdjust")
                            else:
                                flag = 1
                                dic_result[
                                    "Sync Timer Adjust"] = "Sync Timer Adjust Must be Integer"
                        else:
                            dic_result[
                    ...
                    if html.var("ru.syncClock.syncConfigTable.percentageDownlinkTransmitTime"):
                        if Validation.is_required(
                            ...
                            dic_result[
                                "Percentage Downlink"] = "Sync timer Adjust is required"
                            flag = 1
                    if flag == 1:
                        dic_result["success"] = 1
                        html.write(str(dic_result))
                ...
                if html.var("ru.syncClock.syncConfigTable.rasterTime"):
                    dic_result["ru.syncClock.syncConfigTable.rasterTime"] = html.var(
                        "ru.syncClock.syncConfigTable.rasterTime")
                if html.var("ru.syncClock.syncConfigTable.syncLossThreshold"):
                    dic_result["ru.syncClock.syncConfigTable.syncLossThreshold"] = html.var(
                        "ru.syncClock.syncConfigTable.syncLossThreshold")
                if html.var("ru.syncClock.syncConfigTable.leakyBucketTimer"):
                    dic_result["ru.syncClock.syncConfigTable.leakyBucketTimer"] = html.var(
                        "ru.syncClock.syncConfigTable.leakyBucketTimer")
                if html.var("ru.syncClock.syncConfigTable.syncLostTimeout"):
                    dic_result["ru.syncClock.syncConfigTable.syncLostTimeout"] = html.var(
                        "ru.syncClock.syncConfigTable.syncLostTimeout")
                if html.var("ru.syncClock.syncConfigTable.syncConfigTimerAdjust"):
                    dic_result["ru.syncClock.syncConfigTable.syncConfigTimerAdjust"] = html.var(
                ...
                dic_result["success"] = 0
                dic_result["result"] = {}
                dic_result["result"]["ru.syncClock.syncConfigTable.rasterTime"] = html.var(
                    "ru.syncClock.syncConfigTable.rasterTime")
                dic_result["result"]["ru.syncClock.syncConfigTable.syncLossThreshold"] = html.var(
                    "ru.syncClock.syncConfigTable.syncLossThreshold")
                dic_result["result"]["ru.syncClock.syncConfigTable.leakyBucketTimer"] = html.var(
                    "ru.syncClock.syncConfigTable.leakyBucketTimer")
                dic_result["result"]["ru.syncClock.syncConfigTable.syncLostTimeout"] = html.var(
                    "ru.syncClock.syncConfigTable.syncLostTimeout")
                dic_result["result"]["ru.syncClock.syncConfigTable.syncConfigTimerAdjust"] = html.var(
                    "ru.syncClock.syncConfigTable.syncConfigTimerAdjust")
                dic_result["result"]["ru.syncClock.syncConfigTable.percentageDownlinkTransmitTime"] = html.var(
                    "ru.syncClock.syncConfigTable.percentageDownlinkTransmitTime")
                result = odu100_common_cancel(
                    host_id, device_type_id, dic_result)
                html.write(str(result))
    ...
    try:
        ...
        if html.var("odu100_submit") == "Save":
            if html.var("host_id") == "" or html.var("host_id") is None:
                ...
                if html.var("ru.ipConfigTable.ipAddress"):
                    if Validation.is_required(html.var("ru.ipConfigTable.ipAddress")):
                        if Validation.is_valid_ip(html.var("ru.ipConfigTable.ipAddress")):
                            dic_result["ru.ipConfigTable.ipAddress"] = html.var(
                ...
                if Validation.is_required(html.var("ru.ipConfigTable.ipNetworkMask")):
                    if Validation.is_valid_ip(html.var("ru.ipConfigTable.ipNetworkMask")):
                        dic_result["ru.ipConfigTable.ipNetworkMask"] = html.var(
                            "ru.ipConfigTable.ipNetworkMask")
                    else:
                        flag = 1
                        dic_result[
                            "IpNetworkmask"] = "IpNetworkMask is not valid"
                else:
                    flag = 1
                ...
                if Validation.is_required(html.var("ru.ipConfigTable.ipDefaultGateway")):
                    if Validation.is_valid_ip(html.var("ru.ipConfigTable.ipDefaultGateway")):
                        dic_result["ru.ipConfigTable.ipDefaultGateway"] = html.var(
                            "ru.ipConfigTable.ipDefaultGateway")
                    else:
                        flag = 1
                        dic_result[
                            "IpDefaultGateway"] = "IpDefault Gateway is not valid"
                else:
                    flag = 1
                    dic_result[
                        "IpDefaultGateway"] = "IpDefault Gateway is required"
                if html.var("ru.ipConfigTable.autoIpConfig"):
                    if Validation.is_required(html.var("ru.ipConfigTable.autoIpConfig")):
                        dic_result["ru.ipConfigTable.autoIpConfig"] = html.var(
            ...
            if html.var("ru.ipConfigTable.ipAddress"):
                dic_result["ru.ipConfigTable.ipAddress"] = html.var(
                    "ru.ipConfigTable.ipAddress")
            if html.var("ru.ipConfigTable.ipNetworkMask"):
                dic_result["ru.ipConfigTable.ipNetworkMask"] = html.var(
                    "ru.ipConfigTable.ipNetworkMask")
            if html.var("ru.ipConfigTable.ipDefaultGateway"):
                dic_result["ru.ipConfigTable.ipDefaultGateway"] = html.var(
                    "ru.ipConfigTable.ipDefaultGateway")
            if html.var("ru.ipConfigTable.autoIpConfig"):
                dic_result["ru.ipConfigTable.autoIpConfig"] = html.var(
            ...
            dic_result["result"] = {}
            if html.var("ru.ipConfigTable.ipAddress"):
                dic_result["result"]["ru.ipConfigTable.ipAddress"] = html.var(
                    "ru.ipConfigTable.ipAddress")
            dic_result["result"]["ru.ipConfigTable.ipNetworkMask"] = html.var(
                "ru.ipConfigTable.ipNetworkMask")
            dic_result["result"]["ru.ipConfigTable.ipDefaultGateway"] = html.var(
                "ru.ipConfigTable.ipDefaultGateway")
            dic_result["result"]["ru.ipConfigTable.autoIpConfig"] = html.var(
                "ru.ipConfigTable.autoIpConfig")
        ...
        if host_op_status is None or host_op_status == 0:
            if html.var("odu100_submit") == "Save":
                ...
                if html.var("host_id") == "" or html.var("host_id") is None:
                            ...
                            if Validation.is_required(html.var("ru.ra.raConfTable.numSlaves")):
                                if int(html.var("ru.ra.raConfTable.numSlaves")) == 1:
                                    ...
                                    if str(check_timeslot(host_id, int(
                                        ...
                                        timeslot_chk = 1
                                        timeslot = check_timeslot(host_id, int(
                                            html.var("ru.ra.raConfTable.numSlaves")))
                            else:
                                flag = 1
                                dic_result[
                                    "result"] = "Please Select Number of Slaves"
                        else:
                            flag = 1
                            dic_result[
                                "ru.ra.raConfTable.numSlaves"] = "Please Select Number of Slaves"
                    ...
                    if html.var("ru.ra.raConfTable.ssID") is None:
                        ...
                        if len(html.var("ru.ra.raConfTable.ssID")) >= 0 and len(
                            ...
                            flag = 1
                            dic_result[
                                "result"] = "ssID must be in 0 to 32 character"

                    if html.var("ru.ra.tddMac.raTddMacConfigTable.encryptionType"):
                        if Validation.is_required(html.var("ru.ra.tddMac.raTddMacConfigTable.encryptionType")):
                            dic_result["ru.ra.tddMac.raTddMacConfigTable.encryptionType"] = html.var(
                    ...
                    if html.var("ru.ra.tddMac.raTddMacConfigTable.passPhrase"):
                        if len(html.var("ru.ra.tddMac.raTddMacConfigTable.passPhrase")) >= 0 and len(
                            ...
                            flag = 1
                            dic_result[
                                "result"] = "Pass Phrase must be in upto 64 character"
                    else:
                        dic_result[
                            "ru.ra.tddMac.raTddMacConfigTable.passPhrase"] = ""
                    if html.var("ru.ra.tddMac.raTddMacConfigTable.txPower"):
                        if Validation.is_required(html.var("ru.ra.tddMac.raTddMacConfigTable.txPower")):
                            ...
                            if Validation.is_number(html.var("ru.ra.tddMac.raTddMacConfigTable.txPower")):
                                dic_result["ru.ra.tddMac.raTddMacConfigTable.txPower"] = html.var(
                                    "ru.ra.tddMac.raTddMacConfigTable.txPower")
                            else:
                                flag = 1
                                dic_result[
                                    "result"] = "Tx Power Must be Integer"
                        else:
                            flag = 1
                            dic_result["Tx Power"] = "Tx power is required"
                    if html.var("ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors"):
                        if Validation.is_required(html.var("ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors")):
                            if Validation.is_number(html.var("ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors")):
                                dic_result["ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors"] = html.var(
                                    "ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors")
                            else:
                                flag = 1
                                dic_result[
                                    "result"] = "Max Crc Error Must be Integer"
                        else:
                            flag = 1
                            dic_result["result"] = "Max Crc Error is required"

                    if html.var("ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue"):
                        if Validation.is_required(html.var("ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue")):
                            if Validation.is_number(html.var("ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue")):
                                dic_result["ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue"] = html.var(
                                    "ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue")
                            else:
                                flag = 1
                                dic_result[
                                    "result"] = "Leaky Bucket Timer Must be Integer"
                        else:
                            flag = 1
                            dic_result[
                                "result"] = "Leaky Bucket Timer is required"
                    if int(html.var("node_type")) == 0 or int(html.var("node_type")) == 2:
                        if html.var("ru.ra.raConfTable.acm"):
                            if Validation.is_required(html.var("ru.ra.raConfTable.acm")):
                                dic_result["ru.ra.raConfTable.acm"] = html.var(
                    ...
                    if int(html.var("node_type")) == 0 or int(html.var("node_type")) == 2:
                        if html.var("ru.ra.raConfTable.dba"):
                            if Validation.is_required(html.var("ru.ra.raConfTable.dba")):
                                dic_result["ru.ra.raConfTable.dba"] = html.var(
                    ...
                    if html.var("ru.ra.raConfTable.guaranteedBroadcastBW"):
                        if Validation.is_required(html.var("ru.ra.raConfTable.guaranteedBroadcastBW")):
                            if Validation.is_number(html.var("ru.ra.raConfTable.guaranteedBroadcastBW")):
                                    ...
                                    if Validation.is_required(html.var("ru.ra.raConfTable.numSlaves")):
                                        if int(html.var("ru.ra.raConfTable.numSlaves")) == 1:
                                            ...
                                            if int(html.var("ru.ra.raConfTable.numSlaves")) > 1 and int(
                                                ...
                                                flag = 1
                                                dic_result[
                                                    "result"] = "If number of slaves more than one, guaranteed broadcast Bandwidth cant be zero"
                                    else:
                                        flag = 1
                                else:
                                    dic_result["ru.ra.raConfTable.guaranteedBroadcastBW"] = html.var(
                                ...
                                flag = 1
                                dic_result[
                                    "result"] = "GuaranteedBroadcastBW Must be Integer"
                        else:
                            flag = 1
                            dic_result[
                                "result"] = "GuaranteedBroadcast is required"
                    if int(html.var("node_type")) == 0 or int(html.var("node_type")) == 2:
                        if html.var("ru.ra.raConfTable.acs"):
                            if Validation.is_required(html.var("ru.ra.raConfTable.acs")):
                                dic_result["ru.ra.raConfTable.acs"] = html.var(
                                    "ru.ra.raConfTable.acs")
                            else:
                                flag = 1
                                dic_result["result"] = "Please Select ACS"
                    if int(html.var("node_type")) == 0 or int(html.var("node_type")) == 2:
                        if html.var("ru.ra.raConfTable.dfs"):
                            if Validation.is_required(html.var("ru.ra.raConfTable.dfs")):
                                dic_result["ru.ra.raConfTable.dfs"] = html.var(
                                    "ru.ra.raConfTable.dfs")
                            else:
                                flag = 1
                                dic_result["result"] = "Please Select DFS"

                    if html.var("ru.ra.raConfTable.antennaPort"):
                        if Validation.is_required(html.var("ru.ra.raConfTable.antennaPort")):
                            dic_result["ru.ra.raConfTable.antennaPort"] = html.var(
                                "ru.ra.raConfTable.antennaPort")
                        else:
                            flag = 1
                            dic_result["result"] = "Please Select Antenna Port"
                    if int(html.var("node_type")) == 0 or int(html.var("node_type")) == 2:
                        if html.var("ru.ra.raConfTable.linkDistance"):
                            if Validation.is_required(html.var("ru.ra.raConfTable.linkDistance")):
                                dic_result["ru.ra.raConfTable.linkDistance"] = html.var(
                                    "ru.ra.raConfTable.linkDistance")
                            else:
                                flag = 1
                                dic_result[
                                    "result"] = "Please Select Link Distance"
                    ...
                    if html.var("ru.ra.raConfTable.forceMimo"):
                        if Validation.is_required(html.var("ru.ra.raConfTable.forceMimo")):
                            if (html.var("ru.ra.raConfTable.antennaPort") == "3" or
                                ...
                                flag = 1
                                dic_result[
                                    "result"] = "For forceMIMO, antenna port should be set to MIMO"
                        else:
                            flag = 1
                            dic_result[
                                "result"] = "Please Select force Mimo"
                if flag == 1:
                    dic_result["success"] = 1
                    html.write(str(dic_result))
                ...
                if html.var("ru.ra.raConfTable.numSlaves"):
                    dic_result["ru.ra.raConfTable.numSlaves"] = html.var(
                        "ru.ra.raConfTable.numSlaves")
                if html.var("ru.ra.raConfTable.ssID"):
                    dic_result["ru.ra.raConfTable.ssID"] = html.var(
                        "ru.ra.raConfTable.ssID")
                if html.var("ru.ra.tddMac.raTddMacConfigTable.encryptionType"):
                    dic_result["ru.ra.tddMac.raTddMacConfigTable.encryptionType"] = html.var(
                ...
                dic_result["result"] = {}
                if html.var("ru.ra.raConfTable.numSlaves"):
                    dic_result["result"]["ru.ra.raConfTable.numSlaves"] = html.var(
                        "ru.ra.raConfTable.numSlaves")
                dic_result["result"]["ru.ra.raConfTable.ssID"] = html.var(
                    "ru.ra.raConfTable.ssID")
                dic_result["result"]["ru.ra.tddMac.raTddMacConfigTable.encryptionType"] = html.var(
                    "ru.ra.tddMac.raTddMacConfigTable.encryptionType")
                dic_result["result"]["ru.ra.tddMac.raTddMacConfigTable.passPhrase"] = html.var(
                    "ru.ra.tddMac.raTddMacConfigTable.passPhrase")
                dic_result["result"]["ru.ra.tddMac.raTddMacConfigTable.txPower"] = html.var(
                    "ru.ra.tddMac.raTddMacConfigTable.txPower")
                dic_result["result"]["ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors"] = html.var(
                    "ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors")
                dic_result["result"]["ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue"] = html.var(
                    "ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue")
                if html.var("ru.ra.raConfTable.acm"):
                    dic_result["result"]["ru.ra.raConfTable.acm"] = html.var(
                        "ru.ra.raConfTable.acm")
                if html.var("ru.ra.raConfTable.dba"):
                    dic_result["result"]["ru.ra.raConfTable.dba"] = html.var(
                        "ru.ra.raConfTable.dba")
                if html.var("ru.ra.raConfTable.guaranteedBroadcastBW"):
                    dic_result["result"]["ru.ra.raConfTable.guaranteedBroadcastBW"] = html.var(
                        "ru.ra.raConfTable.guaranteedBroadcastBW")
                if html.var("ru.ra.raConfTable.dfs"):
                    dic_result["result"]["ru.ra.raConfTable.dfs"] = html.var(
                        "ru.ra.raConfTable.dfs")
                if html.var("ru.ra.raConfTable.anc"):
        ...
        if host_op_status is None or host_op_status == 0:
            if html.var("odu100_submit") == "Save":
                ...
                if Validation.is_required(html.var("ru.ruConfTable.channelBandwidth")):
                    dic_result["ru.ruConfTable.channelBandwidth"] = html.var(
                        "ru.ruConfTable.channelBandwidth")
                else:
                    flag = 1
                    dic_result["result"] = "Please Select Channel Bandwidth "

                if html.var("ru.ruConfTable.synchSource"):
                    if Validation.is_required(html.var("ru.ruConfTable.synchSource")):
                        dic_result["ru.ruConfTable.synchSource"] = html.var(
                ...
                if html.var("RU.RUConfTable.alignmentControl"):
                    if Validation.is_required(html.var("RU.RUConfTable.alignmentControl")):
                        if Validation.is_number(html.var("RU.RUConfTable.alignmentControl")):
                            dic_result["RU.RUConfTable.alignmentControl"] = html.var(
                                "RU.RUConfTable.alignmentControl")
                        else:
                            dic_result[
                                "result"] = "Alignment Control should be Number"
                    else:
                ...
                obj_essential.host_status(host_id, 12)
                if html.var("ru.ruConfTable.channelBandwidth"):
                    dic_result["ru.ruConfTable.channelBandwidth"] = html.var(
                        "ru.ruConfTable.channelBandwidth")
                if html.var("ru.ruConfTable.synchSource"):
                    dic_result["ru.ruConfTable.synchSource"] = html.var(
                        "ru.ruConfTable.synchSource")
                if html.var("ru.ruConfTable.countryCode"):
                    dic_result["ru.ruConfTable.countryCode"] = html.var(
                        "ru.ruConfTable.countryCode")
                if html.var("ru.ruConfTable.poeState"):
                    dic_result["ru.ruConfTable.poeState"] = html.var(
                ...
                dic_result["success"] = 0
                dic_result["result"] = {}
                dic_result["result"]["ru.ruConfTable.channelBandwidth"] = html.var(
                    "ru.ruConfTable.channelBandwidth")
                dic_result["result"]["ru.ruConfTable.synchSource"] = html.var(
                    "ru.ruConfTable.synchSource")
                dic_result["result"]["ru.ruConfTable.countryCode"] = html.var(
                    "ru.ruConfTable.countryCode")
                dic_result["result"]["ru.ruConfTable.poeState"] = html.var(
                    "ru.ruConfTable.poeState")
                dic_result["result"]["RU.RUConfTable.alignmentControl"] = html.var(
                    "RU.RUConfTable.alignmentControl")
                result = odu100_common_cancel(
                    host_id, device_type_id, dic_result)
                html.write(str(result))
    ...
    try:
        ...
        if html.var("odu100_submit") == "Save":
            if host_op_status is None or host_op_status == 0:
                ...
                if html.var("host_id") == "" or html.var("host_id") is None:
                    ...
                    #logme(" i am in "+str(html.req.vars))
                    if html.var("timeslot_val"):
                            ...
                            if html.var("ru.ra.peerNode.peerConfigTable.peerMacAddress.%s" % (i)) is not None:
                                if html.var("acl_val"):
                                    if int(html.var("acl_val")) == 1 or int(html.var("acl_val")) == 2:
                                        mac_list.append(html.var(
                                            "ru.ra.peerNode.peerConfigTable.peerMacAddress.%s" % (i)))
                                mac_duplicate_list.append(
                                    html.var("ru.ra.peerNode.peerConfigTable.peerMacAddress.%s" % (i)))
                            ...
                            if html.var("ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s" % (i)) is not None:
                                if Validation.is_required(
                                    ...
                                    if Validation.is_number(
                                        ...
                                        flag = 1
                                        dic_result[
                                            "result"] = "GuaranteedUplinkBW must be number"
                                else:
                                    flag = 1
                                    dic_result[
                            ...
                            if html.var("ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s" % (i)) is not None:
                                if Validation.is_required(
                                    ...
                                    if Validation.is_number(
                                        ...
                                        flag = 1
                                        dic_result[
                                            "result"] = "GuaranteedDownlinkBW must be number"
                                else:
                                    flag = 1
                                    dic_result[
                            ...
                            if html.var("ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s" % (i)) is not None:
                                if Validation.is_required(
                                    ...
                                    if Validation.is_number(
                                        ...
                                        flag = 1
                                        dic_result[
                                            "result"] = "BasicRateMCSIndex must be number"
                                else:
                                    flag = 1
                                    dic_result[
                            ...
                            if html.var("ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s" % (i)) is not None:
                                if Validation.is_required(
                                    ...
                                    if Validation.is_number(
                                        ...
                                        flag = 1
                                        dic_result[
                                            "result"] = "Maximum Uplink must be number"
                                else:
                                    flag = 1
                                    dic_result[
                            ...
                            if html.var("ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s" % (i)) is not None:
                                if Validation.is_required(
                                    ...
                                    if Validation.is_number(
                                        ...
                                        flag = 1
                                        dic_result[
                                            "result"] = "Maximum Downlink must be number "
                                else:
                                    flag = 1
                                    dic_result[
                                        "result"] = "Maximum Downlink is required "
                        if flag == 1:
                            dic_result["success"] = 1
                            html.write(str(dic_result))
                        else:
                            if chk_mac_duplicacy(mac_duplicate_list) == 0:
                                ...
                                if mac_accept == 0 and mac_deny == 0 or mac_deny == 2:
                                    dic_result["success"] = 0
                                    #logme(" result "+str(dic_result))
                                    # html.write(str(dic_result))
                                    result = peer_set(
                                        host_id, device_type_id, timeslot_val, dic_result, 1)
                                    ...
                                    if mac_deny == 1:
                                        dic_result["success"] = 1
                                        dic_result[
                                            "result"] = "ACL DENY mode PEER MAC should be different from ACL MAC list"
                                    html.write(str(dic_result))
                            else:
                                duplicate = 1
                            if duplicate == 1:
                                dic_result["success"] = 1
                                dic_result[
                                    "result"] = "Same MAC Address are not allowed"
                                html.write(str(dic_result))
                    else:
            ...
            if host_op_status is None or host_op_status == 0:
                ...
                if html.var("timeslot_val"):
                    ...
                    for i in range(1, timeslot_val + 1):
                        ...
                        if html.var("ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s" % (i)) is not None:
                            dic_result["ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s" % (
                                i)] = html.var("ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s" % (1))
                        if html.var("ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s" % (i)) is not None:
                            dic_result["ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s" % (
                                i)] = html.var("ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s" % (1))
                        if html.var("ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s" % (i)) is not None:
                            dic_result["ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s" % (
                                i)] = html.var("ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s" % (1))
                        if html.var("ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s" % (i)) is not None:
                            dic_result["ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s" % (
                                i)] = html.var("ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s" % (1))
                        if html.var("ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s" % (i)) is not None:
                            dic_result["ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s" % (
                                i)] = html.var("ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s" % (1))
                    ...
                    if mac_accept == 0 or mac_deny == 0:
                        dic_result["success"] = 0
                        result = peer_set(host_id, device_type_id,
                                          timeslot_val, dic_result, 2)
                        html.write(str(result))
                    else:
                        if mac_accept == 1:
                            dic_result["success"] = 1
        ...
        if html.var("odu100_submit") == "Cancel":
            for i in range(1, timeslot_val + 1):
                ...
                if html.var("ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s" % (i)) is not None:
                    dic_result["ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s" % (i)] = html.var(
        ...
        if host_op_status is None or host_op_status == 0:
            if html.var("odu100_submit") == "Save":
                ...
                if html.var("host_id") == "" or html.var("host_id") is None:
                    ...
                    if html.var("ru.ra.llc.raLlcConfTable.arqWinHigh"):
                        if Validation.is_required(html.var("ru.ra.llc.raLlcConfTable.arqWinHigh")):
                            if Validation.is_number(html.var("ru.ra.llc.raLlcConfTable.arqWinHigh")):
                                dic_result["ru.ra.llc.raLlcConfTable.arqWinHigh"] = html.var(
                                    "ru.ra.llc.raLlcConfTable.arqWinHigh")
                            else:
                                dic_result[
                                    "result"] = "Retransmit Window Size(High) must be number"
                                flag = 1
                        else:
                            dic_result[
                                "result"] = "Retransmit Window Size(High) is required"
                            flag = 1
                    if html.var("ru.ra.llc.raLlcConfTable.arqWinLow"):
                        if Validation.is_required(html.var("ru.ra.llc.raLlcConfTable.arqWinLow")):
                            if Validation.is_number(html.var("ru.ra.llc.raLlcConfTable.arqWinLow")):
                                dic_result["ru.ra.llc.raLlcConfTable.arqWinLow"] = html.var(
                                    "ru.ra.llc.raLlcConfTable.arqWinLow")
                            else:
                                dic_result[
                                    "result"] = "Retransmit Window Size(Low) must be number"
                                flag = 1
                        else:
                            dic_result[
                                "result"] = "Retransmit Window Size(Low) is required"
                            flag = 1
                    if html.var("ru.ra.llc.raLlcConfTable.frameLossThreshold"):
                        if Validation.is_required(html.var("ru.ra.llc.raLlcConfTable.frameLossThreshold")):
                            if Validation.is_number(html.var("ru.ra.llc.raLlcConfTable.frameLossThreshold")):
                                dic_result["ru.ra.llc.raLlcConfTable.frameLossThreshold"] = html.var(
                                    "ru.ra.llc.raLlcConfTable.frameLossThreshold")
                            else:
                                dic_result[
                                    "result"] = "FrameLoss Threshold must be number must be number"
                                flag = 1
                        else:
                            dic_result[
                                "result"] = "FrameLoss Threshold is required"
                            flag = 1
                    if html.var("ru.ra.llc.raLlcConfTable.leakyBucketTimerVal"):
                        if Validation.is_required(html.var("ru.ra.llc.raLlcConfTable.leakyBucketTimerVal")):
                            if Validation.is_number(html.var("ru.ra.llc.raLlcConfTable.leakyBucketTimerVal")):
                                dic_result["ru.ra.llc.raLlcConfTable.leakyBucketTimerVal"] = html.var(
                                    "ru.ra.llc.raLlcConfTable.leakyBucketTimerVal")
                            else:
                                dic_result[
                                    "result"] = "Leaky Bucket Timer must be number"
                                flag = 1
                        else:
                            dic_result[
                                "result"] = "Leaky Bucket Timer is required"
                            flag = 1
                    if html.var("ru.ra.llc.raLlcConfTable.frameLossTimeout"):
                        if Validation.is_required(html.var("ru.ra.llc.raLlcConfTable.frameLossTimeout")):
                            if Validation.is_number(html.var("ru.ra.llc.raLlcConfTable.frameLossTimeout")):
                                dic_result["ru.ra.llc.raLlcConfTable.frameLossTimeout"] = html.var(
                                    "ru.ra.llc.raLlcConfTable.frameLossTimeout")
                            else:
                                dic_result[
                                    "result"] = "Frame Loss Timeout must be number"
                                flag = 1
                        else:
                            dic_result[
                                "result"] = "Frame Loss Timeout is required"
                            flag = 1
                ...
                dic_result["success"] = 0
                dic_result["result"] = {}
                dic_result["result"]["ru.ra.llc.raLlcConfTable.arqWinHigh"] = html.var(
                    "ru.ra.llc.raLlcConfTable.arqWinHigh")
                dic_result["result"]["ru.ra.llc.raLlcConfTable.arqWinLow"] = html.var(
                    "ru.ra.llc.raLlcConfTable.arqWinLow")
                dic_result["result"]["ru.ra.llc.raLlcConfTable.frameLossThreshold"] = html.var(
                    "ru.ra.llc.raLlcConfTable.frameLossThreshold")
                dic_result["result"]["ru.ra.llc.raLlcConfTable.leakyBucketTimerVal"] = html.var(
                    "ru.ra.llc.raLlcConfTable.leakyBucketTimerVal")
                dic_result["result"]["ru.ra.llc.raLlcConfTable.frameLossTimeout"] = html.var(
                    "ru.ra.llc.raLlcConfTable.frameLossTimeout")
                result = odu100_common_cancel(
                    host_id, device_type_id, dic_result)
                html.write(str(result))
        ...
        if host_op_status is None or host_op_status == 0:
            if html.var("odu100_submit") == "Save" or html.var("odu100_submit") == "Retry" or html.var(
                ...
                if flag == 1:
                    ...
                    # html.write(str(dic_result))
                    result = channel_config_set(
                        host_id, device_type_id, dic_result)
                    html.write(str(result))
            elif html.var("odu100_submit") == "Cancel":
                for i in range(0, channel_config):
                    if html.var('RU.RA.PrefRfChanFreq.frequency.%s' % (i + 1)):
                        dic_result['RU.RA.PrefRfChanFreq.frequency.%s' % (
                            i + 1)] = html.var('RU.RA.PrefRfChanFreq.frequency.%s' % (i + 1))
                dic_result["success"] = 0
                # result =
                # odu100_common_cancel(host_id,device_type_id,dic_result)
                html.write(str(dic_result))
        else:
            dic_result['result'] = "Device is busy, Device " + str(
                host_status_dic[host_op_status]) + " is in progress. please wait ..."
        ...
        if host_op_status == 0:
            if html.var("odu100_submit") == "Save" or html.var("odu100_submit") == "Retry" or html.var(
                    ...
                    for i in range(1, ip_range):
                            ...
                            if html.var('ru.packetFilters.ipFilterTable.ipFilterIpAddress.%s' % (i)) != "":
                                if Validation.is_valid_ip(
                                    ...
                                    i)) not in dic_result.values():
                                        ...
                                        flag = 1
                                        dic_result[
                                            "result"] = "Dupliacte IP Address used of index no %s " % (i)
                                else:
                                    flag = 1
                        ...
                        if html.var('ru.packetFilters.ipFilterTable.ipFilterNetworkMask.%s' % (i)):
                            if html.var('ru.packetFilters.ipFilterTable.ipFilterNetworkMask.%s' % (i)) != "":
                                ...
                                flag = 1
                                dic_result[
                                    "result"] = "Net-Mask should be a valid IP Address of index no %s " % (i)
                ...
                if flag == 1:
                    ...
                    # html.write(str(dic_result))
                    result = packet_filter_set(
                        host_id, device_type_id, 0, dic_result)
                    html.write(str(result))
            elif html.var("odu100_submit") == "Cancel":
                for i in range(1, ip_range):
                    if html.var('ru.packetFilters.ipFilterTable.ipFilterIpAddress.%s' % (i)):
                        dic_result['ru.packetFilters.ipFilterTable.ipFilterIpAddress.%s' % (1)] = html.var(
                            'ru.packetFilters.ipFilterTable.ipFilterIpAddress.%s' % (i))
                    if html.var('ru.packetFilters.ipFilterTable.ipFilterNetworkMask.%s' % (i)):
                        dic_result['ru.packetFilters.ipFilterTable.ipFilterNetworkMask.%s' % (1)] = html.var(
                            'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.%s' % (i))
                dic_result["success"] = 0
                # result =
                # odu100_common_cancel(host_id,device_type_id,dic_result)
                html.write(str(dic_result))
        ...
        if host_op_status == 0:
            if html.var("odu100_submit") == "Save" or html.var("odu100_submit") == "Retry" or html.var(
                ...
                if flag == 1:
                    ...
                    # html.write(str(dic_result))
                    result = packet_filter_set(
                        host_id, device_type_id, 1, dic_result)
                    html.write(str(result))
            elif html.var("odu100_submit") == "Cancel":
                for i in range(1, ip_range):
                    if html.var('ru.packetFilters.macFilterTable.filterMacAddress.%s' % (i)):
                        dic_result['ru.packetFilters.macFilterTable.filterMacAddress.%s' % (1)] = html.var(
                            'ru.packetFilters.macFilterTable.filterMacAddress.%s' % (i))
                dic_result["success"] = 0
                # result =
                # odu100_common_cancel(host_id,device_type_id,dic_result)
        ...
        if host_op_status == 0:
            if html.var("odu100_submit") == "Save" or html.var("odu100_submit") == "Retry" or html.var(
                ...
                if html.var("host_id") == "" or html.var("host_id") is None:
                    ...
                    if html.var('ru.ruConfTable.ethFiltering'):
                        ...
                        html.write(str({"success": 1, "result": "Please select Filter Mode"}))
                        return

                if flag == 2:
                    dic_result["success"] = 1
                    dic_result[
                        "result"] = "Can not enable IP filtering if there are no entries in IP filter config table"
                    html.write(str(dic_result))
                elif flag == 3:
                    dic_result["success"] = 1
                    dic_result[
                        "result"] = "Can not enable MAC filtering if there are no entries in MAC filter config table"
                    html.write(str(dic_result))
                else:
                    if flag == 0:
                        dic_result['ru.ruConfTable.ethFiltering'] = html.var(
        ...
        if host_op_status is None or host_op_status == 0:
            ...
            if mac_exist == 1:
                ...
                if int(acl_len) == 1 and int(acl_mode) == 1:
                    dic_result["success"] = 1
                    dic_result[
                        "result"] = "Last mac address cant be removed if ACL mode is ACCEPT"
                    html.write(str(dic_result))
                else:
    ...
    try:
        if host_op_status is None or host_op_status == 0:
            ...
            if html.var("odu100_submit") == "Add" or html.var("odu100_submit") == "Edit":
                if html.var("aclindex") == "" or html.var("aclindex") is None:
                    flag = 1
                ...
                if html.var("raacl_index") == "" or html.var("raacl_index") is None:
                    raindex = ""
                else:
                ...
                if flag == 1:
                    ...
                    dic_result["success"] = 1
                    dic_result[
                        "result"] = "Mac Address Present in PeerMac Address"
                    html.write(str(dic_result))
                    ...
                    result = acl_controller_add_edit(
                        host_id, device_type_id, dic_result, raindex)
                    html.write(str(result))
                ...
                dic_result["result"]["ru.ra.raConfTable.aclMode"] = html.var(
...
def odu100_acl_mode(h):
    ...
    try:
        ...
        if host_op_status is None or host_op_status == 0:
            obj_essential.host_status(host_id, 12)
            if html.var("odu100_submit") == "Save":
                if html.var("ru.ra.raConfTable.aclMode") is None or html.var("ru.ra.raConfTable.aclMode") == "":
                    flag = 1
                ...
                if flag == 1:
                    ...
                    dic_result["success"] = 1
                    dic_result[
                        "result"] = "ACL Mode cannot be ACCEPT if the ACL MAC list is empty"
                    html.write(str(dic_result))
                elif acl_deny == 1:
                ...
                if html.var("ru.ra.raConfTable.aclMode"):
                    dic_result["ru.ra.raConfTable.aclMode"] = html.var(
                        "ru.ra.raConfTable.aclMode")
                result = controller_validation(
                    host_id, device_type_id, dic_result)
...
def update_mac_list(h):
    ...
    if len(acl_configuration) > 0:
        ...
        # This variable is used for storing total number of records of database
        # table [type - int]
        for i in range(0, len(acl_configuration)):
            odu100_acl_config_form += "<tr>\
                                            <td>%s</td>\
...
def odu16_reconcilation(h):
    ...
    if html.var("insert_update") is None:
        insert_update = True
    else:
    ...
    if device_type_id == "odu16":
        result = obj_reconcilation.odu16_reconcilation_controller_update(
            host_id, device_type_id, table_prefix, insert_update, user_name)
    else:
        result = obj_reconcilation.update_reconcilation_controller(
            host_id, device_type_id, "odu100_", datetime.now(), user_name)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(result)))
...
def odu100_acl_reconcile(h):
    ...
    try:
        ...
        if host_op_status is None or host_op_status == 0:
            ...
            obj_reconcilation = OduReconcilation()
            result = obj_reconcilation.odu100_acl_reconciliation(
                host_id, device_type)
        else:
            result = "Device is busy, Device " + str(
                host_status_dic[host_op_status]) + " is in progress. please wait ..."
...
def refresh_channel_list(h):
    ...
    if int(refresh) == 0 or int(refresh) == 2:
        ...
        result = obj.odu100_channel_configuration(
            host_id, device_type, 2 if int(refresh) == 2 else 0)
    else:
...
def ra_channel_list_table(h):
    ...
    channel_list = obj_get_channel_list.common_get_data_by_host(
        'Odu100RaChannelListTable', host_id)
    table_str = ""
...
def site_survey_data(h):
    ...
    if host_op_status is None or host_op_status == 0:
        ...
        if int(site_survey_status) == 2:
            site_survey_record = obj_site_survey_data.common_get_data_by_host(
                'Odu100RaSiteSurveyResultTable', host_id)
            if len(site_survey_record) > 0 or site_survey_record != []:
                html.write(str(0))
            else:
                html.write(str(1))
...
def site_survey_snmp(h):
    ...
    try:
        ...
        if host_op_status is None or host_op_status == 0:
            ...
            result['success'] = 1
            result['result'] = "Device is busy, Device " + str(
                host_status_dic[host_op_status]) + " is in progress. please wait ..."
        html.req.content_type = 'application/json'
        html.req.write(str(JSONEncoder().encode(result)))
    except Exception as e:
...
def hw_sw_frequency_status(h):
    ...
    if int(result_dic['success']) == 0:
        html_str += "<tr><th>S/W Version<th><td>%s</td></tr>" % (
            "-" if result_dic['result']['sw_status'] is None else result_dic['result']['sw_status'])
        html_str += "<tr><th>H/W Serial Number<th><td>%s</td></tr>" % (
            "-" if result_dic['result']['hw_serail_number'] is None else result_dic['result']['hw_serail_number'])
            ...
            if len(result_dic['result']['channel_list']) > 0:
                ...
                for i in result_dic['result']['channel_list']:
                    html_str += "<tr>\
                                <td>%s</td>\
                                <td>%s</td>\
                            </tr>" % (0 if i is None else i, result_dic['result']['channel_list'][i])
        else:
            html_str += "<tr><th>Frequency<th><td>%s</td></tr>" % (
                "-" if result_dic['result']['frequency'] is None else result_dic['result']['frequency'])
...
def change_admin_state(h):
    ...
    try:
        ...
        if host_op_status is None or host_op_status == 0:
            obj_essential.host_status(host_id, 12)
            obj = OduStatus()
            result_dic = obj.admin_state_change(
                host_id, device_type, admin_state_name, state)
        else:
            result_dic['result'] = "Device is busy, Device " + str(
                host_status_dic[host_op_status]) + " is in progress. please wait ..."
...
def all_lock_unlock(h):
    ...
    state = html.var("state")
    obj = OduStatus()
    result_dic = obj.all_lock_unlocked(
        host_id, device_type, admin_state_name, state)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(result_dic)))
