...
from utility import (ErrorMessages, Validation,
                     HostState, NagiosConfiguration, SystemSetting, DiscoveryName,
                     PingDiscovery, SnmpDiscovery, UpnpDiscovery, UNMPDeviceType)
...
class HostgroupBll(object):
    """
    Hostgroup related Modal Class
    """
    def get_data_table_sqlalchemy(self, a_columns, table_columns, table_classes, table_join, other_conditions,
                                  i_display_start, i_display_length, s_search, sEcho, sSortDir_0, iSortCol_0, req_vars,
                                  join_conditions=[], group_by=False, diff_count=3):
        """
                            ...
                            if len(join_conditions) > class_i - 1:
                                ...
                                for join_condition in join_conditions[class_i - 1]:
                                    if isinstance(join_condition, dict):
                                        if isinstance(join_condition.get("join_with", None), str) or isinstance(
                                                join_condition.get("join_with", None), int) or isinstance(
                                                join_condition.get("join_with", None), long) or isinstance(
                                                join_condition.get("join_with", None), float):
                                            join_conditions_list.append(getattr(
                                                table_classes[class_i], join_condition["table_column"]) ==
                                                                        join_condition["join_with"])
                                        elif isinstance(join_condition.get("join_with", None), dict):
                                            join_conditions_list.append(
                                                getattr(table_classes[class_i], join_condition["table_column"])
                                                == getattr(join_condition["join_with"]["table_class"],
                                                           join_condition["join_with"]["table_column"]))
            ...
            for other_condition in other_conditions:
                if isinstance(other_condition, dict):
                    if other_condition.get("type", None) == "like":
                        if other_condition.get("table_class", None) != None and other_condition.get("table_column",
                                                                                                    None) != None:
                    ...
                    for class_i in range(0, len(table_classes)):
                        ...
                        if len(table_columns) > class_i:
                            for column_i in range(0, len(table_columns[class_i])):
                                ...
                                if table_columns[class_i][column_i] == a_columns[i_sort_col_i] and abs(
                                                i_sort_col_i - col_number) <= diff_count:
                                    if s_sort_dir_i == "asc":
    ...
    def grid_view(self, hostgroup_id_list, flag=1):
        ...
        try:
            ...
            if flag:
                hostgroup = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name,
                                          Hostgroups.hostgroup_alias, Hostgroups.is_default,
                                          Groups.group_name).outerjoin(
                    HostgroupsGroups, Hostgroups.hostgroup_id == HostgroupsGroups.hostgroup_id).outerjoin(Groups,
                                                                                                          HostgroupsGroups.group_id == Groups.group_id).filter(
                    and_(Hostgroups.is_deleted == 0, Hostgroups.hostgroup_id.in_(hostgroup_id_list))).order_by(
                    Hostgroups.hostgroup_name).all()          # execute query and fetch data
            else:
                hostgroup = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name,
                                          Hostgroups.hostgroup_alias, Hostgroups.is_default,
                                          Groups.group_name).outerjoin(
                    HostgroupsGroups, Hostgroups.hostgroup_id == HostgroupsGroups.hostgroup_id).outerjoin(Groups,
                                                                                                          HostgroupsGroups.group_id == Groups.group_id).filter(
                    Hostgroups.is_deleted == 0).order_by(
                    Hostgroups.hostgroup_name).all()          # execute query and fetch data
            return hostgroup
        except Exception, e:
            return e
    ...
    def grid_view_number_of_hosts(self, hostgroup_id_list, flag=1):
        ...
        try:
            ...
            if flag:
                hostgroup = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name,
                                          Hostgroups.hostgroup_alias, func.count(Hosts.device_type_id),
                                          DeviceType.device_name).outerjoin(HostsHostgroups,
                                                                            Hostgroups.hostgroup_id == HostsHostgroups.hostgroup_id).outerjoin(
                    Hosts, HostsHostgroups.host_id == Hosts.host_id).outerjoin(DeviceType,
                                                                               Hosts.device_type_id == DeviceType.device_type_id).filter(
                    and_(Hostgroups.is_deleted == 0, Hosts.is_deleted == 0, Hosts.host_state_id == 'e',
                         Hostgroups.hostgroup_id.in_(hostgroup_id_list))).group_by(Hosts.device_type_id,
                                                                                   Hostgroups.hostgroup_id).order_by(
                    Hostgroups.hostgroup_name).all()          # execute query and fetch data
            else:
                hostgroup = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name,
                                          Hostgroups.hostgroup_alias, func.count(Hosts.device_type_id),
                                          DeviceType.device_name).outerjoin(
                    HostsHostgroups, Hostgroups.hostgroup_id == HostsHostgroups.hostgroup_id).outerjoin(Hosts,
                                                                                                        HostsHostgroups.host_id == Hosts.host_id).outerjoin(
                    DeviceType, Hosts.device_type_id == DeviceType.device_type_id).filter(
                    and_(Hostgroups.is_deleted == 0, Hosts.is_deleted == 0, Hosts.host_state_id == 'e')).group_by(
                    Hosts.device_type_id, Hostgroups.hostgroup_id).order_by(Hostgroups.hostgroup_name).all()
    ...
    def get_hostgroup_by_id(self, hostgroup_id):
        ...
        try:
            ...
            hostgroups = session.query(
                Hostgroups.hostgroup_id, Hostgroups.hostgroup_name, Hostgroups.hostgroup_alias, Hostgroups.is_default,
                Hostgroups.timestamp, Hostgroups.created_by, Hostgroups.creation_time,
                Hostgroups.updated_by).filter(
                Hostgroups.hostgroup_id == hostgroup_id).all()          # execute query and fetch data
            if len(hostgroups) == 1:
                return hostgroups[0]
            else:
                err = ErrorMessages()
    ...
    def add(self, hostgroup_name, hostgroup_alias, timestamp, created_by, creation_time, is_deleted, updated_by,
            is_default, nms_instance, group_name):
        """
                ...
                if self.validate(hostgroup_name, hostgroup_alias):
                    ...
                    if is_duplicate == 0:
                        ...
                        if (condition_nagios):
                            ...
                            el_obj = EventLog()
                            el_obj.log_event("New Hostgroup named " +
    ...
    def edit(self, hostgroup_id, hostgroup_name, hostgroup_alias, timestamp, is_deleted, updated_by, is_default,
             nms_instance):
        """
        ...
        try:
            ...
            if hostgroup_count > 0:
                ...
                for hg in hostgroups:
                    el_obj.log_event("Hostgroup %s Deleted." %
    ...
    def validate(self, hostgroup_name, hostgroup_alias):
        ...
        @return:
        """
        if Validation.no_space(hostgroup_name) and Validation.is_alpha_numeric(
                hostgroup_name) and Validation.is_required(hostgroup_name) and Validation.is_required(
                hostgroup_alias) and Validation.is_alpha_numeric(hostgroup_alias):
            return True
        else:
            return False
    ...
    def is_duplicate_hostgroup_for_add(self, hostgroup_name, hostgroup_alias):
        ...
        try:
            ...
            if hostgroup_name_count == 0:
                hostgroup_alias_count = session.query(Hostgroups).filter(
                    Hostgroups.hostgroup_alias == hostgroup_alias).count()
...
class DiscoveryBll(object):
    """
    Device discovery data from SQL
    """
    ...
    def ping_discovery(self, base_ip, base_ip_start, base_ip_end, timeout, service_mngt, timestamp, created_by,
        ...
        try:
            ...
            if self.ping_validate(base_ip, base_ip_start, base_ip_end, timeout, service_mngt):
                discovery = Discovery(DiscoveryName.ping, (base_ip + "." + base_ip_start), (
                    base_ip + "." + base_ip_end), timeout, None, None, None, None, None, service_mngt, 0, timestamp,
                                      created_by, creation_time, updated_by, is_deleted)
                session.add(discovery)
                session.commit()
                el_obj = EventLog()
                el_obj.log_event("Ping Discovery Initiated.", updated_by)
    ...
    def ping_validate(self, base_ip, base_ip_start, base_ip_end, timeout, service_mngt):
        ...
        @return:
        """
        if Validation.is_valid_ip(base_ip + "." + base_ip_start) and Validation.is_valid_ip(
                                base_ip + "." + base_ip_end) and Validation.is_required(
                base_ip) and Validation.is_required(base_ip_end) and Validation.is_required(
                base_ip_start) and Validation.is_required(timeout) and Validation.is_required(
                service_mngt) and Validation.is_number(base_ip_start) and Validation.is_number(
                base_ip_end) and Validation.is_number(timeout):
            return True
        else:
            return False
    ...
    def snmp_discovery(self, base_ip, base_ip_start, base_ip_end, timeout, snmp_community, snmp_port, snmp_version,
                       service_mngt, timestamp, created_by, creation_time, updated_by, is_deleted):
        """
        ...
        try:
            session = Session()                 # creating new session object
            if self.snmp_validate(base_ip, base_ip_start, base_ip_end, timeout, snmp_community, snmp_port, snmp_version,
                                  service_mngt):
                ...
                discovery = Discovery(DiscoveryName.snmp, (base_ip + "." + base_ip_start), (
                    base_ip + "." + base_ip_end), timeout, snmp_community, snmp_port, snmp_version, None, None,
                                      service_mngt, 0, timestamp, created_by, creation_time, updated_by, is_deleted)
                session.add(discovery)
                session.commit()
                el_obj = EventLog()
                el_obj.log_event("SNMP Discovery Initiated.", updated_by)
                return discovery.discovery_id
            else:
                err = ErrorMessages()
                err.error_msg = err.validation_error
    ...
    def snmp_validate(self, base_ip, base_ip_start, base_ip_end, timeout, snmp_community, snmp_port, snmp_version,
                      service_mngt):
        """
        ...
        if Validation.is_valid_ip(base_ip + "." + base_ip_start) and Validation.is_valid_ip(
                ...
                base_ip_start) and Validation.is_required(timeout) and Validation.is_required(
                service_mngt) and Validation.is_number(base_ip_start) and Validation.is_number(
                base_ip_end) and Validation.is_number(timeout) and Validation.is_required(
                snmp_port) and Validation.is_number(snmp_port) and Validation.is_required(
                snmp_community) and Validation.is_required(snmp_version) and Validation.is_alpha_numeric(
                snmp_community) and Validation.is_alpha_numeric(snmp_version):
            return True
        else:
            return False
    ...
    def upnp_discovery(self, timeout, service_mngt, timestamp, created_by, creation_time, updated_by, is_deleted):
        ...
        try:
            ...
            if self.upnp_validate(timeout, service_mngt):
                ...
                session.commit()
                el_obj = EventLog()
                el_obj.log_event("UNMP Discovery Initiated.", updated_by)
                return discovery.discovery_id
            else:
                err = ErrorMessages()
                err.error_msg = err.validation_error
    ...
    def run_snmp_discovery(self, discovery_id):
        ...
        try:
            session = Session()                 # creating new session object
            discovery = session.query(Discovery.ip_start_range, Discovery.ip_end_range, Discovery.timeout,
                                      Discovery.snmp_community, Discovery.snmp_port, Discovery.snmp_version).filter(
                Discovery.discovery_id == discovery_id).all()          # execute query and fetch data
            if len(discovery) == 1:
                dis = discovery[0]
                start_range = dis.ip_start_range
...
class HostBll(object):
    ...
    def grid_view_active_host(self, i_display_start, i_display_length, s_search, sEcho, sSortDir_0, iSortCol_0,
                              req_vars):
    ...
    def grid_view_disable_host(self, i_display_start, i_display_length, s_search, sEcho, sSortDir_0, iSortCol_0,
                               req_vars):
        # Session = sessionmaker(bind=engine)     # making session of our
        # current database
        """
    ...
    def grid_view_deleted_host(self, i_display_start, i_display_length, s_search, sEcho, sSortDir_0, iSortCol_0,
                               req_vars):
        # Session = sessionmaker(bind=engine)     # making session of our
        # current database
        """
    ...
    def grid_view_discovered_host(self, i_display_start, i_display_length, s_search, sEcho, sSortDir_0, iSortCol_0,
                                  req_vars):
        """
    ...
    def grid_view_tcp_discovered_host(self, i_display_start, i_display_length, s_search, sEcho, sSortDir_0, iSortCol_0,
                                      req_vars):
        """
    ...
    def grid_view_active_host_report(self):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(
                Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias, Hosts.ip_address,
                Hosts.mac_address, Hosts.created_by, Hosts.creation_time, Hosts.host_state_id,
                DeviceType.device_name).filter(and_(Hosts.is_deleted == 0, Hosts.host_state_id == HostState.enable,
                                                    Hosts.device_type_id == DeviceType.device_type_id,
                                                    UsersGroups.user_id == '%s' % (self.userid),
                                                    UsersGroups.group_id == HostgroupsGroups.group_id,
                                                    HostsHostgroups.hostgroup_id == HostgroupsGroups.hostgroup_id,
                                                    Hosts.host_id == HostsHostgroups.host_id)).order_by(
                Hosts.host_name).all()          # execute query and fetch data
            return hosts
        except Exception, e:
            return e
        finally:
    ...
    def grid_view_disable_host_report(self):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias,
                                  Hosts.ip_address, Hosts.mac_address, Hosts.updated_by, Hosts.timestamp,
                                  Hosts.host_state_id, DeviceType.device_name).filter(
                and_(Hosts.is_deleted == 0, Hosts.host_state_id == HostState.disable,
                     Hosts.device_type_id == DeviceType.device_type_id, UsersGroups.user_id == '%s' % (self.userid),
                     UsersGroups.group_id == HostgroupsGroups.group_id,
                     HostsHostgroups.hostgroup_id == HostgroupsGroups.hostgroup_id,
                     Hosts.host_id == HostsHostgroups.host_id)).order_by(
                Hosts.host_name).all()          # execute query and fetch data
    ...
    def grid_view_deleted_host_report(self):
        ...
        try:
            ...
            hosts = session.query(
                Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias, Hosts.ip_address,
                Hosts.mac_address, Hosts.host_state_id, DeviceType.device_name, Hosts.timestamp,
                Hosts.updated_by).filter(and_(Hosts.is_deleted == 1, Hosts.device_type_id == DeviceType.device_type_id,
                                              UsersGroups.user_id == '%s' % (self.userid),
                                              UsersGroups.group_id == HostgroupsGroups.group_id,
                                              HostsHostgroups.hostgroup_id == HostgroupsGroups.hostgroup_id,
                                              Hosts.host_id == HostsHostgroups.host_id)).order_by(
    ...
    def grid_view_discovered_host_report(self):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(
                DiscoveredHosts.discovered_host_id, DiscoveredHosts.ip_address, DiscoveredHosts.host_alias,
                DiscoveredHosts.mac_address, Discovery.timestamp, Discovery.discovery_type_id,
                DeviceType.device_name).filter(and_(~DiscoveredHosts.ip_address.in_(session.query(Hosts.ip_address)),
                                                    Discovery.discovery_id == DiscoveredHosts.discovery_id,
                                                    DiscoveredHosts.device_type_id == DeviceType.device_type_id))          # execute query and fetch data
            return hosts
        except Exception, e:
            return e
        finally:
    ...
    def grid_view_tcp_discovered_host_report(self):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(TcpDiscovery.ne_id, TcpDiscovery.site_mac, TcpDiscovery.ip_address,
                                  TcpDiscovery.timestamp, TcpDiscovery.product_id).filter(~TcpDiscovery.ip_address.in_(
    ...
    def get_host_by_id(self, host_id):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(
                Hosts.host_id, Hosts.host_name, Hosts.host_alias, Hosts.ip_address, Hosts.phone_no, Hosts.mac_address,
                Hosts.device_type_id, Hosts.config_profile_id,
                ...
                Hosts.snmp_version_id, Hosts.snmp_port, Hosts.snmp_trap_port, Hosts.lock_status, HostAssets.longitude,
                HostAssets.latitude, HostAssets.serial_number, HostAssets.hardware_version, HostVendor.vendor_name,
                HostOs.os_name, DeviceType.device_name, Hosts.firmware_mapping_id, Hosts.device_path, Hosts.number_of_ports).filter(
                and_(Hosts.host_id == host_id, Hosts.host_asset_id == HostAssets.host_asset_id,
                     Hosts.host_vendor_id == HostVendor.host_vendor_id, Hosts.host_os_id == HostOs.host_os_id,
                     Hosts.device_type_id == DeviceType.device_type_id)).all()          # execute query and fetch data
            ...
            if len(hosts) == 1:
                ...
                if UNMPDeviceType.odu16 == host.device_type_id:
                    ...
                    node_type_table_obj = session.query(
                        GetOdu16_ru_conf_table.default_node_type).filter(
                        GetOdu16_ru_conf_table.host_id == host_id).all()
                    ...
                    vlan_table_obj = session.query(Odu100IpConfigTable.managementMode,
                                                   Odu100IpConfigTable.managementVlanTag).filter(
                    ...
                    ccu_network_table_obj = session.query(
                        CcuNetworkConfigurationTable.ccuNCDHCPNetMask).filter(
                        CcuNetworkConfigurationTable.host_id == host_id).all()
                ...
                host_details_dic = {
                    ...
                    "phone_no": host.phone_no != None and host.phone_no or "",
                    ...
                    "device_type": host.device_type_id != None and host.device_type_id or "",
                    "device_path": host.device_path!=None and host.device_path or "",
                    "number_of_ports": host.number_of_ports!=None and host.number_of_ports or ""
                }
                # get service parameters
                hu = HostUtils()
                sp_list = hu.get_host_service_parameters(host_id)
                hsd_list = hu.get_host_service_details(host_id)
                hu.close_database_connection()
                sp_dict = {}
                for sp in sp_list:
                    sp_dict["service_parameter_" + str(sp[0])] = sp[1]
                host_details_dic.update(sp_dict)
                if hsd_list:
                    hsd_list = hsd_list[0]
                    hsd_dict = {
                        "service_username": hsd_list[0],
                        "service_password": hsd_list[1],
                        "service_user_id": hsd_list[2]
                    }
                    host_details_dic.update(hsd_dict)
    ...
    def add(self, host_name, host_alias, ip_address, phone_no, mac_address, device_type_id, netmask, gateway, primary_dns,
            secondary_dns, dns_state, odu100_management_mode, odu100_vlan_tag, idu4_management_mode, idu4_vlan_tag,
            idu4_tdm_ip, ccu_dhcp_netmask, timestamp, created_by, creation_time, is_deleted, updated_by, ne_id, site_id,
            host_state_id, priority_id, host_vendor_id, host_os_id, http_username, http_password, http_port,
            ssh_username, ssh_password, ssh_port, snmp_read_community, snmp_write_community, snmp_port, snmp_trap_port,
            snmp_version_id, comment, nms_instance, parent_name, lock_status, is_localhost, longitude, latitude,
            serial_number, hardware_version, hostgroup, ra_mac="", node_type="", master_id="",
            firmware_mapping_id="7.2.20", is_reconciliation=True, hsp_list=[], hsd_dict = {}, device_path=None, number_of_ports=None):
        """
        ...
        @param secondary_dns:
        @param dns_state:
        @param odu100_management_mode:
        @param odu100_vlan_tag:
        @param idu4_management_mode:
        @param idu4_vlan_tag:
        @param idu4_tdm_ip:
        @param ccu_dhcp_netmask:
        @param timestamp:
        @param created_by:
        @param creation_time:
        @param is_deleted:
        ...
        try:
            ...
            # if True:
                if self.validate(host_name, host_alias, ip_address, phone_no, mac_address, device_type_id, netmask, gateway,
                                 primary_dns, secondary_dns, odu100_management_mode, odu100_vlan_tag,
                                 idu4_management_mode, idu4_vlan_tag, idu4_tdm_ip, ccu_dhcp_netmask, host_state_id,
                                 priority_id, host_vendor_id, host_os_id, http_username, http_password, http_port,
                                 ssh_username, ssh_password, ssh_port, snmp_read_community, snmp_write_community,
                                 snmp_port, snmp_trap_port, comment, longitude, latitude, serial_number,
                                 hardware_version):
                    if self.is_duplicate_host_for_add(host_name, host_alias, ip_address, mac_address):
                        ...
                        reconcile_status = 0
                        host = Hosts(
                            host_name, host_alias, ip_address, phone_no, mac_address, device_type_id, netmask, gateway,
                            primary_dns, secondary_dns, dns_state, config_profile_id, timestamp, created_by,
                            creation_time, is_deleted, updated_by, ne_id, site_id, host_state_id, priority_id,
                            host_vendor_id, host_os_id, host_asset_id, http_username, http_password, http_port,
                            snmp_read_community,
                            snmp_write_community, snmp_port, snmp_trap_port, snmp_version_id, comment, nms_id,
                            parent_name, lock_status, is_localhost, reconcile_health, reconcile_status, ssh_username,
                            ssh_password, ssh_port, firmware_mapping_id, device_path, number_of_ports)
                        session.add(host)
                        session.flush()
                        ...
                        for srv in services_list:
                            ...
                            if (flag_nagios_call == 1):
                                wnc = n_bll.add_new_service(
                                    ...
                                    srv.max_check_attempts, srv.normal_check_interval, srv.retry_check_interval,
                                    srv.check_command)
                                condition_nagios = wnc["success"] == 0
                        ...
                        if device_type_id == "odu100" and type(config_profile_id) == dict:
                            ...
                            if len(host_data) > 0 and config_profile_id != None and type(
                                    config_profile_id) == tuple and len(config_profile_id) == 2:
                                host_data[
                            ...
                            if len(vlan_table_obj) > 0:
                                ...
                                if ccu_dhcp_netmask != None:
                                    vlan_table = CcuNetworkConfigurationTable(
                                        host.host_id, 1, mac_address, ip_address, netmask, "", "", ccu_dhcp_netmask,
                                        gateway)
                                    session.add(vlan_table)
                                    # end - manage vlan
                        ...
                        # manage service parameter
                        if hsp_list and hsd_dict:
                            hu = HostUtils()
                            hu.add_update_host_service_parameters(host_id, hsp_list)
                            hu.add_update_host_service_details(host_id, hsd_dict)
                            hu.close_database_connection()
                        ...
                        if condition_nagios:
                            ...
                            el_obj = EventLog()
                            el_obj.log_event(
                                "Add New Device '%s'." % (ip_address), updated_by)
                            return host.host_id
                        else:
                            err = ErrorMessages()
                            err.error_msg = err.nagios_config_error
    ...
    def edit(self, host_id, host_name, host_alias, ip_address, phone_no, mac_address, device_type_id, netmask, gateway,
             primary_dns, secondary_dns, dns_state, odu100_management_mode, odu100_vlan_tag, idu4_management_mode,
             idu4_vlan_tag, idu4_tdm_ip, ccu_dhcp_netmask, is_deleted, updated_by, host_state_id, priority_id,
             host_vendor_id, host_os_id, http_username, http_password, http_port, ssh_username, ssh_password, ssh_port,
             snmp_read_community, snmp_write_community, snmp_port, snmp_trap_port, snmp_version_id, comment,
             nms_instance, parent_name, lock_status, longitude, latitude, serial_number, hardware_version, hostgroup,
             ra_mac="", node_type="", master_id="", firmware_mapping_id="7.2.20", hsp_list = [], hsd_dict = {}, device_path=None, number_of_ports=None):
        """
        ...
        try:
            ...
            cursor = db.cursor()
            if self.validate(host_name, host_alias, ip_address, phone_no, mac_address, device_type_id, netmask, gateway,
                             primary_dns, secondary_dns, odu100_management_mode, odu100_vlan_tag, idu4_management_mode,
                             idu4_vlan_tag, idu4_tdm_ip, ccu_dhcp_netmask, host_state_id, priority_id, host_vendor_id,
                             host_os_id, http_username, http_password, http_port, ssh_username, ssh_password, ssh_port,
                             snmp_read_community, snmp_write_community, snmp_port, snmp_trap_port, comment, longitude,
                             latitude, serial_number, hardware_version):
                if self.is_duplicate_host_for_edit(host_id, host_name, host_alias, ip_address, mac_address):
                    ...
                    if len(hosts) == 1:
                        ...
                        if host.device_type_id != device_type_id:
                            ...
                            if host.host_id != None:
                                ...
                                if host.device_type_id == UNMPDeviceType.odu16:
                                    ...
                                    cursor.execute(
                                        "delete from get_odu16_peer_node_status_table where host_id = %s" % host.host_id)
                                    cursor.execute(
                                        "delete from get_odu16_ra_tdd_mac_statistics_entry where host_id = %s" % host.host_id)
                                    cursor.execute(
                                        "delete from get_odu16_synch_statistics_table where host_id = %s" % host.host_id)
                                    # odu 16 analyze table will empty
                                    cursor.execute(
                                        "delete from analyze_get_odu16_nw_interface_statistics_table where host_id = %s" % host.host_id)
                                    cursor.execute(
                                ...
                                if host.device_type_id == UNMPDeviceType.odu100:
                                    ...
                                    cursor.execute(
                                        "delete from odu100_peerNodeStatusTable where host_id = %s" % host.host_id)
                                    ...
                                    cursor.execute(
                                        "delete from analyze_odu100_nwInterfaceStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute(
                                        "delete from analyze_odu100_peerNodeStatusTable where host_id = %s" % host.host_id)
                                    cursor.execute(
                                        "delete from analyze_odu100_raTddMacStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute(
                                        "delete from analyze_odu100_synchStatisticsTable where host_id = %s" % host.host_id)

                                if host.device_type_id == UNMPDeviceType.idu4:
                                    # idu 4 get table will empty
                                    cursor.execute(
                                        "delete from idu_tdmoipNetworkInterfaceStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute(
                                        "delete from idu_iduNetworkStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute(
                                    ...
                                    cursor.execute(
                                        "delete from idu_portSecondaryStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from idu_portstatisticsTable where host_id = %s" %
                                                   host.host_id)
                                    cursor.execute(
                                        "delete from idu_swPrimaryPortStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute(
                                ...
                                if device_type_id == UNMPDeviceType.ap25 or device_type_id == UNMPDeviceType.idu4 or device_type_id == UNMPDeviceType.generic or device_type_id == UNMPDeviceType.swt4:
                                    cursor.execute(
                                        "delete from master_slave_linking where master = %s OR slave = %s" % (
                            ...
                            if device_type_id == "odu100":
                                ...
                                if config_profile_id != None and type(config_profile_id) == tuple and len(
                                        config_profile_id) == 2:
                        ...
                        if device_type_id == "odu100":
                            ...
                            if len(result) > 0 and result[0][0] != None and result[0][
                                0] != "" and firmware_mapping_id != result[0][0]:
                        ...
                        if len(hosts) == 1:
                            ...
                            host.device_path = device_path
                            host.number_of_ports = number_of_ports
                        ...
                        if UNMPDeviceType.odu16 == device_type_id:
                            ...
                            if len(vlan_table_obj) > 0:
                                ...
                                if ccu_dhcp_netmask != None:
                                    vlan_table = CcuNetworkConfigurationTable(
                                        host.host_id, 0, mac_address, ip_address, netmask, "", "", ccu_dhcp_netmask,
                                        gateway)
                                    session.add(vlan_table)
                                # end - manage vlan
                            session.commit()
                        ...
                        if (condition_nagios):
                            ...
                            el_obj = EventLog()
                            el_obj.log_event(
                                "Edit Device '%s' Details." % (ip_address), updated_by)
        ...
        try:
            ...
            for host_i in range(0, len(host_ids)):
                ...
                if len(deleted_host) > 0:
                    ...
                    if self.is_duplicate_host_for_add(d_host.host_name, d_host.host_alias, d_host.ip_address,
                                                      d_host.mac_address):
                        hosts = session.query(
                            Hosts).filter(Hosts.host_id == host_id).all()
                        if len(hosts) == 1:
        ...
        try:
            ...
            if localhost_count > 0:
                ...
                if host_count > 0:
                    el_obj = EventLog()
                    for hs in hosts:
                        el_obj.log_event("Host '%s' Deleted [You can view it in deleted host]." %
                    ...
                    for hst in hosts:
                        ...
                        if hst.host_id != None:
                            ...
                            if hst.device_type_id == UNMPDeviceType.odu16:
                                ...
                                cursor.execute(
                                    "delete from get_odu16_ra_tdd_mac_statistics_entry where host_id = %s" % hst.host_id)
                                cursor.execute(
                                ...
                                cursor.execute(
                                    "delete from analyze_get_odu16_peer_node_status_table where host_id = %s" % hst.host_id)
                                cursor.execute(
                                    "delete from analyze_get_odu16_ra_tdd_mac_statistics_entry where host_id = %s" % hst.host_id)
                                cursor.execute(
                                    "delete from analyze_get_odu16_synch_statistics_table where host_id = %s" % hst.host_id)
                            ...
                            if hst.device_type_id == UNMPDeviceType.odu100:
                                ...
                                cursor.execute(
                                    "delete from analyze_odu100_peerNodeStatusTable where host_id = %s" % hst.host_id)
                                cursor.execute(
                            ...
                            if hst.device_type_id == UNMPDeviceType.idu4:
                                ...
                                # idu 4 analyze table will empty
                                cursor.execute(
                                    "delete from analyze_idu_portSecondaryStatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute(
                                    "delete from analyze_idu_portstatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute(
                                    "delete from analyze_idu_swPrimaryPortStatisticsTable where host_id = %s" % hst.host_id)
                        ...
                        cursor.close()
                        el_obj.log_event(
                            "Device '%s' Deleted." % (hst.ip_address), updated_by)
                    db.commit()
                    db.close()
                    # ncbll = NagioConfigurationBll()
                    # wnc = ncbll.write_nagios_config(nms_instance)
    ...
    def validate(self, host_name, host_alias, ip_address, phone_no, mac_address, device_type_id, netmask, gateway, primary_dns,
                 secondary_dns, odu100_management_mode, odu100_vlan_tag, idu4_management_mode, idu4_vlan_tag,
                 idu4_tdm_ip, ccu_dhcp_netmask, host_state_id, priority_id, host_vendor_id, host_os_id, http_username,
                 http_password, http_port, ssh_username, ssh_password, ssh_port, snmp_read_community,
                 snmp_write_community, snmp_port, snmp_trap_port, comment, longitude, latitude, serial_number,
                 hardware_version):
        ...
        @return:
        """
        if Validation.is_alpha_numeric(host_name) and Validation.no_space(host_name) and Validation.is_required(
                host_alias) and Validation.is_alpha_numeric(host_alias) and \
                (Validation.is_required(ip_address) or Validation.is_required(phone_no)) and Validation.is_valid_ip(ip_address) and Validation.is_required(
                mac_address) and Validation.is_valid_mac(mac_address) and \
                Validation.is_required(device_type_id) and Validation.is_required(netmask) and Validation.is_required(
                gateway) and Validation.is_valid_netmask(netmask) and Validation.is_valid_ip(gateway) and \
                Validation.is_valid_ip(primary_dns) and Validation.is_valid_ip(
                secondary_dns) and Validation.is_required(host_state_id) and \
                Validation.is_required(priority_id) and Validation.is_alpha_numeric(
                http_username) and Validation.is_alpha_numeric(ssh_username) and Validation.is_required(
                host_vendor_id) and Validation.is_required(host_os_id) and \
                Validation.is_number(http_port) and Validation.is_number(ssh_port) and Validation.is_alpha_numeric(
                snmp_read_community) and Validation.is_alpha_numeric(snmp_write_community) and \
                Validation.is_number(snmp_port) and Validation.is_number(
                snmp_trap_port) and Validation.is_alpha_numeric(comment) and \
                Validation.is_number(longitude) and Validation.is_required(longitude) and Validation.is_number(
                latitude) and Validation.is_required(latitude) and Validation.is_alpha_numeric(
                serial_number) and Validation.is_alpha_numeric(hardware_version):
            if UNMPDeviceType.ap25 == device_type_id:
                ...
                if Validation.is_required(odu100_management_mode) and Validation.is_integer(odu100_management_mode):
                    if int(odu100_management_mode) == 1:
                        if Validation.is_integer(odu100_vlan_tag) and Validation.is_required(odu100_vlan_tag) and int(
                                odu100_vlan_tag) <= 4094 and int(odu100_vlan_tag) > 0:
                            return True
                        else:
                            return False
                    else:
                ...
                if Validation.is_required(idu4_management_mode) and Validation.is_integer(idu4_management_mode):
                    if int(idu4_management_mode) == 1:
                        if Validation.is_required(idu4_vlan_tag) and Validation.is_integer(
                                idu4_vlan_tag) and Validation.is_required(idu4_tdm_ip) and Validation.is_valid_ip(
                                idu4_tdm_ip) and int(idu4_vlan_tag) <= 4094 and int(idu4_vlan_tag) > 0:
                            return True
                        else:
                            return False
                    else:
    ...
    def is_duplicate_host_for_add(self, host_name, host_alias, ip_address, mac_address):
        ...
        try:
            ...
            host_count = session.query(Hosts).filter(and_(
                or_(Hosts.host_alias == host_alias, Hosts.ip_address == ip_address, Hosts.mac_address == mac_address),
                Hosts.is_deleted == 0)).count()
            if host_count == 0:
                return True
            else:
                return False
    ...
    def is_duplicate_host_for_edit(self, host_id, host_name, host_alias, ip_address, mac_address):
        ...
        try:
            ...
            host_count = session.query(Hosts).filter(and_(Hosts.host_id != host_id, or_(
                Hosts.host_name == host_name, Hosts.host_alias == host_alias, Hosts.ip_address == ip_address,
                Hosts.mac_address == mac_address), Hosts.is_deleted == 0)).count()
            if host_count == 0:
                return True
            else:
                return False
    ...
    def host_parents(self, host_id=None):
        ...
        try:
            ...
            hosts = session.query(
                Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias, Hosts.ip_address,
                Hosts.mac_address).filter(and_(Hosts.is_deleted == 0,
                                               Hosts.host_id != host_id)).order_by(
                Hosts.host_alias).all()          # execute query and fetch data
            return hosts
        except Exception, e:
            return e
        finally:
...
WHERE hosts.is_deleted = '0' AND hosts.host_id IS NOT NULL AND users_groups.user_id = '%s' AND users_groups.group_id = hostgroups_groups.group_id AND hosts_hostgroups.hostgroup_id = hostgroups_groups.hostgroup_id AND hosts.host_id = hosts_hostgroups.host_id OR hosts.ip_address = 'localhost' ORDER BY hosts.host_name" % (
            user_id)
            cursor.execute(sql)
    ...
    def host_hostgroups(self):
        ...
        try:
            session = Session()                 # creating new session object
            hostgroups = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name,
                                       Hostgroups.hostgroup_alias).filter(
                Hostgroups.is_deleted == 0).order_by(
                Hostgroups.hostgroup_alias).all()          # execute query and fetch data
            return hostgroups
        except Exception, e:
            return e
        finally:
    ...
    def host_hostgroups2(self, userid):
        ...
        try:
            ...
            hostgroups = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name,
                                       Hostgroups.hostgroup_alias).filter(
                and_(Hostgroups.is_deleted == 0, UsersGroups.user_id == '%s' % (userid),
                     UsersGroups.group_id == HostgroupsGroups.group_id,
                     HostgroupsGroups.hostgroup_id == Hostgroups.hostgroup_id)).order_by(
                Hostgroups.hostgroup_alias).all()          # execute query and fetch data
            return hostgroups
        except Exception, e:
            return e
    ...
    def host_vendors(self):
        ...
        try:
            ...
            vendors = session.query(HostVendor.host_vendor_id, HostVendor.vendor_name).filter(
                HostVendor.is_deleted == 0).order_by(
                HostVendor.vendor_name).all()          # execute query and fetch data
            return vendors
        except Exception, e:
            return e
        finally:
                ...
                if host_details[0].config_profile_id == "" or host_details[0].config_profile_id == None:
                    ...
                    if device_dict["device_type_id"] == UNMPDeviceType.odu16:
                        ...
                        change_admin_state = pysnmp_set(
                            admin_dic, host_details[0].ip_address, host_details[0].snmp_port,
                            host_details[0].snmp_write_community)
                    ...
                    if device_dict["device_type_id"] == UNMPDeviceType.idu4:
                        ...
                        change_net_config = pysnmp_set(
                            dic_odu, host_details[0].ip_address, host_details[0].snmp_port,
                            host_details[0].snmp_write_community)
                    ...
                    if device_dict["device_type_id"] == UNMPDeviceType.ap25:
                        ...
                        change_net_config = pysnmp_set(
                            dic_ap, host_details[0].ip_address, host_details[0].snmp_port,
                            host_details[0].snmp_write_community)
                    for i in range(0, 3):
                        if device_dict["device_type_id"] == UNMPDeviceType.ccu:
                        ...
                        if snmp_ping_ccu(str(device_dict["ip_address"]), str(host_details[0].snmp_read_community),
                                         int(host_details[0].snmp_port)) == 0:
                    ...
                    if chk == 1:
                        if device_dict["device_type_id"] == UNMPDeviceType.odu16:
                            ...
                            if len(odu16_ip_details) > 0:
                                ...
                                odu16_detail = SetOdu16IPConfigTable(None, host_details[0].config_profile_id, 1,
                                                                     device_dict[
                                                                         "ip_address"], device_dict["ip_network_mask"],
                                                                     device_dict["ip_gateway"], device_dict["dhcp"])
                                sqlalche_obj.add(odu16_detail)
                            result = {"success": 0}
                        elif device_dict["device_type_id"] == UNMPDeviceType.odu100:

                            odu16_ip_details = sqlalche_obj.query(
                                Odu100IpConfigTable).filter(
                                Odu100IpConfigTable.config_profile_id == host_details[0].config_profile_id).all()
                            if len(odu16_ip_details) > 0:
                                odu16_ip_details[
                                ...
                                odu100_detail = Odu100IpConfigTable(None, host_details[0].config_profile_id, 0, 1,
                                                                    device_dict[
                                                                        "ip_address"], device_dict["ip_network_mask"],
                                                                    device_dict["ip_gateway"], device_dict["dhcp"])
                            ...
                            odu16_ip_details = sqlalche_obj.query(IduNetworkConfigurationsTable).filter(
                                IduNetworkConfigurationsTable.config_profile_id == host_details[
                            ...
                            if len(odu16_ip_details) > 0:
                                ...
                                idu_detail = IduNetworkConfigurationsTable(None, host_details[0].config_profile_id, 0,
                                                                           1, device_dict[
                                        "ip_address"], device_dict["ip_network_mask"], device_dict["ip_gateway"],
                                                                           device_dict["dhcp"])
                            ...
                            if len(ap25_details) > 0:
                                ...
                                ap25_detail = Ap25AccesspointIPsettings(host_details[0].config_profile_id,
                                                                        device_dict["ip_address"], device_dict[
                                        "ip_network_mask"], device_dict["ip_gateway"], device_dict["primarydns"],
                                                                        device_dict["secondarydns"])
        ...
        try:
            ...
            if device_type_id == UNMPDeviceType.odu16:
                device = session.query(
                    Hosts.ip_address, Hosts.mac_address, Hosts.host_id).filter(
                    and_(Hosts.is_deleted == 0, GetOdu16_ru_conf_table.host_id == Hosts.host_id,
                         or_(GetOdu16_ru_conf_table.default_node_type == 0,
                             GetOdu16_ru_conf_table.default_node_type == 2),
                         Hosts.device_type_id == device_type_id)).all()          # execute query and fetch data
            elif device_type_id == UNMPDeviceType.odu100:
                device = session.query(
                    Hosts.ip_address, Hosts.mac_address, Hosts.host_id).filter(
                    and_(Hosts.is_deleted == 0, Odu100RuConfTable.config_profile_id == Hosts.config_profile_id,
                         or_(Odu100RuConfTable.defaultNodeType == 0,
                             Odu100RuConfTable.defaultNodeType == 2),
                         Hosts.device_type_id == device_type_id)).all()          # execute query and fetch data
            return device
        except Exception, e:
            return e
    ...
    def is_duplicate_host_with_ip_address(self, ip_address):
        ...
        try:
            ...
            host = session.query(Hosts.host_id).filter(
                and_(Hosts.is_deleted == 0,
                     Hosts.ip_address == ip_address)).count()          # execute query and fetch data
            if host == 0:
                return True
            else:
                return False
    ...
    def is_duplicate_host_with_host_alias(self, host_alias):
        ...
        try:
            ...
            host = session.query(Hosts.host_id).filter(
                and_(Hosts.is_deleted == 0,
                     Hosts.host_alias == host_alias)).count()          # execute query and fetch data
            if host == 0:
                return True
            else:
                return False
    ...
    def is_duplicate_host_with_mac_address(self, mac_address):
        ...
        try:
            ...
            host = session.query(Hosts.host_id).filter(
                and_(Hosts.is_deleted == 0,
                     Hosts.mac_address == mac_address)).count()          # execute query and fetch data
            if host == 0:
                return True
            else:
                return False
...
class VendorBll(object):
    """
    Unused functionality to add and delte device vendor
    """
    def grid_view(self):
        """
    ...
    def delete(self, vendor_ids, updated_by):
        ...
        try:
            ...
            if vendor_count > 0:
                el_obj = EventLog()
                for vd in vendors:
                    el_obj.log_event("Vendor '%s' Deleted." %
    ...
    def validate(self, vendor_name, description):
        ...
        @return:
        """
        if Validation.is_alpha_numeric(vendor_name) and Validation.is_required(vendor_name) and Validation.is_required(
                description) and Validation.is_alpha_numeric(description):
            return True
        else:
            return False
...
class BlackListMacBll(object):
    """
    Unused class to blacklist some MAC from UNMP inventory
    """
    def grid_view(self):
        """
        ...
        try:
            session = Session()                 # creating new session object
            black_list_macs = session.query(BlackListMacs.black_list_mac_id, BlackListMacs.mac_address,
                                            BlackListMacs.description).filter(
                BlackListMacs.is_deleted == 0).order_by(
                BlackListMacs.mac_address).all()          # execute query and fetch data
        ...
        try:
            ...
            if self.validate(mac_address, description):
                if self.is_duplicate_black_list_mac_for_add(mac_address):
                    ...
                    el_obj = EventLog()
                    el_obj.log_event(
                        "Add New Black Listed Device '%s'." % (mac_address), updated_by)
            ...
            if self.validate(mac_address, description):
                if self.is_duplicate_black_list_mac_for_edit(black_list_mac_id, mac_address):
                    ...
                    if len(black_list_macs) == 1:
                        ...
                        el_obj = EventLog()
                        el_obj.log_event(
                            "Black Listed Device '%s' Updated." % (mac_address), updated_by)
                        return black_list_mac.black_list_mac_id
                    else:
                        err = ErrorMessages()
                        err.error_msg = err.no_record_error
        ...
        try:
            ...
            if black_list_mac_count > 0:
                ...
                for vd in black_list_macs:
                    el_obj.log_event(
                        "Black Listed Device '%s' Deleted." % (mac_address), updated_by)
                    session.delete(vd)
                session.commit()
    ...
    def validate(self, mac_address, description):
        ...
        @return:
        """
        if Validation.is_valid_mac(mac_address) and Validation.is_required(mac_address) and Validation.is_required(
                description) and Validation.is_alpha_numeric(description):
            return True
        else:
            return False
    ...
    def is_duplicate_black_list_mac_for_edit(self, black_list_mac_id, mac_address):
        ...
        try:
            session = Session()                 # creating new session object
            black_list_mac_count = session.query(BlackListMacs).filter(
                and_(BlackListMacs.black_list_mac_id != black_list_mac_id,
                     BlackListMacs.mac_address == mac_address)).count()
            if black_list_mac_count == 0:
                return True
            else:
...
class ServiceBll(object):
    ...
    def grid_view_service(self, host_id):
        ...
        try:
            session = Session()                 # creating new session object
            services = session.query(HostServices.service_description, HostServices.normal_check_interval,
                                     HostServices.host_id).filter(
                HostServices.host_id == host_id).order_by(
                HostServices.service_description).all()          # execute query and fetch data
            return services
        except Exception, e:
            return e
        finally:
    ...
    def set_service_time(self, hosts_list, service_name, service_time, user_name):
        ...
        try:
            ...
            for i in range(len(hosts_list)):                 # creating new session object
                services = session.query(HostServices).filter(
                    and_(HostServices.host_id == hosts_list[i], HostServices.service_description == service_name[i]
            ...
            services = session.query(HostServices.service_description,
                                     ...
                                     Hosts.priority_id,
                                     Hosts.host_name).filter(
                and_(HostServices.is_deleted == 0, HostServices.host_id == Hosts.host_id, Hosts.is_deleted == 0)).all()
            s_config = ""
            ...
            SystemSetting.reload_nagios_config()
            # el_obj = EventLog()
            # el_obj.log_event("Service time modified for added in unmp system",created_by)
            # if isinstance(wnc, bool):
            # 	el_obj = EventLog()
            # 	el_obj.log_event("New Hostgroup named "+hostgroup_name+" added in unmp system",created_by)
            # else:
            #    err = ErrorMessages()
            #    err.error_msg = err.nagios_config_error
            # return err
...
class HostUtils(object):
    ...
    def get_active_service_parameters_by_host_id (self, host_id):
        sel_query = "SELECT service_parameter.service_parameter_id, parameter_name, parameter_index, parameter_type, parameter_unit, host_service_parameter.min_value, host_service_parameter.max_value, host_service_parameter.is_active, host_service_parameter.host_service_parameter_id FROM service_parameter INNER JOIN hosts ON hosts.device_type_id = service_parameter.device_type_id INNER JOIN host_service_parameter ON host_service_parameter.service_parameter_id = service_parameter.service_parameter_id AND hosts.host_id = host_service_parameter.host_id where service_parameter.is_active = 1 AND hosts.host_id = '" + host_id + "' ORDER BY  service_parameter.parameter_index ASC"
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple

    def get_service_parameters(self, device_type_id):
        sel_query = "select service_parameter_id, parameter_name, parameter_index, parameter_type, min_value, max_value, is_active, query_param from service_parameter where service_parameter.is_active = 1 AND device_type_id = '" + device_type_id + "' ORDER BY  parameter_index ASC"
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple

    def get_host_service_parameters(self, host_id):
        sel_query = "select service_parameter_id, is_active from host_service_parameter where host_id = '%s'" % str(host_id)
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple

    def delete_host_service_parameters(self, host_id):
        sel_query = "delete from host_service_parameter where host_id = '%s'" % str(host_id)
        self.cursor.execute(sel_query)
        self.db.commit()
        return True

    def add_update_host_service_parameters(self, host_id, sp_list):
        # delete existing host service parameters
        self.delete_host_service_parameters(host_id)
        for sp in sp_list:
            insert_query = "insert into host_service_parameter (`host_id`, `service_parameter_id`, `is_active`) values ('%s','%s','%s')" % (str(host_id), str(sp[0]), str(sp[1]))
            self.cursor.execute(insert_query)
        self.db.commit()
        return True

    def get_host_service_details(self, host_id):
        sel_query = "select username, password, user_id from host_service_details where host_id = '%s'" % str(host_id)
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple

    def add_update_host_service_details(self, host_id, hsd_list):
        hsd = self.get_host_service_details(host_id)
        if hsd:
            update_query = "update host_service_details SET username = '%s', password = '%s', user_id = '%s'  WHERE host_id = %s" % (str(hsd_list["service_username"]), str(hsd_list["service_password"]), str(hsd_list["service_user_id"]), str(host_id))
            self.cursor.execute(update_query)
            self.db.commit()
        else:
            insert_query = "insert into host_service_details (`host_id`, `username`, `password`, `user_id`) values ('%s','%s','%s', '%s')" % (str(host_id), str(hsd_list["service_username"]), str(hsd_list["service_password"]), str(hsd_list["service_user_id"]))
            self.cursor.execute(insert_query)
            self.db.commit()
        return True
    
    def get_latest_host_service_parameter_data(self, host_id):
        sel_query = "select service_data, timestamp from host_service_parameter_data where host_id = '%s' ORDER BY  timestamp DESC LIMIT 1" % str(host_id)
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple
        
    def get_host_service_parameter_data(self, host_id, time_interval):
        ...
        sel_query = "select service_data, timestamp from host_service_parameter_data where host_id = '%s' AND timestamp > '%s' AND timestamp <= '%s' ORDER BY  timestamp ASC" % (str(host_id), end_time, start_time)
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple
        
    def get_host_service_parameter_data_by_time(self, end_time, start_time):   # end_time < start_time
        sel_query = "select service_data, timestamp from host_service_parameter_data where timestamp > '%s' AND timestamp <= '%s' ORDER BY  timestamp DESC" % (end_time, start_time)
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple
        
    def get_alarm_by_time(self, serevity, end_time, start_time):
        if serevity:
            s_query = ""
            ...
            sel_query = "select description, trap_ip, event_id, trap_event_type, serevity, timestamp from trap_alarms where timestamp > '%s' AND timestamp <= '%s' %s ORDER BY  timestamp DESC" % (end_time, start_time, s_query)
        else:
            sel_query = "select description, trap_ip, event_id, trap_event_type, serevity, timestamp from trap_alarms where timestamp > '%s' AND timestamp <= '%s' ORDER BY  timestamp DESC" % (end_time, start_time)
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple
        
    def delete_alarm_by_time(self, serevity, end_time, start_time):
        if serevity:
            s_query = ""
            for i in range(len(serevity)):
                if i > 0:
            ...
            sel_query = "DELETE from trap_alarms where timestamp > '%s' AND timestamp <= '%s' %s ORDER BY  timestamp DESC" % (end_time, start_time, s_query)
        else:
            sel_query = "DELETE from trap_alarms where timestamp > '%s' AND timestamp <= '%s' ORDER BY  timestamp DESC" % (end_time, start_time)
        self.cursor.execute(sel_query)
        self.db.commit();
        return True
        
    def get_host_port_path_details(self, host_id):
        sel_query = "SELECT device_path, number_of_ports, http_username, http_password FROM hosts WHERE host_id = '" + host_id + "'"
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple
        
    def get_host_service_parameter_data_by_id(self, parameter_id):
        sel_query = "SELECT host_service_parameter.min_value, host_service_parameter.max_value, host_service_parameter.min_arm, host_service_parameter.max_arm, service_parameter.parameter_type FROM host_service_parameter INNER JOIN service_parameter ON host_service_parameter.service_parameter_id = service_parameter.service_parameter_id WHERE host_service_parameter_id = '%s'" % parameter_id
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple
        
    def update_host_service_parameter_data_by_id(self, parameter_id, min_value, max_value, min_arm, max_arm):
        if parameter_id:
            if None != min_value and "" != str(min_value).strip():
                min_value = "'%s'" % str(min_value)
            else:
                min_value = "null"
            
            if None != max_value and "" != str(max_value).strip():
                max_value = "'%s'" % str(max_value)
            else:
                max_value = "null"
                
            if None != min_arm and "" != str(min_arm).strip():
                min_arm = "'%s'" % str(min_arm)
            else:
                min_arm = "null"
                
            if None != max_arm and "" != str(max_arm).strip():
                max_arm = "'%s'" % str(max_arm)
            else:
                max_arm = "null"
                
            update_query = "update host_service_parameter SET min_value = %s, max_value = %s, min_arm = %s, max_arm = %s  WHERE host_service_parameter_id = %s" % (min_value, max_value, min_arm, max_arm, str(parameter_id))
            self.cursor.execute(update_query)
            self.db.commit()
            return update_query
        return False
         
    def update_relay_status(self, host_id, arm, new_status):
        sel_query = "SELECT service_status FROM host_service_details WHERE host_id = '%s'" % host_id
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        service_status = ""
        if result_tuple:
            result_tuple = result_tuple[0]
            service_status = result_tuple[0] and result_tuple[0] or service_status
            service_status = service_status.split(",")
            service_status_len = len(service_status)
            if service_status_len < arm:
                for s in range(service_status_len, arm):
                    service_status.append("NA")
            service_status[arm-1] = new_status
            service_status = ",".join(service_status)
        update_query = "UPDATE host_service_details SET service_status = '%s' WHERE host_id = '%s'" % (service_status, host_id)
        self.cursor.execute(update_query)
        self.db.commit()
        
    def get_host_details(self, host_id):
        sel_query = "SELECT device_type_id, ip_address, http_username, http_password, http_port FROM hosts WHERE host_id = '%s'" % host_id
        self.cursor.execute(sel_query)
        result_tuple = self.cursor.fetchall()
        return result_tuple
...
class NagioConfigurationBll(object):
    """
    Nagios configuration related database modal class
    """
    def write_nagios_config(self, nms_instance):
        ...
        try:
            ...
            hosts = session.query(Hosts.host_name, Hosts.host_alias, Hosts.ip_address, Hosts.priority_id,
                                  Hostgroups.hostgroup_name, (
                    Parents.host_name).label("parent")).outerjoin(Parents, Hosts.parent_name == Parents.host_id).filter(
                and_(Hosts.is_deleted == 0, HostsHostgroups.hostgroup_id == Hostgroups.hostgroup_id,
                     Hosts.host_id == HostsHostgroups.host_id)).all()          # execute query and fetch data
            ...
            for host in hosts:
                h_config += ng_cng.host_config(
                    use_template, host.host_name, host.host_alias, host.ip_address, host.hostgroup_name, check_command,
                    host.parent)
...
def add_demo_host(ip_address, parent_name, mac, device_type_id, node_type, master_id="", hostgroup="7"):
    ...
    h = HostBll()
    rvalue = h.add(
        ip_address, ip_address, ip_address, mac, device_type_id, "255.255.255.0", "10.10.10.1", '', '', '', None, '',
        None, 0, '', None, None, 'e', "normal", '1', 'lenny', '', '', '', 'public', 'private', '161', '162', '2c', '',
