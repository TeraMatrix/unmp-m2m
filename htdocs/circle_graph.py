...
# Purpose                       :       This file display the Network  map.
#                                       This file show  all connected host from NMS.
...
def recursive_function(graph, start, parent_len, temp_len):
    ...
    for node in graph[start]:
        json += "{\"id\": \"%s\",\"name\":\"%s\",\"children\":[" % (node, node)
        # id why we are giving same id to all childern
        #  it should be different each 0 time
        parent_len -= 1
...
def graph(h):
    ...
    html.write("""<div id="center-container">\
                    ...
                    """)
    html.write('<input type=\"hidden\" id=\"nms_instance\"'
               ' name=\"nms_instance\" value=\"%s\" />' % (nms_instance))
...
def show_network_graph(h):
    ...
    try:
        ...
        # end of code
        j = 0
        sel_query = "SELECT hosts.host_id,hosts.ip_address,hosts.host_name," \
                    "host1.ip_address as parent_name,hosts.lock_status," \
                    "hosts.host_state_id,host_states.state_name," \
                    "host_assets.longitude,host_assets.latitude," \
                    "hosts.device_type_id " \
                    "FROM nms_instance INNER JOIN hosts " \
                    "ON nms_instance.nms_id=hosts.nms_id \
                INNER JOIN  host_states" \
                    " ON hosts.host_state_id = host_states.host_state_id \
                INNER JOIN host_assets " \
                    "ON host_assets.host_asset_id = hosts.host_asset_id \
                LEFT JOIN hosts as host1 " \
                    "ON host1.host_id = hosts.parent_name \
                WHERE hosts.is_deleted=0 " \
                    "and hosts.ip_address='localhost' " \
                    "and nms_instance.nms_name='%s'" % (nms_name)
        ...
        if len(hostgroup_ids_list) > 0:
            sql = "SELECT hosts.host_id,hosts.ip_address," \
                  "hosts.host_name,host1.ip_address as parent_name," \
                  "hosts.lock_status,hosts.host_state_id," \
                  "host_states.state_name,host_assets.longitude," \
                  "host_assets.latitude,hosts.device_type_id FROM nms_instance " \
                  "INNER JOIN hosts " \
                  "ON nms_instance.nms_id=hosts.nms_id \
            INNER JOIN  host_states " \
                  "ON hosts.host_state_id = host_states.host_state_id \
            ...
            WHERE hosts.is_deleted=0 and nms_instance.nms_name='%s' " \
                  "AND hostgroups.hostgroup_id IN (%s)" \
        ...
        if len(results) > 0:
            recursive_function(
                relation_dic, 'localhost', len(relation_dic['localhost']), 0)
        else:
            #@TODO:
            #json = "{}"
            #json should return the Localhost added in the system and not
            #unknown host. Here JSON variable has been rewritten
            # where as the JSON variable should have been used from above
            #fixed for version 1.9.2.2
            json = '{"id":"1","name":"localhost","children":[]}'
...
# This function is count the no of nms exist in the and create the json
# format for that nms with detail information.
...
def network_nms_details(h):
    ...
    @version: 0.1
    @date: 18 sept 2011
    @note: this function is count the no of nms exist in the and
    create the json format for that nms with detail information and
     its alseo provide the total enable host and disable host and
     NMS status.
    @organization: Code Scape Consultants Pvt. Ltd.
    @copyright: 2011 Code Scape Consultants Pvt. Ltd.
    """
    ...
    try:
        ...
        for i in range(len(nms_result)):
            nms_data_json += "{\"type\":\"nms\",\"name\":\"%s\"}%s" % (
                nms_result[i][0], (',' if i < len(nms_result) - 1 else ''))
...
# This fuction is responsible for the showing the details of all the
# hosts on the google map we will show this result in table format
# on host click event.
...
def show_host_details(h):
    ...
    try:
        ...
        sel_query = "SELECT device_type_id,device_name " \
                    "FROM device_type"
        cursor.execute(sel_query)
        ...
        sql = "SELECT  hosts.host_name,hosts.ip_address,hosts.mac_address," \
              "host_states.state_name,hosts.timestamp,hosts.host_id," \
              "hosts.device_type_id,hosts.host_alias " \
              "FROM hosts " \
              "INNER JOIN host_states " \
              "ON hosts.host_state_id=host_states.host_state_id " \
              "WHERE hosts.ip_address='%s' and hosts.is_deleted=0" % host_ip
        # execute the query and fetch the result.
        cursor.execute(sql)
        result = cursor.fetchall()
        sql = "SELECT trap_event_type " \
              "FROM trap_alarm_current " \
              "WHERE agent_id='%s' " \
              "ORDER BY timestamp DESC LIMIT 1" % host_ip
        cursor.execute(sql)
        ...
        if len(result) > 0:
            for row in result:
                ...
                sevices_status += "<table style=\"width:251px;font-size:11px;\">" \
                                  "<colgroup><col width=\"25%%\"/><col width=\"25%%\"/>" \
                                  "<col width=\"25%%\"/><col width=\"25%%\"/></colgroup>"
                sevices_status += "<tr><td title=\"ok\" style=\"background-color:#66C947; height:20px; width:20px; \">" \
                                  "<lable>" \
                ...
                <td><a href='javascript:enabledDevice(\"%s\");'> " \
                              "<img src=\"images/alert_restart.png\" alt=\"Enabled Device\" \
                title=\"Enabled Device\" style=\"width:12px;\"/> Enabled Device</a>" \
                              "</td></tr></table>" % ('' if row[1] == "" or row[1] is None else row[1], row[1])

                some_link += "<table style=\"width:251px;font-size:11px;\"><colgroup>" \
                             "<col width=\"33%%\"/><col width=\"33%%\"/><col/></colgroup><tr>"
                ...
                <img src=\"images/new/alert.png\" alt=\"Alarm\" title=\"Alarm\" style=\"width:12px;\"/>\
                <a href=status_snmptt.py?ip_address=%s- alt=\"Alarms\">Alarms</td>" % (
                        '' if row[1] == "" or row[1] is None else row[1])
                some_link += "<td style=\"width:30px\"><lable style=\" width:100px; font-size:13px;\">\
                ...
                host_detail_data += "<tr><td>Device Type </td><td>:</td><td> %s </td></tr>" % (
                    '-' if row[6] == "" or
                           row[6] is None
                    else device_name_dict[row[6].strip()])
                if row[6] == 'ap25':
                    sel_query = "SELECT count(*) FROM ap_connected_client " \
                                "inner join hosts on ap_connected_client.host_id=hosts.host_id  " \
                                "WHERE state='1' and hosts.ip_address='%s'" % (row[1])
                    cursor.execute(sel_query)
                    result = cursor.fetchall()
                ...
                host_detail_data += "<tr><td>Device Age </td><td>:</td><td> %s </td></tr>" % (
                    '-' if host_age == 0
                    else paint_age(host_age, checked == 1, 60 * 10))
                host_detail_data += "<tr><td>Last Update </td><td>:</td><td> %s </td></tr>" % (
                    '-' if row[4] == "" or
                           row[4] is None
                    else datetime.strftime(row[4], "%d %B %Y"))
                host_detail_data += "<tr><td>Current Alarm</td><td>:</td><td>%s</td></tr></table>" % (
                    '-' if last_alarm is None else last_alarm[0])
            ...
            host_detail_data += "<div>" \
                                "<table style=\"width:300px;font-size:11px;margin-bottom:10px;\">" \
                                "<colgroup><col width=\"30%%\"/>" \
                                "<col width=\"5%%\"/>" \
                                "<col/>" \
                                "</colgroup>" \
                                "<tr>" \
                                "<th align=\"left\" colspan=\"3\">No device inforamtion exits." \
                                "</tr></th>" \
                                "</table>"
...
def paint_age(timestamp, has_been_checked, bold_if_younger_than):
    ...
    2. this process checked or not(boolean value) ,
    3. total up time is grether than 10 min or not. .
    @author: Rajendra Sharma
    @since: 20 sept 2011
    @version: 0.1
    @date: 18 sept 2011
