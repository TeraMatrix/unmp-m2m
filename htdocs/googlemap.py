...
def recursive_function(graph1, start, service, json):
    """
    @param graph1:
    @param start:
    @param service:
    ...
    for node in graph1[start]:
        json.append({
            ...
            "name": service[node][1],
            "id": service[node][0],
            "state": service[node][5],
            "lt": service[node][8],
            "lg": service[node][7],
            "lck": service[node][4],
            "device_type": service[node][9],
            "child": []
        })
        if not node in host_chain2:
            recursive_function(graph1, node, service, json[-1]["child"])
...
#    try:
        ...
        host_chain2 = []
        if len(results) > 0:
            recursive_function(relation_dic, 'localhost', service_dic, json[-1]["child"])
        else:
            json = []

        output_dictt = {"success": 0, "output": json, "service_dic": service_dic, "relation_dic": relation_dic}
        html.req.content_type = 'application/json'
        html.write(JSONEncoder().encode(output_dictt))
...
<div class="cntr hide">
    <div id="ctr" class="h"></div>
        <div id="cnt">
            ...
            <div id="s" class="inc">
                <div class="ftc"><span>Devices</span><input type="text" value=""  name="searchHost" id="searchHost" onkeyup="searchHostResult(event)" /></div>
...
def save_updates(h):
    ...
    try:
        ...
        output_dictt = {"success": 0, "output":
            "Successfully updated device location."}
        html.write(str(output_dictt))
...
def show_details(h):
    ...
    try:
        ...
        if len(result) > 0:
            ...
            host_detail_data += "<div><table style=\"width:300px;font-size:11px;margin-bottom:10px;\"><colgroup><col width=\"30%%\"/><col width=\"5%%\"/><col/></colgroup><tr><th align=\"left\" colspan=\"3\">No device inforamtion exits.</tr></th></table>"
...
def enabled_device_state(h):
    ...
    try:
        ...
        output_dict = {"success": 0, "output":
            'Device State successfully updated.'}
