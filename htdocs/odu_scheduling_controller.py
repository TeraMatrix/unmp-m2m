...
def odu_scheduling(h):
    ...
    try:
        ...
        html.new_header("Device Scheduling", "odu_scheduling.py",
        ...
        device_list = odu_bll_obj.get_hostgroup_device(
            user_id, hostgroup_id_list)
        ...
        odu_bll_obj = OduSchedulingBll()
        device_type = ""
        result = odu_bll_obj.odu_multiple_select_list(
            odu_list, selectListId, device_type)
        html_str += OduScheduling.odu_multiple_select_list(
            odu_list, selectListId, result)
...
def odu_scheduling_get_device_info(h):
    ...
    result = odu_bll_obj.odu_multiple_select_list(
        odu_list, selectListId, device_type)
    html.write(str(OduScheduling.odu_multiple_select_list(
        odu_list, selectListId, result)))
        ...
        for apId in accessPoint:
            if apId.strip() != "":
                ...
                # firmware_file_name=html.var("selected_firmware")#html.req.session["firmware_file"]
                    ...
                    firmware_file_name = ""
                result3 = odu_bll_obj.update_odu_scheduler_insert(
                    scheduleId, apId, event, firmware_file_name)
        user_name = html.req.session["username"]
        create_crontab_file(user_name)
