...
def search_logs(h):
    ...
    for i in range(len(query_events_data)):
        ...
        # datetime.fromtimestamp(query_events_data[i][2]).strftime('%Y-%m-%d
        # %H:%M:%S')
        if query_events_data[i][4] in hosts_dict:
...
def manage_events(h):
    ...
    log_plugin_output = html.var(
        "log_plugin_output") and html.var("log_plugin_output") or ""
    ...
    logtime_hours = html.var(
        "logtime_hours") and html.var("logtime_hours") or 0
    ...
    snapin_list = []
    header_btn = LogsEvents().header_buttons()
    html.new_header("Device and Service Events", "manage_events.py",
                    header_btn, css_list, js_list, snapin_list)
    html.write(
        LogsEvents(
        ).manage_events(host, service, log_plugin_output, logtime_sec,
                        logtime_min, logtime_hours, logtime_days, logtime))
    html.new_footer()
...
def search_events(h):
    ...
    for i in range(len(query_events_data)):
        ...
        # datetime.fromtimestamp(query_events_data[i][2]).strftime('%Y-%m-%d
        # %H:%M:%S')
        if query_events_data[i][3] in hosts_dict:
