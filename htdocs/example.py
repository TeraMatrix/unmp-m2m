...
from inventory_bll import HostBll, NagioConfigurationBll, HostUtils
from lib import *
...
def device_details_example(h):
    ...
    if es.is_host_allow(user_id, host_id) == 0 or host_id == '1':    # temporary solution for localhost view
        javascript_list = ["js/lib/main/highcharts.js", "js/unmp/main/device_details.js"]
        search_bar = '\
	    \
	    '
        html.new_header(
            "Manage Device", "manage_host.py", "", css_list, javascript_list)
...
def ivis_action(h):
    ...
    username = ""
    hu = HostUtils()
    port_data = hu.get_host_port_path_details(host_id)
    if port_data:
        data["status"] = True
        port_data = port_data[0]
        device_path = port_data[0]
        username = port_data[2]
        password = port_data[3]
        
        if device_path:
            device_path = device_path.replace("http://","")
        if action == "restart":
            url = "http://%s/restart.cgi" % (device_path)
        else:   
            url = "http://%s/command?port=P%s&action=%s" % (device_path, port, action)
        try:
            ...
            data["msg"] = "Device Unreachable"
            data["url"] = url
            data["error"] = str(e)
    else:
        data["status"] = True
        data["msg"] = "Device does not exist."
    ...
    try:
        ...
        if host_result is not None:
            if len(host_result) > 0:
                ...
                group_results = [group_result[i][0]
		...
		<tbody>\
			...
			<th class="cell-title" colspan="4">\
                ...
                grid_str += '<input type=\"submit\" class=\"yo-small yo-button\" id=\"host_more_detail_button\" value=\"More...\" style="float:right;margin-right:15px;"/>'
        ...
        if host_result is not None:
            if len(host_result) > 0:
                ...
                query_events = "GET log\nColumns: state log_time log_type host_name service_description log_plugin_output\nFilter: host_name ~~ %s\nFilter: service_description ~~ %s\nFilter: log_plugin_output ~~ %s\nFilter: log_time %s %s" % (
                    host_result[0][0], service, log_plugin_output, logtime_, time_stamp)
                html.live.set_prepend_site(True)
...
def get_time_tick(timetick, flag=0):
    ...
    if second < 60:
        ...
        minute = int((second - hour * 3600) / 60)
        last_check = str(
            delta.days) + " days," + str(hour) + " hour," + str(minute) + " min"
    return last_check
...
def service_details_grid(host_id):
    ...
    try:
        ...
        if flag == 0:
            grid_str += '<tr><td colspan=\"7\">No polling exist</td></tr>'
        grid_str += '</tbody>\
		</table>\
		<div class="status_legend">\
		    <span class="span-status-container span-extra-padding icon-0">Ok</span>\
...
def graph_container():
    html = '''<table style="display:none;" id="graph_container" width="100%" border="0" cellpadding="0" cellspacing="0" class="yo-table">
		<colgroup>
		    <col style="width:85%;">
		    <col style="width:10%;">
		    <col style="width:5%;">
		...
		<tbody>
		<tr class="yo-table-head">
		    <th class=" vertline">Service Graph</th>
		    ...
		    <th>
		    <a href="#" id="close_graph"><img src="images/new/delete.png"/></a>
		    </th>
		</tr>
		</table>
		<div id="graph_div" style="display:none;">Loading...</div>
		'''
    return html
...
def manage_ports(host_id):
    ...
    if port_data:
        port_data = port_data[0]
        if port_data[1] is not None and port_data[0]:
            status = True
            number_of_ports = port_data[1]
            
    hu.close_database_connection()
    grid_str = '''<table id="table_container" width="100%" border="0" cellpadding="0" cellspacing="0" class="yo-table">
		<colgroup>
		    <col style="width:20%;">
		    <col style="width:15%;">
		    <col style="width:auto">
		...
		<tbody>
		<tr class="yo-table-head">
		    <th class=\"vertline\">Port Number</th>
		    <th class=\"vertline\">Action</th>
		    <th class=\"vertline\">Info</th>
		</tr>'''
    for i in range(number_of_ports):
        grid_str += "<tr>\
                          <td class=\"vertline\"><div class=\"port\">Port %(index)s</div></td>\
					      <td class=\"vertline\">\
					      <a data-action=\"status\" data-host=\"%(host_id)s\" data-port=\"%(index)s\" href=\"#\" class=\"ivis-action\"><img src=\"images/new/refresh3.png\" class=\"daemon-img refresh w-tip-image\" original-title=\"Get Status\"/></a>\
					      <a data-action=\"on\" data-host=\"%(host_id)s\" data-port=\"%(index)s\" href=\"#\" class=\"ivis-action\"><img src=\"images/new/play.png\" class=\"daemon-img start w-tip-image\" original-title=\"ON\"/></a>\
					      <a data-action=\"off\" data-host=\"%(host_id)s\" data-port=\"%(index)s\" href=\"#\" class=\"ivis-action\"><img src=\"images/new/stop.png\" class=\"daemon-img stop w-tip-image\" original-title=\"OFF\"/></a>\
					      </td>\
					      <td style=\"text-align:left\">\
					      </td>\
					    </tr>" % { "index": str(i), "host_id": host_id}
    grid_str += "<tr>\
                  <td class=\"vertline\"><div class=\"device\">Device</div></td>\
			      <td class=\"vertline\">\
			      <a href=\"#\" data-action=\"restart\" data-host=\"%(host_id)s\" class=\"ivis-action\"><img src=\"images/new/restart.png\" class=\"daemon-img restart w-tip-image\" original-title=\"Restart\"/></a>\
			      </td>\
			      <td style=\"text-align:left\">\
			      </td>\
			    </tr>" % {"host_id": host_id}
...
def ip_camera_video(host_id):
    status = False
    html = ""
    device_type = ""
    ip_address = ""
    hu = HostUtils()
    host_data = hu.get_host_details(host_id)
    if host_data:
        host_data = host_data[0]
        device_type = host_data[0]
        ip_address = host_data[1]
        http_username = host_data[2]
        http_password = host_data[3]
        http_port = host_data[4]
        http_user_pass = ""
        if http_username and http_password:
            http_user_pass = "%s:%s" % (http_username, http_password)
    hu.close_database_connection()
    if "ipc" == device_type:
        status = True
        grid_str = '''<table id="table_container" width="100%%" border="0" cellpadding="0" cellspacing="0" class="yo-table">
		    ...
		    <tbody>
		    <tr class="yo-table-head">
		        <th class=\"vertline\">State</th>
		        <th class=\"vertline\">Service Name</th>
		        <th style=\"text-align:left\" class=\"vertline\">Action</th>
		    </tr>
		    <tr>
		        <td class="vertline">
		            <span class="span-status-container icon-0"></span>
		        </td>
		        <td class="vertline">
		            Video
		        </td>
		        <td class="vertline">
		            <a href="#" class="video-action"  data-ip="%(ip_address)s"  data-port="%(http_port)s"  data-username-password="%(http_user_pass)s">
		                <img src="images/new/computer.png" class="w-tip-image" original-title="Start Video"/>
		            </a>
		        </td>
		    </tr>
		    ''' % {"http_port": http_port, "http_user_pass": http_user_pass, "ip_address":ip_address }
...
def service_monitoring(host_id):
    status = False
    html = ""
    hu = HostUtils()
    sp_list = hu.get_active_service_parameters_by_host_id(host_id)
    sp_data = hu.get_latest_host_service_parameter_data(host_id)
    sp_data_list  = []
    sp_timestamp = ""
    hu.close_database_connection()
    grid_str = '''<table id="table_container" width="100%" border="0" cellpadding="0" cellspacing="0" class="yo-table">
		...
		<tr class="yo-table-head">
		    <th class=\"vertline\">State</th>
		    <th class=\"vertline\">Service Name</th>
		    <th style=\"text-align:left\" class=\"vertline\">Value</th>
		    <th style=\"text-align:left\" class=\"vertline\">Min Threshold</th>
		    <th style=\"text-align:left\" class=\"vertline\">Max Threshold</th>
		    <th style=\"text-align:left;\" class=\"vertline\">Data type</th>
		    <th style=\"text-align:left;\" class=\"vertline\">Time</th>
		    <th style=\"text-align:left\" class=\"vertline\">Action</th>
		</tr>'''
    if sp_list:
        if sp_data:
            sp_data = sp_data[0]
            if sp_data[0] and sp_data[1]:
                sp_data_list = sp_data[0].split(";")
                sp_timestamp = sp_data[1].strftime("%Y-%m-%d %H:%M:%S")
        ...
        for i in range(len(sp_list)):
            ...
            if i < sp_data_list_len - 1:
                ...
                data = "-"
            unit = sp_list[i][4] and "(%s)" % sp_list[i][4] or ""
            ...
            if 1 == sp_list[i][7]:
                grid_str += "<tr>\
                          <td class=\"vertline\"><img src=\"images/new/status-%s.png\"></td>\
					      <td style=\"text-align:left\" class=\"vertline\">%s</td>\
					      <td style=\"text-align:left\" class=\"vertline\">%s %s</td>\
					      <td style=\"text-align:left\" class=\"vertline\">%s</td>\
					      <td style=\"text-align:left\" class=\"vertline\">%s</td>\
					      <td style=\"text-align:left\" class=\"vertline\">%s</td>\
					      <td style=\"text-align:left\" class=\"vertline\">%s</td>\
					      <td style=\"text-align:left\" class=\"vertline\">\
					      <a data-graph-name=\"%s\" data-graph-id=\"%s\" data-host-id=\"%s\" data-graph-index=\"%s\" data-graph-unit=\"%s\" data-graph-data-type=\"%s\" class=\"sp_graph\" href=\"#\" title=\"View Graph\"><img src=\"images/new/graph.png\"/></a>\
					      <a href=\"#\" class=\"service_setting\" title=\"Update Threshold\" data-pid=\"%s\"><img src=\"images/new/settings.png\"/></a>\
					      </td>\
					    </tr>" % (state, sp_list[i][1], data, unit, (None != sp_list[i][5] and sp_list[i][5] or "-"), (None != sp_list[i][6] and sp_list[i][6] or "-"), sp_list[i][3], sp_timestamp, sp_list[i][1], sp_list[i][0], host_id, i, (sp_list[i][4] and sp_list[i][4] or ""), sp_list[i][3], sp_list[i][8])
        grid_str += "</tbody></table><div class=\"status_legend\" id=\"table_container_help\">\
		    <span class=\"span-status-container span-extra-padding icon-0\">Ok</span>\
		    <span class=\"span-status-container span-extra-padding icon-1\">Warning</span>\
		    <span class=\"span-status-container span-extra-padding icon-2\">Critical</span>\
		    <span class=\"span-status-container span-extra-padding icon-3\">Unknown</span>\
...
def tabs2(host_id):
    ...
    """
    # for Sbsmart Device
    # get host service parameters
    hsp = service_monitoring(host_id)
    hsp_link = ''
    hsp_html = ''
    if hsp[0]:
        hsp_link = '<li><a href="#content_4" id="host_service_details">Service Details</a></li>'
        hsp_html = hsp[1]
    
    # For Ip Camera
    ipc_video = ip_camera_video(host_id)
    if ipc_video[0]:
        hsp_link = '<li><a href="#content_4" id="ip_cam_video">Service Details</a></li>'
        hsp_html = ipc_video[1]
        
    # For Ivis Device
    manage_port = manage_ports(host_id)
    manage_port_link = ''
    manage_port_html = ''
    if manage_port[0]:
        manage_port_link = '<li><a href="#content_5">Manage Ports</a></li>'
        manage_port_html = manage_port[1]
    tab_str = ''
    tab_str += '\
    <div class="yo-tabs">\
        <ul>\
            <li><a class="active" href="#content_1">Host Details</a></li><li><a href="#content_2">Polling</a></li><li><a href="#content_3">Logs</a></li>%s%s\
        </ul>\
        <div id="content_1" class="tab-content">\
            %s\
        </div>\
    ...
    </div>' % (hsp_link, manage_port_link, host_details_grid(host_id), service_details_grid(host_id), log_details_grid(host_id), hsp_html, manage_port_html)
    return tab_str
...
def host_bar(host_id):
    ...
    try:
        ...
        sql = "SELECT  hosts.host_name,hosts.ip_address,hosts.mac_address,hosts.host_state_id,hosts.device_type_id,hosts.host_alias,hosts.creation_time,hosts.timestamp,hosts.lock_status, device_type.icon_name, hosts.http_username, hosts.http_password, hosts.http_port FROM hosts INNER JOIN device_type ON hosts.device_type_id = device_type.device_type_id LEFT JOIN host_states ON hosts.host_state_id=host_states.host_state_id WHERE hosts.host_id='%s'" % host_id
        # execute the query and fetch the result.
        cursor.execute(sql)
        host_result = cursor.fetchall()
        
        # get host arm status
        sql = "SELECT service_status FROM host_service_details WHERE host_id = '%s'" % (host_id)
        cursor.execute(sql)
        host_arm_status = cursor.fetchall()
        if host_arm_status:
            host_arm_status = host_arm_status[0]
            if host_arm_status:
                host_arm_status = str(host_arm_status[0]).split(",")
                host_arm_status_len = len(host_arm_status)
                if host_arm_status_len > 0:
                    action1 = host_arm_status[0]
                if host_arm_status_len > 1:
                    action2 = host_arm_status[1]
                if host_arm_status_len > 2:
                    action3 = host_arm_status[2]
                if host_arm_status_len > 3:
                    action4 = host_arm_status[3]
        cursor.close()
        db.close()
        ...
        if host_result is not None:
            if len(host_result) > 0:
                ...
                device_icon = host_result[0][9] and host_result[0][9] or device_icon
                http_username = host_result[0][10]
                http_password = host_result[0][11]
                http_port = host_result[0][12]
                if http_username and http_password:
                    http_user_pass = "%s:%s" % (http_username, http_password)
                host_status = "0"
                age = str('0' if host_age == 0 else paint_age(
                    host_age, checked == 1, 60 * 10))
        ...
        relay1 = "ON" == action1 and "green" or "red"
        relay2 = "ON" == action2 and "green" or "red"
        relay3 = "ON" == action3 and "green" or "red"
        relay4 = "ON" == action4 and "green" or "red"
        bar = '<div id="host_bar" class=\"host_bar\">\
            ...
            <div class="device_info" style=\"border-left:0px none;\">\
                ...
                <p class="text">Total Polling: %(total)s</p>\
            ...
            <div class=\"host-action-div\" id="host_relay_action">\
            <ul> \
            <li> \
            <a href=\"#\" data-hid=\"%(host_id)s\" data-arm=\"1\" data-action=\"%(action1)s\" class=\"relay-action\" title="Relay 1"><img src=\"images/new/r-%(relay1)s.png\" /></a>\
            <li> \
            <li> \
            <a href=\"#\" data-hid=\"%(host_id)s\" data-arm=\"2\" data-action=\"%(action2)s\" class=\"relay-action\" title="Relay 2"><img src=\"images/new/r-%(relay2)s.png\" /></a>\
            <li> \
            <li> \
            <a href=\"#\" data-hid=\"%(host_id)s\" data-arm=\"3\" data-action=\"%(action3)s\" class=\"relay-action\" title="Relay 3"><img src=\"images/new/r-%(relay3)s.png\" /></a>\
            <li> \
            <li> \
            <a href=\"#\" data-hid=\"%(host_id)s\" data-arm=\"4\" data-action=\"%(action4)s\" class=\"relay-action\" title="Relay 4"><img src=\"images/new/r-%(relay4)s.png\" /></a>\
            <li> \
            <li> \
            <a href="#" class="video-action" data-ip="%(ip_address)s"  data-port="%(http_port)s"  data-username-password="%(http_user_pass)s">\
                <img src="images/new/computer.png" class="s-tip-image" original-title="Start Video">\
            </a>\
            </li> \
            </ul> \
            </div>\
                   ...
                   "warning": warning, "critical": critical, "unknown": unknown, "relay1": relay1, "relay2": relay2,
                   "relay3": relay3, "relay4": relay4, "action1": action1, "action2":action2, "action3":action3,
                   "action4": action4, "host_id": host_id, "http_port": http_port, "http_user_pass": http_user_pass}
...
def host_extra_bar(host_details_dic={}):
    ...
    for dic in host_details_dic:
        total_dic_item += 1
        bar += '<p class="text"><strong>%s</strong></p><p class="text">%s</p>' % (
...
def nagios_hostgroup(h):
    ...
    try:
        ...
        html.new_header("Service Management",
                        "nagios_hostgroup.py", "", css_list, js_list)
...
left join hosts as hosts2 on hosts2.host_id=hosts.parent_name join device_type on device_type.device_type_id=hosts.device_type_id where hosts.host_id='%s'" % (
        ...
        if flag == 0:
            html_str += '<tr><td colspan=\"7\">No polling exist</td></tr>'
        html_str += '</tbody>\
		</table>\
		<div class="status_legend">\
		    <span class="span-status-container span-extra-padding icon-0">Ok</span>\
...
def edit_hostgroup_service_details(h):
    ...
    try:
        conn = MySQLdb.connect(*SystemConfig.get_mysql_credentials())
        cursor = conn.cursor()
        query = "select host_alias,host_id from hosts where host_id in (%s) and is_localhost=0 " % (
            child_hosts if child_hosts != "" else '" "')
        cursor.execute(query)
        result = cursor.fetchall()
        conn.close()
...
def apply_nagios_hostgroup_changes(h):
    ...
    try:
        ...
        if hosts_service_hosts_array != None:
            query = "update host_services set normal_check_interval='%s' where service_description like 'Statictics Service%%' and host_id in ('%s') " % (
                selected_service_hosts_time, "','".join(hosts_service_hosts_array.split(',')))
            cursor.execute(query)
        if hosts_snmp_uptime_array != None:
            query = "update host_services set normal_check_interval='%s' where service_description like 'SNMP UPTIME%%' and host_id in ('%s') " % (
                selected_snmp_uptime_time, "','".join(hosts_snmp_uptime_array.split(',')))
            cursor.execute(query)
        conn.commit()
        conn.close()
...
def apply_hostgroup_host_changes(h):
    ...
    try:
        ...
        for key in hostgroup_json:
            query = "update hosts_hostgroups set hostgroup_id='%s' where host_id in ('%s') " % (
                key[9:], "','".join(hostgroup_json[key]))
            cursor.execute(query)
            conn.commit()
        conn.close()
