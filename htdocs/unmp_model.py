...
##
##	def __repr__(self):
# return "<ApSchedulingHostMapping('%s','%s','%s')>"
# %(self.ap_scheduling_host_mapping_id,self.ap_scheduling_id,self.host_id)
...
class Odu16ConfigProfiles(Base):
    ...
    config_profile_type_id = Column(
        VARCHAR(16), ForeignKey('config_profile_type.config_profile_type_id'))
...
class DiscoveredHosts(Base):
    ...
    device_type_id = Column(
        VARCHAR(64), ForeignKey('device_type.device_type_id'))
    mac_address = Column(VARCHAR(32))
...
class Discovery(Base):
    ...
    discovery_type_id = Column(
        INTEGER, ForeignKey('discovery_type.discovery_type_id'))
...
class Hostgroups(Base):
    ...
    def __repr__(self):
        return "<Hostgroups('%s','%s','%s','%s','%s','%s','%s','%s','%s',%s)>" % (
        self.hostgroup_id, self.hostgroup_name, self.hostgroup_alias, self.timestamp, self.created_by,
        self.creation_time, self.is_deleted, self.updated_by, self.is_default, self.is_application)
...
class Hosts(Base):
    ...
    config_profile_id = Column(
        INTEGER, ForeignKey('config_profiles.config_profile_id'))
    timestamp = Column(TIMESTAMP)
    created_by = Column(VARCHAR(64))
    creation_time = Column(TIMESTAMP)
    is_deleted = Column(SMALLINT)
    ...
    device_path = Column(VARCHAR(128))
    number_of_ports = Column(INTEGER)

    def __init__(self, host_name, host_alias, ip_address, phone_no, mac_address, device_type_id, netmask, gateway, primary_dns,
                 ...
                 firmware_mapping_id=None, device_path=None, number_of_ports=None):
        self.host_id = None
    ...
    def __repr__(self):
        return "<Hosts('%s','%s','%s', %s,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')>" % (
        self.host_id, self.host_name, self.host_alias, self.ip_address, self.phone_no, self.mac_address, self.device_type_id,
...
class HostAlertActionMapping(Base):
    ...
    host_alert_masking_id = Column(
        VARCHAR(64), ForeignKey('host_alert_masking.host_alert_masking_id'))
...
class HostAlertMasking(Base):
    ...
    is_repeated = Column(SMALLINT)
    acknowledge_id = Column(
        VARCHAR(16), ForeignKey('acknowledge.acknowledge_id'))
...
# %(self.host_asset_id,self.longitude,self.latitude,self.serial_number,self.hardware_version,self.software_update_time,self.software_update_state,self.software_update_msg)
...
##
##	def __repr__(self):
# return "<RetryApScheduling('%s','%s','%s','%s','%s','%s','%s','%s')>"
# %(self.retry_ap_scheduling_id,self.retry_date,self.retry_time,self.host_id,self.message,self.event,self.timestamp,self.created_by)
...
class TrapAlarmActionMapping(Base):
    ...
    trap_alarm_masking_id = Column(
        VARCHAR(64), ForeignKey('trap_alarm_masking.trap_alarm_masking_id'))
    acknowledge_id = Column(
        VARCHAR(64), ForeignKey('acknowledge.acknowledge_id'))
    timestamp = Column(TIMESTAMP)
    created_by = Column(VARCHAR(64))
    creation_time = Column(TIMESTAMP)
    updated_by = Column(VARCHAR(64))
...
class TrapAlarmMasking(Base):
    ...
    trap_alarm_field = Column(
        VARCHAR(32), ForeignKey('trap_alarm_field_table.trap_alarm_field'))
...
class GetOdu16PeerNodeStatusTable(Base):
    ...
    peer_node_status_num_slaves = Column(
        Enum('1', '2', '3', '4', '5', '6', '7', '8', name='enumslaves'))
    peer_node_status_timer_adjust = Column(INT)
    peer_node_status_rf_config = Column(VARCHAR(16))
    index = Column(
        Enum('1', '2', '3', '4', '5', '6', '7', '8', name='enumIndex'))
    timeslot_index = Column(
...
class Ap25AclMacTable(Base):
    ...
    config_profile_id = Column(
        INTEGER, ForeignKey('config_profiles.config_profile_id'))
    vapselection_id = Column(
        INTEGER, ForeignKey('ap25_vapSelection.ap25_vapSelection_id'))
...
class HostStatus(Base):
    ...
    status = Column(
        Enum(
            ...
            '14', '15', '16', '17', '18', '19', '20', name='enumstate'))
...
mapper(Odu1007_2_29_oid_table, Odu1007_2_29_oid_table.__table__)

## i don't know how to define the primary mapper
...
mapper(
    IduAlarmPortConfigurationTable, IduAlarmPortConfigurationTable.__table__)
...
mapper(IduPortSecondaryStatisticsTable,
       IduPortSecondaryStatisticsTable.__table__)
...
mapper(IduSwPrimaryPortStatisticsTable,
       IduSwPrimaryPortStatisticsTable.__table__)
...
mapper(IduTdmoipNetworkInterfaceConfigurationTable,
       IduTdmoipNetworkInterfaceConfigurationTable.__table__)
mapper(IduTdmoipNetworkInterfaceStatisticsTable,
       IduTdmoipNetworkInterfaceStatisticsTable.__table__)
mapper(IduTemperatureSensorConfigurationTable,
       IduTemperatureSensorConfigurationTable.__table__)
...
mapper(Countries, Countries.__table__, properties={"states": relationship(
    States, backref="countries", cascade="all,delete,delete-orphan")})

# mapper(ApScheduling,ApScheduling.__table__,properties={"ap_scheduling_host_mapping":relationship(ApSchedulingHostMapping,backref="ap_scheduling",
# cascade="all,delete,delete-orphan")})
...
mapper(ConfigProfileType, ConfigProfileType.__table__, properties={"config_profiles": relationship(
    Odu16ConfigProfiles, backref="config_profile_type", cascade="all,delete,delete-orphan")})
...
mapper(HostAlertMasking, HostAlertMasking.__table__, properties={"host_alert_action_mapping": relationship(
    HostAlertActionMapping, backref="host_alert_masking", cascade="all,delete,delete-orphan")})

mapper(HostAssets, HostAssets.__table__, properties={"hosts": relationship(
    Hosts, backref="host_assets", cascade="all,delete,delete-orphan")})

# mapper(FirmwareMapping,FirmwareMapping.__table__,properties={"hosts":relationship(Hosts,backref="firmware_mapping",cascade="all,delete
# ,delete-orphan")})

mapper(HostOs, HostOs.__table__, properties={"hosts": relationship(
    Hosts, backref="host_os", cascade="all,delete,delete-orphan")})

mapper(HostStates, HostStates.__table__, properties={"hosts": relationship(
    Hosts, backref="host_states", cascade="all,delete,delete-orphan")})

mapper(HostVendor, HostVendor.__table__, properties={"hosts": relationship(
...
mapper(Odu1007_2_25_oids, Odu1007_2_25_oids.__table__, properties={"odu100_7_2_25_oids_multivalues": relationship(
    Odu1007_2_25_oids_multivalues, backref="odu100_7_2_25_oids", cascade="all,delete,delete-orphan")})

mapper(Odu1007_2_29_oids, Odu1007_2_29_oids.__table__, properties={"odu100_7_2_29_oids_multivalues": relationship(
    Odu1007_2_29_oids_multivalues, backref="odu100_7_2_29_oids", cascade="all,delete,delete-orphan")})

mapper(Pages, Pages.__table__, properties={"modules": relationship(
    Modules, backref="pages", cascade="all,delete,delete-orphan")})
...
mapper(Sites, Sites.__table__, properties={"hosts": relationship(
    Hosts, backref="sites", cascade="all,delete,delete-orphan")})

mapper(Snapins, Snapins.__table__, properties={"pages": relationship(
    Pages, backref="snapins", cascade="all,delete,delete-orphan")})

mapper(TrapAlarmFieldTable, TrapAlarmFieldTable.__table__, properties={"trap_alarm_masking": relationship(
    TrapAlarmMasking, backref="trap_alarm_field_table", cascade="all,delete,delete-orphan")})

mapper(TrapAlarmMasking, TrapAlarmMasking.__table__, properties={"trap_alarm_action_mapping": relationship(
