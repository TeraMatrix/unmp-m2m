...
def webssh(h):
    ...
    try:
        if host:
            result = ()
            ssh_dict = {'ip': None, 'port': 22, 'uname':
                        'root', 'pwd': 'password'}
            try:
...
def data_table_data_sqlalchemy(ip_address, mac_address, device_type, i_display_start, i_display_length, s_search, sEcho, sSortDir_0, iSortCol_0, userid=None, html_var={}):  # ,
    ...
    if device_type == "odu100" or device_type == "odu16" or device_type == "odu":
        a_columns = [
            "host_id", "host_alias", "hostgroup_name", "ip_address", "mac_address",
            "ra_mac", "device_type_id", "reconcile_health", "config_profile_id"]
        table_classes = [Hosts, Hostgroups, Hosts, HostAssets,
                         Hosts, HostsHostgroups, HostgroupsGroups, UsersGroups]
        table_columns = [["host_id", "host_alias"],
                         ["hostgroup_name"], ["ip_address", "mac_address"], ["ra_mac"], ["device_type_id", "reconcile_health", "config_profile_id", "is_deleted"], ["hostgroup_id", "host_id"], ["group_id", "hostgroup_id"], ["user_id", "group_id"]]

    elif device_type == 'idu4':
        a_columns = [
            "host_id", "host_alias", "hostgroup_name", "ip_address", "mac_address",
            "tdmoipInterfaceMac", "hwType", "device_type_id", "reconcile_health", "config_profile_id"]
        table_classes = [Hosts, Hostgroups, Hosts, IduIduInfoTable,
                         Hosts, HostsHostgroups, HostgroupsGroups, UsersGroups]
        table_columns = [["host_id", "host_alias"],
                         ["hostgroup_name"], ["ip_address", "mac_address"], ["tdmoipInterfaceMac", "hwType"], ["device_type_id", "reconcile_health", "config_profile_id", "is_deleted"], ["hostgroup_id", "host_id"], ["group_id", "hostgroup_id"], ["user_id", "group_id"]]
        ...
        a_columns = ["host_id", "host_alias", "hostgroup_name", "ip_address",
                     "mac_address", "device_type_id", "reconcile_health", "config_profile_id"]
        table_classes = [Hosts, Hostgroups, Hosts, HostsHostgroups,
                         HostgroupsGroups, UsersGroups]
        table_columns = [["host_id", "host_alias"],
                         ["hostgroup_name"], ["ip_address", "mac_address", "device_type_id", "reconcile_health", "config_profile_id", "is_deleted"], ["hostgroup_id", "host_id"], ["group_id", "hostgroup_id"], ["user_id", "group_id"]]

    else:
        a_columns = [
            "host_id", "host_alias", "hostgroup_name", "ip_address", "mac_address",
            "device_type_id", "reconcile_health", "config_profile_id", "radioAPmode"]
        table_classes = [Hosts, Hostgroups, Hosts, HostsHostgroups,
                         HostgroupsGroups, UsersGroups, Ap25RadioSetup]
        table_columns = [["host_id", "host_alias"],
                         ["hostgroup_name"], ["ip_address", "mac_address", "device_type_id", "reconcile_health", "config_profile_id", "is_deleted"], ["hostgroup_id", "host_id"], ["group_id", "hostgroup_id"], ["user_id", "group_id"], ["radioAPmode"]]
    # if tables are two or more than 2 then define join type [join or
    # outerjoin or user_defined_join or user_defined_outerjoin]
    table_join = [""]
    ...
    try:
        ...
        for class_i in range(0, len(table_classes)):
            if class_i == 0:
                query = sqlalche_obj.session.query(*tuple(
                ...
                if len(table_join) > class_i - 1:
                    ...
                    if table_join[class_i - 1] == "join":   # creating type of joins in tables
                        query = query.join(getattr(table_classes[class_i -
                                           1], table_classes[class_i].__tablename__))
                        # print query,'simple join'
                    elif table_join[class_i - 1] == "outerjoin":
                        query = query.outerjoin(getattr(table_classes[class_i -
                                                1], table_classes[class_i].__tablename__))
        ...
        for other_condition in other_conditions:
            if isinstance(other_condition, dict):
                if other_condition.get("type", None) == "like":
                    if other_condition.get("table_class", None) is not None and other_condition.get("table_column", None) is not None:
        ...
        if len(and_condition) == 0:
            ...
            if len(or_condition) == 0:
                ...
                query = query.filter(
                    and_(and_(*tuple(and_condition)), or_condition[0]))
            else:
                query = query.filter(
                    and_(and_(*tuple(and_condition)), *tuple(or_condition)))
...
# query = query.order_by(desc(getattr(Hosts,a_columns[int(iSortCol_0)])))
        ...
        if (i_display_start is not None and i_display_length != '-1'):  # creating paging in query
            query = query.limit(
    ...
    if connection_chk == 0 or connection_chk == "0":
        ...
        try:
            ...
            if device_list_state == "enabled":
                select_list_html += "<select id=\"%s\" name=\"select_device_list\" %s>" % (
                    select_list_id, style)
            else:
                select_list_html += "<select id=\"%s\" name=\"%s\" disabled=\"disabled\" %s>" % (
                    select_list_id, select_list_id, style)
            select_list_html += "<option value=\"\">-- Select Device --</option>"
            if device_types is None or device_types == "":
                device_list = sqlalche_obj.session.query(
                    DeviceType.device_type_id, DeviceType.device_name).filter(DeviceType.is_deleted == 0).order_by(DeviceType.sequence).all()
        ...
        select_list_html = ""
        select_list_html += "<select id=\"%s\" name=\"select_device_list\">" % (
            select_list_id)
        select_list_html += "<option value=\"\">-- Select Device --</option>"
...
def make_select_list_using_dictionary(select_param_dic, selected_field, select_list_state, select_list_id, is_readonly, select_list_initial_msg, attr={}):
    ...
    try:
        ...
        if select_list_state == "enabled":
            select_list_html += "<select id=\"%s\" name=\"%s\" %s >" % (
                select_list_id, select_list_id, attr_html)
        else:
            select_list_html += "<select id=\"%s\" name=\"%s\" disabled=\"disabled\" %s>" % (
                select_list_id, select_list_id, attr_html)

        select_list_html += "<option value=\"\" selected=\"selected\">-- Select %s --</option>" % (
            select_list_initial_msg)
        for i in range(len(select_param_dic["value"])):
            if select_param_dic["value"][i] == selected_field:
                select_list_html += "<option value=\"%s\" selected=\"selected\">%s</option>" % (
                    select_param_dic["value"][i], select_param_dic["name"][i])
            else:
                select_list_html += "<option value=\"%s\">%s</option>" % (
                    select_param_dic["value"][i], select_param_dic["name"][i])
        select_list_html += "</select>"
...
class IpMacSearch(object):
    ...
    def ip_search(self, device_type, search_value, userid, ip_mac_search):
        ...
        ip_mac_list = sqlalche_obj.session.query(
            Hosts.ip_address if int(
                ip_mac_search) == 1 else Hosts.mac_address).filter(and_(Hosts.is_deleted == 0,
                                                                        Hosts.ip_address.like('%s%%' % (search_value)) if int(ip_mac_search) == 1 else Hosts.mac_address.like(
                                                                            '%s%%' % (search_value)), Hosts.device_type_id != 'unknown', Hosts.device_type_id.like('%s%%' % (device_type)), UsersGroups.user_id == '%s' % (userid),
                                                                        UsersGroups.group_id == HostgroupsGroups.group_id, HostsHostgroups.hostgroup_id == HostgroupsGroups.hostgroup_id, Hosts.host_id == HostsHostgroups.host_id))\
            .order_by(Hosts.host_alias).order_by(Hosts.ip_address).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
        return ip_mac_list
    ...
    def get_ip_mac_selected_device(self, selected_val, ip_mac_val):
        ...
        try:
            sqlalche_obj.sql_alchemy_db_connection_open()
            ip_mac_list = []
            ip_mac_dic = {'ip_address': "", 'mac_address': "",
                          'error': "", "success": 0}

            ip_mac_list = sqlalche_obj.session.query(Hosts.mac_address if int(ip_mac_val) == 1 else Hosts.ip_address, Hosts.device_type_id).\
                filter(Hosts.ip_address == selected_val if int(ip_mac_val)
                       == 1 else Hosts.mac_address == selected_val).all()
            sqlalche_obj.sql_alchemy_db_connection_close()
            if len(ip_mac_list) != []:
                if int(ip_mac_val) == 1:
    ...
    def make_select_list_using_dictionary(select_param_dic, selected_field, select_list_state, select_list_id, is_readonly, select_list_initial_msg, style=""):
        ...
        try:
            ...
            if select_list_state == "enabled":
                ...
                select_list_html += "<select id=\"%s\" name=\"%s\" disabled=\"disabled\" %s>" % (
                    select_list_id, select_list_id, style)
            select_list_html += "<option value=\"\"  selected=\"selected\" >-- Please select %s --</option>" % (
                select_list_initial_msg)
            for i in range(len(select_param_dic["value"])):
                if select_param_dic["value"][i] == selected_field:
                    select_list_html += "<option value=\"%s\" selected=\"selected\">%s</option>" % (
...
def make_select_list(table_name="", value_field_name="", display_field_name="", selected_field="", select_list_state="enabled", select_list_id="", is_readonly="false", select_list_initial_msg=""):
    ...
    try:
        ...
        if db == 1:
            ...
            select_list_html = ""
            if select_list_state == "enabled":
                select_list_html += "<select id=\"%s\" name=\"%s\">" % (
                    select_list_id, select_list_id)
            else:
                select_list_html += "<select id=\"%s\" name=\"%s\" disabled=\"disabled\">" % (
                    select_list_id, select_list_id)
            select_list_html += "<option value=\"\">-- Select %s --</option>" % (
                display_field_name)
        if table_name == "groups":
            ...
            sql = "SELECT " + value_field_name + "," + \
                display_field_name + " FROM " + table_name
            cursor.execute(sql)
            result = cursor.fetchall()
            for row in result:
...
def localhost_dashboard(h):
    ...
    header_btn = ""
    html.new_header("Nocout System Dashboard", "", header_btn, css_list, js_list)
...
def system_processor_details(h):
    ...
    total = total is not None and total or 1
    ls = LocalSystemBll()
...
class TrapStatus(object):
    ...
    def trap_alarm_chk(self, host_id, device_type):
        ...
        try:
            ...
            trap_alarm_dic = {}
            severity = ""
            host_detail_list = sqlalche_obj.session.query(
                Hosts.ip_address).filter(Hosts.host_id == host_id).all()
            trap_alram_list = sqlalche_obj.session.query(TrapAlarms.trap_event_type, TrapAlarms.serevity, TrapAlarms.timestamp).filter(
                TrapAlarms.agent_id == host_detail_list[0].ip_address if len(host_detail_list) > 0 else TrapAlarms.agent_id == 0).order_by(desc(TrapAlarms.timestamp)).limit(5).all()
            if len(trap_alram_list) > 0:
                for i in range(0, len(trap_alram_list)):
                    if int(trap_alram_list[i].serevity) == 0 or int(trap_alram_list[i].serevity) == 2:
                        ...
                        severity = "Major"
                    trap_alarm_dic.update({'%s' % (i + 1): {'event_type': trap_alram_list[i].trap_event_type, 'Serevity': severity, 'intime':
                                          datetime.strftime(trap_alram_list[i].timestamp, "%d-%b-%Y %A %I:%M:%S %p")}})
...
def show_trap_alarms(h):
    ...
    if int(result_dic['success']) == 0:
        for i in result_dic['result']:
            html_str += "<tr>"
            html_str += "<td>%s</td><td>%s</td><td>%s</td>" % ("-" if result_dic['result'][i]['event_type'] is None else result_dic['result'][i]['event_type'],
                                                               "-" if result_dic[
                                                                   'result'][i]['Serevity'] is None else result_dic['result'][i]['Serevity'],
                                                               "-" if result_dic['result'][i]['intime'] is None else result_dic['result'][i]['intime'])

            html_str += "</tr>"
        html_str += "<tr><td colspan=3><a id=\"more_trap\" href=\"status_snmptt.py?ip_address=%s-\">More</a></td></tr>" % (
            ip_address)
    else:
        html_str += "<tr><td colspan=3>%s</td></tr>" % (result_dic['result'])
    html_str += "</table>"
    html.write(html_str)
# make_alarm_field_select_list('agent_id',"enabled","trap_alarm_field",False,"trap
# field")
...
class DeviceStatus(object):
    ...
    def device_status(self, host_id):
        ...
        try:
            sqlalche_obj.sql_alchemy_db_connection_open()
            if host_id == "" or host_id is None or host_id == 'undefined':
                result = {"success": 1, "result": "No Host Exist"}
            else:
                status_data = sqlalche_obj.db.execute(
                    "SELECT trap_event_id,host_id,timestamp FROM (SELECT * FROM  system_alarm_table WHERE host_id IN (%s)ORDER BY TIMESTAMP DESC) AS t GROUP BY host_id" % (host_id))
                host_ids = host_id.split(",")
                host_data_list = sorted(host_ids)
                host_status_dic = {}
                for row in status_data:
                    host_data = []
                    host_data.append(1) if int(row['trap_event_id']
                                               ) == 50001 else host_data.append(0)
                    host_data.append(str(datetime.strftime(
                ...
                for i in range(0, len(host_data_list)):
                    ...
                    if int(host_data_list[i]) in host_status_dic:
                        ...
                        host_list_data = sqlalche_obj.session.query(
                            Hosts.timestamp).filter(Hosts.host_id == host_data_list[i]).all()
                        host_data.append(0)
                        host_data.append(str(datetime.strftime(host_list_data[0].timestamp, "%d-%b-%Y %a %I:%M:%S %p")) if len(host_data) >
                                         0 else str(datetime.strftime(datetime.now(), "%d-%b-%Y %A %I:%M:%S %p")))
    ...
    def common_list_device_status(self, host_id):
        ...
        try:
            ...
            timer_val = datetime.strftime(
                datetime.now(), "%d-%b-%Y %a %I:%M:%S %p")
            if host_id == "" or host_id is None or host_id == 'undefined':
                result = {"success": 1, "result": {'device_status': [
                    "Device Unreachable since " + str(timer_val), "images/temp/red_dot.png"]}}
            else:
                snmp_up_time_data = sqlalche_obj.db.execute(
                    "select trap_event_id,timestamp from system_alarm_table where host_id='%s' order by timestamp desc limit 1" % (host_id))
                ...
                if snmp_up_down_time == "":
                    ...
                    device_status_image_path = "images/temp/green_dot.png"
                result = {"success": 1, "result": {'device_status':
                                                   [device_status, device_status_image_path]}}
        except Exception as e:
            result = {"success": 1, "result": {'device_status':
                                               [str(e), "images/temp/red_dot.png"]}}
        finally:
            sqlalche_obj.sql_alchemy_db_connection_close()
            return result
            ...
            try:
                for i in range(0, len(tablename_list)):
                    ...
                    if device_type == UNMPDeviceType.odu100:
                        ...
                        sql_tablename = str(
                            table_prefix) + str(tablename_list[i])
                        primary_key_id = sql_tablename + "_id"
                        sqlalche_tablename = rename_tablename(sql_tablename)
                        table_result = sqlalche_obj.session.query(
                            Odu100Oid_table.table_oid, Odu100Oid_table.varbinds).filter(Odu100Oid_table.table_name == tablename_list[i]).all()
                        ...
                        sqlalche_tablename = rename_tablename(sql_tablename)
                        table_result = sqlalche_obj.session.query(
                            IduOidTable.table_oid, IduOidTable.varbinds).filter(IduOidTable.table_name == tablename_list[i]).all()
                        ...
                        sqlalche_tablename = rename_tablename(sql_tablename)
                        table_result = sqlalche_obj.session.query(
                            Ap25Oid_table.table_oid, Ap25Oid_table.varbinds).filter(Ap25Oid_table.table_name == tablename_list[i]).all()
                    result_db = sqlalche_obj.db.execute(
                        "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE table_name = '%s' and table_schema = '%s'" % (sql_tablename, database_name[4]))
                    ...
                    if device_type == UNMPDeviceType.odu100 or device_type == UNMPDeviceType.idu4:
                        result_dic = main_bulk(str(table_result[0].table_oid) + str(".1"), host_data[0].ip_address, int(
                            host_data[0].snmp_port), host_data[0].snmp_read_community, table_result[0].varbinds)
                    else:
                        result_dic = bulktable(table_result[0].table_oid, host_data[0].ip_address, int(
                            host_data[0].snmp_port), host_data[0].snmp_read_community, table_result[0].varbinds)
                    if result_dic['success'] == 0:
                        if "config_profile_id" in column_list:
                            config_id = 1
                            sqlalche_table_result = sqlalche_obj.session.query(eval(sqlalche_tablename)).filter(getattr(
                                eval(sqlalche_tablename), "config_profile_id") == host_data[0].config_profile_id).all()
                        else:
                            config_id = 0
                        ...
                        if tablename_list[i] == "raAclConfigTable" or tablename_list[i] == "peerConfigTable" or tablename_list[i] == "basicACLconfigTable" or tablename_list[i] == "aclMacTable" or tablename_list[i] == "linkConfigurationTable":
                            sqlalche_obj.db.execute(
                                "delete from %s where config_profile_id = '%s' " % (sql_tablename,
                                                                                    host_data[0].config_profile_id))
                            time.sleep(1)
                            for row in result_dic['result']:
                                sqlalche_obj.db.execute(
                                    "Insert into %s values(NULL,%s,%s)" % (sql_tablename,
                                                                           host_data[0].config_profile_id, str(result_dic["result"][row])[1:-1]))
                        else:
                                ...
                                for row in result_dic['result']:
                                    for val in range(0, len(result_dic['result'][row])):
                                        setattr(sqlalche_table_result[
                                                row - 1], column_list[val], str(result_dic['result'][row][val]))
                                        if timestamp == 1:
                                            setattr(
                                                sqlalche_table_result[
                                                    row - 1], "timestamp",
                                ...
                                for row in result_dic['result']:
                                    if config_id == 1:
                                        sqlalche_obj.db.execute("Insert into %s values(NULL,%s,%s)" % (
                                            tablename, host_data[0].config_profile_id, str(result_dic["result"][row])[1:-1]))
                                    else:
                                        if timestamp == 0:
                                            sqlalche_obj.db.execute("Insert into %s values(NULL,%s,%s)" % (
                                                tablename, host_id, str(result_dic["result"][row])[1:-1]))
                                        else:
                                            sqlalche_obj.db.execute("Insert into %s values(NULL,%s,%s,'%s')" % (
                                                tablename, host_id, str(result_dic["result"][row])[1:-1], time_stamp[:time_stamp.find('.') - 1]))
                        success = 0
                        result_str = "Form Reconciliation Succesfully"
                    else:
                        success = 1
                        result_str = errorStatus.get(
                            result_dic.keys()[0], "Device is not responding")
            except Exception as e:
                success = 1
                result_str = str(e)
                return
        ...
        try:
            ...
            try:
                ...
                if len(firmware_result):
                    ...
                    if firmware == '7.2.29':
                        oid_table_name = "odu100_7_2_29_oid_table"
                        Odu100Oid_table = aliased(Odu1007_2_29_oid_table)
                                ...
                                for row in result_dic['result']:
                                    if len(result_dic['result'][row]) == len(column_list):
                                        for k in range(len(column_list)):
                                            setattr(sqlalche_table_result[row - 1], column_list[
                                                    k], result_dic['result'][row][k])
                                        if timestamp == 1:
                                            setattr(
                                                sqlalche_table_result[
                                                    row - 1], "timestamp",
                                        ...
                                        for k in range(len(column_list)):
                                            if column_list[k] in odu100_table_model[firmware][tablename_list[i]]:
                                                setattr(sqlalche_table_result[row - 1], column_list[
                                                        k], result_dic['result'][row][index_val])
                                                index_val += 1
                                        if timestamp == 1:
                                            setattr(
                                                sqlalche_table_result[
...
# setattr(sqlalche_table_result[row-1],"timestamp",time_stamp[:time_stamp.find('.')-1])
                                ...
                                for row in result_dic['result']:
                                    if config_id == 1:
                                        sqlalche_obj.db.execute("Insert into %s (%s) values(NULL,%s,%s)" % (
                                            sql_tablename, ",".join(column_list), host_data[0].config_profile_id, str(result_dic['result'][row])[1:-1]))
                                    else:
                                        if timestamp == 0:
                                            sqlalche_obj.db.execute("Insert into %s (%s) values(NULL,%s,%s)" % (sql_tablename, ",".join(
                                                column_list), host_id, str(result_dic['result'][row])[1:-1]))
                                        else:

                                            aa = "Insert into %s (%s) values(NULL,%s,%s,'%s')" % (
                                                sql_tablename, ",".join(column_list), host_id, str(result_dic['result'][row])[1:-1], time_stamp[:time_stamp.find('.') - 1])
...
def common_reconcile(h):
    ...
    try:
        ...
        if html.var("host_id") is not None:
            host_id = html.var("host_id")
        if user_name is None:
            user_name = ""
        if html.var("form_rec") is not None:
            ...
            if Validation.is_required(html.var("form_rec")):
                ...
                if html.var("tableName") is not None:
                    tablename = html.var("tableName")

                if html.var("tableCount") is not None:
                    tablecount = html.var("tableCount")
                host_op_status = essential_obj.get_hoststatus(host_id)
                if host_op_status is not None or host_op_status != "":
                    if int(host_op_status) == 0:
                        if int(form_rec) == 0:
                            table = 1
                            essential_obj.host_status(host_id, 11)
                            if device_type == UNMPDeviceType.odu100:
                                result = obj.reconciliation_model(
                                    host_id, device_type, tablename, tablecount=0)
                            else:
                                result = obj.reconciliation_common(
                                    host_id, device_type, tablename, tablecount=0)
                            ...
                            if device_type == UNMPDeviceType.odu100:
                                ...
                                result = rec_obj.update_reconcilation_controller(
                                    host_id, device_type, table_prefix['odu'], current_time, user_name)
                                # result =
                                # common_reconciliation.update_reconcilation_controller(host_id,device_type,table_prefix['odu'],current_time,user_name)
                            elif device_type == UNMPDeviceType.idu4:
                                rec_obj = idu_profiling_bll.IduReconcilation()
                                result = rec_obj.update_device_reconcilation_controller(
                                    host_id, device_type, table_prefix['idu'], True, user_name)
                            elif device_type == UNMPDeviceType.ap25:
                                rec_obj = ap_profiling_bll.Reconciliation()
                                result = rec_obj.update_configuration(
                                    host_id, device_type, table_prefix['ap'], current_time, user_name)

                    else:
                        flag = 1
                        result["success"] = 1
                        dic_result["result"] = "Device is busy, Device " + \
                            str(host_status_dic[
                                host_op_status]) + " is in progress. please wait ..."
        else:
            flag = 1
            result["success"] = 1
...
class GetAdminState(object):
    ...
    def admin_state_get(self, host_id, device_type):
        ...
        host_data = sqlalche_obj.session.query(
            Hosts).filter(Hosts.host_id == host_id).all()
        get_operation_dic = {
            'ru.ruConfTable.adminstate': '1.3.6.1.4.1.26149.2.2.3.1.7.1', 'ru.syncClock.syncConfigTable.adminStatus': '1.3.6.1.4.1.26149.2.2.11.3.1.1.1',
            'ru.ra.raConfTable.raAdminState': '1.3.6.1.4.1.26149.2.2.13.2.1.3.1'}
        get_dic = {
                ...
                if snmp_get['result'][j] != -1:
                    if j == "ru.ruConfTable.adminstate":
                        ...
                        if device_type == UNMPDeviceType.odu100:
                            ...
                            sync_data = sqlalche_obj.session.query(
                                SetOdu16SyncConfigTable).filter(SetOdu16SyncConfigTable.config_profile_id == host_data[0].config_profile_id).all()
        ...
        if snmp_operation_get['success'] == 0:
            ...
            ra_status = sqlalche_obj.session.query(
                Odu100RaStatusTable).filter(Odu100RaStatusTable.host_id == host_id).order_by(desc(Odu100RaStatusTable.timestamp)).limit(1).all()
                ...
                if snmp_operation_get['result'][j] != -1:
                    if j == "ru.ruConfTable.adminstate":
                        if len(ru_status) > 0:
                            ...
                            # print "hi"
                            admin_states.append(
                                snmp_operation_get['result'][j])
                    elif j == "ru.syncClock.syncConfigTable.adminStatus":
                        if len(sync_status) > 0:
                            sync_status[
                                0].syncoperationalState = snmp_operation_get['result'][j]
                            admin_states.append(
                                snmp_operation_get['result'][j])
                    elif j == "ru.ra.raConfTable.raAdminState":
                        if len(ra_status) > 0:
...
def get_select_list(selected_field, select_list_id, table_name, is_enabled=True, is_readonly=False, attr={}):
    ...
    option_li = ['<option value="" selected="selected">\
                    -- Select %s --\
                </option>' % select_list_id.split('.')[-1]
                 ]

    query = """SELECT m.`value` , m.`name`
        FROM  `{0}_oids_multivalues` AS m
        JOIN  `{0}_oids` AS o ON o.oid_id = m.oid_id
        WHERE o.`oid_name` =  '{1}'\
        """.format(table_name, select_list_id)
