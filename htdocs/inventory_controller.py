...
from inventory import HostUtils as HostUtilsView
...
def manage_host(h):
    ...
    html.new_header("Device", "manage_host.py", header_btn, css_list, js_list)
...
def get_firmware_details(h):
    ...
    if ip_address is not None:
        ...
        if pysnmp_result["success"] == 0:
            ...
            result = {"success": 1, "result": pysnmp_result[
                'result'].values()[0]}
    else:
...
def get_hardware_detail(h):
    ...
    if ip_address is not None:
        ...
        if device_type == "odu100":
            oid_dic = {'hardware_version': '1.3.6.1.4.1.26149.2.2.10.1.3.1',
                       'serial_number': '1.3.6.1.4.1.26149.2.2.10.1.2.1'}
        elif device_type == "idu4":
            oid_dic = {'hardware_version': '1.3.6.1.4.1.26149.2.2.6.1.2.1',
                       'serial_number': '1.3.6.1.4.1.26149.2.1.1.2.1.2.1'}
        else:
            oid_dic = {'hardware_version': '1.3.6.1.4.1.26149.2.2.6.1.2.1',
                       'serial_number': '1.3.6.1.4.1.26149.2.1.1.2.1.2.1'}
        ...
        pysnmp_result = pysnmp_geter(oid_dic, ip_address, port, community)
        if pysnmp_result["success"] == 0:
            hardware_version = pysnmp_result['result'].values()[0]
            serial_number = pysnmp_result['result'].values()[1]
            result = {"success": 0, "hardware_version": hardware_version.split(
                '_')[0], 'serial_number': serial_number.split('_')[0]}
        else:
            result = {"success": 1, "result": pysnmp_result[
                'result'].values()[0]}
    else:
        result = {"success": 1, "result": "Hardware information not found."}
    html.req.content_type = 'application/json'
    html.write(JSONEncoder().encode(result))
...
def odu_master_list(h):
    ...
    master_list_select_list = Common.make_select_list(
        value, name, selected, list_id, list_name, title, message)
...
def form_host(h):
    ...
    snmp_version_select_list = Common.make_select_list(
    ...
    list_id = "dns_state"
    list_name = "dns_state"
    title = "Choose DHCP State"
    dns_state_select_list = Common.make_select_list(
        value, name, selected, list_id, list_name, title)
    ...
    ## -- bellow code is #ed bcoz host parentshould be ajaxed when user select hostgroup
    ...
    host_parent_select_list = Common.make_select_list(
        parents_id, parents_name, selected, list_id, list_name, title)
    ...
    for pr in hostgroups:
        ...
        hostgroups_name.append(pr.hostgroup_name)

    host_hostgroups_select_list = Common.make_select_list(
        hostgroups_id, hostgroups_name, selected, list_id, list_name, title)
    ...
    host_vendor_select_list = Common.make_select_list(
        vendors_id, vendors_name, selected, list_id, list_name, title, message)
    ...
    host_os_select_list = Common.make_select_list(
        os_id, os_name, selected, list_id, list_name, title, message)
    ...
    host_priority_select_list = Common.make_select_list(
        priority_id, priority_name, selected, list_id, list_name, title, message)
    ...
    host_state_select_list = Common.make_select_list(
        state_id, state_name, selected, list_id, list_name, title, message)
    ...
    message = "-- Select Root Node --"
    master_list_select_list = Common.make_select_list(
        master_value, master_name, selected, list_id, list_name, title, message)
...
def get_master_mac_from_slave(h):
    ...
    port = Validation.is_number(port) and port or 161
    du = DeviceUtility()
    ra_mac_dict = du.get_master_mac_from_slave(
        str(ip_address).split(":")[0], community, port)
    hst_bll = HostBll()
    ra_mac = ""
...
def get_network_details(h):
    ...
    port = Validation.is_number(port) and int(port) or 161
    device_type = html.var("device_type")
    result = {"netmask": "", "gateway": "", "dns_state": "",
              "primary_dns": "", "secondary_dns": ""}
    if UNMPDeviceType.ap25 == device_type:
        snmp_result1 = pysnmp_get_node(
            "1.3.6.1.4.1.26149.10.1.2", ip_address, port, community)
        snmp_result2 = pysnmp_get(
            "1.3.6.1.4.1.26149.10.1.3.1.1", ip_address, port, community)
        ...
        if snmp_result1["success"] == 0 and snmp_result2["success"] == 0 and len(snmp_result1["result"][1]) > 4:
            ...
            result["dns_state"] = snmp_result2["result"][
                "1.3.6.1.4.1.26149.10.1.3.1.1"]
        ...
        snmp_result = pysnmp_get_table(
            "1.3.6.1.4.1.26149.2.2.9.1", ip_address, port, community)
        ...
        snmp_result = pysnmp_get_table(
            "1.3.6.1.4.1.26149.2.1.2.4.1", ip_address, port, community)
        ...
        snmp_result = pysnmp_get_table_ccu(
            "1.3.6.1.4.1.26149.3.5.1", ip_address, port, community)
        if snmp_result["success"] == 0 and len(snmp_result["result"][1]) > 7:
            out["success"] = 0
            result["netmask"] = snmp_result["result"][1][3]
...
def is_duplicate_host_with_mac_address(h):
    ...
    html.req.content_type = 'application/json'
    html.write(JSONEncoder(
    ).encode(HostBll().is_duplicate_host_with_mac_address(mac_address)))
...
def is_duplicate_host_with_ip_address(h):
    ...
    html.req.content_type = 'application/json'
    html.write(JSONEncoder(
    ).encode(HostBll().is_duplicate_host_with_ip_address(ip_address)))
...
def is_duplicate_host_with_host_alias(h):
    ...
    html.req.content_type = 'application/json'
    html.write(JSONEncoder(
    ).encode(HostBll().is_duplicate_host_with_host_alias(host_alias)))
...
def add_host(h):
    ...
    try:
        lb = LicenseBll()
        device_type = html.var("device_type")
        lb_result = True#lb.check_license_for_host(device_type)
        if lb_result == True:
            ...
            host_alias = html.var("host_alias")
            ip_address = html.var("ip_address")
            phone_no = html.var("phone_no")
            mac_address = html.var("mac_address").upper(
            ) if html.var("mac_address") else html.var("mac_address")
            ...
            idu4_management_mode = html.var(
                "idu4_management_mode") != None and html.var("idu4_management_mode") or 0
            ...
            device_path = html.var("device_path")
            number_of_ports = html.var("number_of_ports")
            if number_of_ports:
                number_of_ports = str(number_of_ports)
                if not number_of_ports.isdigit():
            ...
            for sp in sp_list:
                hsp_list.append([sp[0], (None != html.var("service_parameter_" + str(sp[0])) and 1 or 0) ])
            
            hsd_dict = {}
            if hsp_list:
                hsd_dict = {
                    "service_username": html.var("service_username"),
                    "service_user_id": html.var("service_user_id"),
                    "service_password": html.var("service_password")
                }
            
            hst_bll = HostBll()                             # creating the HostBll object
            host = hst_bll.add(
                host_name, host_alias, ip_address, phone_no, mac_address, device_type_id, netmask, gateway, primary_dns,
                ...
                node_type, master_id, firmware_version, is_reconciliation, hsp_list, hsd_dict, device_path, number_of_ports)
            if isinstance(host, ErrorMessages):
                result = {"success": 1, "msg": str(host.error_msg)}
            elif isinstance(host, Exception):
...
def edit_host(h):
    ...
    if lb_result == True:
        ...
        if circular_check_result['success'] == 0:
            ...
            result = {
                "success": 1,
                "msg": "Circular reference detected for host: %s " % (host_alias)
            }
            html.write(JSONEncoder().encode(result))
            return
        hst_bll = HostBll(
        ...
        if str(ip_update) == "1":
            ...
            if isinstance(result_dic, dict) and result_dic.get("success") == 0:
                host = hst_bll.edit(
                    host_id, host_name, host_alias, ip_address, phone_no, mac_address, device_type_id, netmask, gateway,
                    ...
                    node_type, master_id, firmware_version, hsp_list, hsd_dict, device_path, number_of_ports)
                if isinstance(host, ErrorMessages):
                    result = {"success": 1, "msg": str(host.error_msg)}
                elif isinstance(host, Exception):
            ...
            host = hst_bll.edit(
                host_id, host_name, host_alias, ip_address, phone_no, mac_address, device_type_id, netmask, gateway, primary_dns,
                secondary_dns, dns_state, odu100_management_mode, odu100_vlan_tag, idu4_management_mode, idu4_vlan_tag,
                ...
                master_id, firmware_version, hsp_list, hsd_dict, device_path, number_of_ports)
            if isinstance(host, ErrorMessages):
                result = {"success": 1, "msg": str(host.error_msg)}
            elif isinstance(host, Exception):
                result = {"success": 1, "msg": "sysError", "h": str(host)}
...
def del_deleted_host(h):
    ...
    host_count = hst_bll.delete_deleted_host(
        host_ids.split(","), nms_instance, updated_by)
    if isinstance(host_count, ErrorMessages):
        result = {"success": 1, "msg": str(host_count.error_msg)}
    elif isinstance(host_count, Exception):
        result = {"success": 1, "msg": "sysError"}
...
def add_deleted_host(h):
    ...
    host_count = hst_bll.add_deleted_host(
        host_ids.split(","), nms_instance, updated_by)
    if isinstance(host_count, ErrorMessages):
        result = {"success": 1, "msg": str(host_count.error_msg)}
    elif isinstance(host_count, Exception):
...
def manage_hostgroup(h):
    ...
    html.new_header(
        "Device groups", "manage_hostgroup.py", header_btn, css_list, js_list)
    ...
    if len(hostgroup_id_list) > 0 or flag_value == 0:
        ...
        for hg in all_hostgroups:
            ...
            if hg.hostgroup_id in temp_list:
                ...
                hostgroups_list[indx][
                    5] += hg.group_name != None and hg.group_name or ""
...
def add_hostgroup(h):
    ...
    hostgroup_id = hg_bll.add(
        hostgroup_name, hostgroup_alias, timestamp, created_by,
        creation_time, is_deleted, updated_by, is_default, nms_instance, group_name)
...
def manage_service(h):
    ...
    # ui-1.8.4.custom.css","css/jquery.multiselect.css"]
    css_list = [
    ...
    header_btn = ""
    html.new_header(
        "Services", "manage_service.py", header_btn, css_list, js_list)
...
def edit_service_details(h):
    ...
    html_str = Service.edit_service_details(
        host_details.host_alias, srv_bll.grid_view_service(host_id))
    html.write(html_str)
...
def apply_service_changes(h):
    ...
    srv_bll = ServiceBll(user_id)
    result = srv_bll.set_service_time(
        hosts_list, service_name, service_time, user_name)
    html.write(JSONEncoder().encode(result))
    ...
    if hosts_list == [""]:
        ...
        try:
            for host_id in all_hosts:
                ...
                serv = html.live.query(query_service)
                html.live.set_prepend_site(False)
                result = update_service_box(
                    srv_bll.grid_view_service(host_id), serv)
...
# Start - Utils
def get_service_parameters(h):
    global html
    html = h
    device_type_id = html.var("device_type_id")
    hu = HostUtils()
    sps = hu.get_service_parameters(device_type_id)
    hu.close_database_connection()
    sp_html = ""
    sp_param_html = ""
    sp_state = False
    for sp in sps:
        sp_state = True
        sp_html += Host.get_service_parameter_html({
            "service_parameter_id": sp[0],
            "parameter_name": sp[1],
            #"parameter_index": sp[2],
            #"parameter_type": sp[3],
            #"min_value": sp[4],
            #"max_value": sp[5],
            #"is_active": sp[6],
    ...
    if sp_state:
        sp_param_html = Host.get_service_parameter_details_html()
    html.req.content_type = 'application/json'
    html.write(JSONEncoder().encode({
        "status": sp_state,
        "html": sp_html,
        "param_html": sp_param_html
    }))

def host_service_parameter_graph(h):
    global html
    html = h
    host_id = html.var("host_id")
    time_interval = html.var("time_interval")
    data = []
    status = False
    hu = HostUtils()
    hsp_list = hu.get_host_service_parameter_data(host_id, time_interval)
    hu.close_database_connection()
    for hsp in hsp_list:
        if hsp[0] and hsp[1]:
            status = True
            data.append([str(hsp[0]).split(";"),time.mktime(hsp[1].timetuple())*1000])
...
def update_form_host_service_parameter(h):
    global html
    html = h
    data = {}
    parameter_id = html.var("parameter_id")
    hu = HostUtils()
    service_data = hu.get_host_service_parameter_data_by_id(parameter_id)
    min_value = ""
    max_value = ""
    min_arm = ""
    max_arm = ""
    ...
    if service_data:
        ...
        min_value = service_data[0] and service_data[0] or ""
        max_value = service_data[1] and service_data[1] or ""
        min_arm = service_data[2] and service_data[2] or ""
        max_arm = service_data[3] and service_data[3] or ""
        parameter_type = service_data[4] and service_data[4] or ""
    html.write(HostUtilsView.update_form_host_service_parameter(parameter_id, min_value, max_value, min_arm, max_arm, parameter_type))
    hu.close_database_connection()
    
def update_host_service_parameter(h):
    from example import isfloat
    global html
    html = h
    data = {}
    parameter_id = html.var("parameter_id")
    min_value = html.var("min_value")
    max_value = html.var("max_value")
    min_arm = html.var("min_arm")
    max_arm = html.var("max_arm")
    parameter_type = html.var("parameter_type")
    parameter_type_error = False
    msg = ""
    if None != min_value and "" != min_value:
        if "integer" == parameter_type:
            if not str(min_value).isdigit():
                msg = "Please enter a valid Integer value."
                parameter_type_error = True
        elif "float" == parameter_type:
            if (not isfloat(min_value) and not str(min_value).isdigit()):
                msg = "Please enter a valid Float value."
                parameter_type_error = True
    if None != max_value and "" != max_value:
        if "integer" == parameter_type:
            if not str(max_value).isdigit():
                msg = "Please enter a valid Integer value."
                parameter_type_error = True
        elif "float" == parameter_type:
            if (not isfloat(max_value) and not str(max_value).isdigit()):
                msg = "Please enter a valid Float value."
                parameter_type_error = True
        
    if parameter_type_error:
        data["status"] = 0
        data["msg"] = msg
    else:
        hu = HostUtils()
        update_status = hu.update_host_service_parameter_data_by_id(parameter_id, min_value, max_value, min_arm, max_arm)
        if update_status:
            data["status"] = 1
            ...
            data["status"] = 0
            data["msg"] = "Server is busy, please try again later."
        hu.close_database_connection()
    html.req.content_type = 'application/json'
...
def relay_action(h):
    ...
    host_id = html.var("host_id")
    arm = html.var("arm")
    action = html.var("action") # ON/OFF
    action_code = "0"
    if action and "ON" == action:
        action = "OFF"
        action_code = "0"
    else:
        action = "ON"
    ...
    try:
        if arm and str(arm).isdigit():
            ...
            if "ACCEPTEDACCEPTED" in data:
                MESSAGE = "IDCB000482;UPDATE;RLJ%s%s;RDDE#" % (str(arm), action_code)
                sckt.send(MESSAGE)
                time.sleep(2)
                data = sckt.recv(BUFFER_SIZE)
                if "IDSB000482;ACK;END#" in data:
                    hu = HostUtils()
                    hu.update_relay_status(host_id, int(arm), action)
                    hu.close_database_connection()
                    json_data["status"] = 1
                    json_data["msg"] = "Updated successfully"
                else:
                    json_data["status"] = 0
                    json_data["msg"] = "Device is busy right now, service is in process."
                    json_data["error"] = "TCP Connection refused"
        else:
            json_data["status"] = 0
            json_data["msg"] = "Invalid data"
    except Exception, e:
        json_data["status"] = 0
        json_data["msg"] = "Device is busy right now, Service is in process."
        json_data["error"] = "Error: (%s)" % str(e)
    html.req.content_type = 'application/json'
    html.write(JSONEncoder().encode(json_data))
# End - Utils
