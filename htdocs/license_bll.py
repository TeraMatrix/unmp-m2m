...
class LicenseBll(object):
    ...
    def update_device_type(self):
        ...
        try:
            ...
            device_list = [k for k, v in dom_dict.items(
            ) if v != '' and int(v) > 0]
            if device_list > 0:
    ...
    def get_total_host(self, device_type=None):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = []
            hosts.append(session.query(
                Hosts).count())          # execute query and fetch data
            if device_type:
    ...
    def get_total_hostgroup(self):
        ...
        try:
            ...
            hostgroups = session.query(
                Hostgroups).count()          # execute query and fetch data
            return hostgroups
        except Exception, e:
            return e
        finally:
    ...
    def get_total_usergroup(self):
        ...
        try:
            session = Session()                 # creating new session object
            usergroups = session.query(Groups).filter(
                Groups.is_default == 0).count()          # execute query and fetch data
            return usergroups
        except Exception, e:
            return e
        finally:
    ...
    def get_device_type(self, host_id):
        ...
        try:
            session = Session()                 # creating new session object
            device_type = session.query(Hosts.device_type_id).filter(
                Hosts.host_id == host_id).all()         # execute query and fetch data
    ...
    def check_license_for_host_edit(self, device_type=None, host_id=None):
        ...
        try:
            if device_type != self.get_device_type(host_id):
                ...
                host_co = int(allowed_host_li[1]) - int(total_host_li[1])
                # return " eeeeeee
                # %s,%s"%(str(allowed_host_li),str(total_host_li))
                if host_co > 0:
                    return True
                else:
                    return False
    ...
    def validate_license(self, file_data):
        ...
        try:
            ...
            for elem in dom_element:
                ...
                issue_date = elem.getAttribute("issuedate")
            dom_element = dom.getElementsByTagName(
                "host")            # get the element by tag name
            for elem in dom_element:
            ...
            dom_element = dom.getElementsByTagName(
                "user")            # get the element by tag name
            for elem in dom_element:
            ...
            if company == None or expire_date == None or lic_id == None or issue_date == None:
                ...
                is_negative = 0
                liObj = session.query(LicenseDetails.license_id).filter(
                    LicenseDetails.license_id == lic_id).all()
                # liObj2 =
                # session.query(LicenseDetails.issued_client).filter(LicenseDetails.issued_client!=company).all()
                if len(liObj) <= 0:  # and len(liObj2) <= 0:
                    ...
                    if now_date_obj <= date_obj:
                        ...
                        hostgroup_li[
                            2] = self.get_allowed_object(dom, "hostgroup")
                        hostgroup_li[3] = self.get_total_hostgroup()
                        hostgroup_li[4] = int(
                            hostgroup_li[2]) - int(hostgroup_li[3])
                        if hostgroup_li[4] < 0:
                        ...
                        usergroup_li[
                            2] = self.get_allowed_object(dom, "usergroup")
                        usergroup_li[3] = self.get_total_usergroup()
                        usergroup_li[4] = int(
                            usergroup_li[2]) - int(usergroup_li[3])
                        if usergroup_li[4] < 0:
